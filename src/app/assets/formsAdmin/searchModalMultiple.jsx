...
const SearchModalMultiple = (props) => {
  ...
  useEffect(() => {
    if (modelName && fields) {
      if ((fieldName === 'vendor_id' || fieldName === 'customer_id' || fieldName === 'manufacturer_id')) {
        const searchValueMultiple = `["&",["company_id","in",[${company}]], ["${fieldName === 'customer_id' ? 'customer' : 'supplier'}", "=", true],"|","|",["email", "ilike", "${searchValue}"],["display_name", "ilike", "${searchValue}"],["mobile", "ilike", "${searchValue}"]]`;
  ...
  return (
    <Row>
      <Col sm="12" md="12" lg="12" xs="12">
        <Card className="p-2 bg-lightblue h-100">
          <CardBody className="bg-color-white p-1 m-0">
            <Row className="pl-2 pr-2 pb-0 pt-0 content-center">
              ...
              <Col sm="12" md="1" lg="1" xs="12" className="mt-3">
      ...
      {/* <Modal size={(addVendorInfo && addVendorInfo.data) ? 'sm' : 'xl'} className="border-radius-50px modal-dialog-centered purchase-modal manufacturer-modal" isOpen={addVendorModal}>
        <ModalHeaderComponent title={modalHead} imagePath={false} closeModalWindow={() => { showAddVendorModal(false); onReset(); }} response={addVendorInfo} />
      ...
      <Dialog maxWidth="lg" open={addVendorModal} fullWidth>
        <DialogHeader title={modalHead} onClose={() => { showAddVendorModal(false); onReset(); }} response={createTenantinfo} imagePath={false} />
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
              ...
              <AddVendor
                ...
                type={fieldName === 'vendor_id' || fieldName === 'manufacturer_id' ? 'vendor' : 'customer'}
                afterReset={() => { showAddVendorModal(false); onReset(); }}
              />
