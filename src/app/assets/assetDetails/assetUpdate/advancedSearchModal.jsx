    ...
    for (let i = 0; i < assetData.length; i += 1) {
      tableTr.push(
        <tr className={selectedId === assetData[i].id ? 'text-info cursor-pointer' : 'cursor-pointer'} onClick={() => handleRowClick(assetData[i])} key={i}>
          <td className="p-2"><span className="font-weight-400">{fieldName === 'category_id' ? getDefaultNoValue(assetData[i].path_name) : getDefaultNoValue(assetData[i].name)}</span></td>
        </tr>,
      );
    }
    return tableTr;
