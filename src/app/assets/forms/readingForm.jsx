...
import {
  generateErrorMessage, getAllCompanies, decimalKeyPress, integerKeyPress, lettersOnly, getColumnArrayByIdWithArray, getArrayFromValuesById, getDateTimeSeconds,
} from '../../util/appUtils';
...
const ReadingForm = (props) => {
  ...
  const {
    ...
    validation_required,
    aggregate_timeperiod, recurrent,
    check_list_id, team_category_id, maintenance_type, priority, category_id, alarm_recipients,
  ...
  const {
    assetReadings, readingsList, checkList, teamCategoryInfo, alarmCategoryInfo, alarmRecipientsInfo, alarmActionsInfo, equipmentInfo,
  ...
  useEffect(() => {
    (async () => {
      if (userInfo && userInfo.data && alarmCategoryOpen) {
        await dispatch(getAlarmCategory(companies, appModels.ALARMCATEGORY, alarmCategoryKeyword));
      }
    })();
  ...
  useEffect(() => {
    (async () => {
      if (userInfo && userInfo.data && alarmRecipientOpen) {
        await dispatch(getAlarmRecipients(companies, appModels.ALARMRECIPIENTS, alarmRecipientKeyword));
      }
    })();
  ...
  useEffect(() => {
    (async () => {
      if (userInfo && userInfo.data && alarmActionOpen) {
        await dispatch(getAlarmActions(companies, appModels.ALARMACTIONS, alarmActionKeyword));
      }
    })();
  ...
  useEffect(() => {
    (async () => {
      if (userInfo.data && measureReadingOpen) {
        await dispatch(getReading(appModels.READINGS, measureReadingKeyword));
      }
    })();
  ...
  useEffect(() => {
    if (reading_id && reading_id.id) {
      const readingsData = readingsList.data.filter((obj) => (obj.id === reading_id.id));
  ...
  return (
    <>
      <Row className="mb-3">
        <Col xs={12} sm={12} lg={6} md={12}>
          <Col xs={12} sm={12} md={12} lg={12}>
            <h6 className="ml-1 mb-3">Reading Configuration</h6>
          </Col>
          ...
          <Col xs={12} md={12} lg={12} sm={12} className="mt-2 ml-1">
            <Label for={isActive.name} className="font-weight-600 m-0">
              Status
            </Label>
            <br />
            <CheckboxFieldGroup
          ...
          <Col xs={12} md={12} lg={12} sm={12} className="ml-1">
            <CheckboxField
              className="ml-1"
            ...
            <FormikAutocomplete
              ...
              renderInput={(params) => (
                <TextField
                  ...
                  variant="outlined"
                  className="without-padding"
                  placeholder={company_id && company_id.name ? company_id.name : getOldData(company_id)}
                  InputProps={{
                    ...params.InputProps,
                    endAdornment: (
                      <>
        ...
        <Col xs={12} sm={12} lg={6} md={12}>
          <Col xs={12} sm={12} md={12} lg={12}>
            <h6 className="ml-1 mb-3">Validation Configuration</h6>
          </Col>
          <Col xs={12} md={12} lg={12} sm={12}>
            <CheckboxField
              className="ml-1"
      ...
      <Row>
        <Col xs={12} sm={12} lg={6} md={12}>
          <Col xs={12} sm={12} md={12} lg={12}>
            <h6 className="ml-1 mb-3">Analytics Configuration</h6>
          </Col>
          <Col sm="12" md="12" xs="12" lg="12">
            <FormikAutocomplete
          ...
          {(condition) && (condition.value === 'Delta' || condition === 'Delta') && (
            <>
              <Col xs={12} sm={12} md={12} lg={12}>
                <h6 className="ml-1 mt-3 mb-3">Delta</h6>
              </Col>
              <Col xs={12} sm={12} lg={12} md={12}>
                <FormikAutocomplete
          ...
          {(condition) && (condition.value === 'Run Hour' || condition === 'Run Hour') && (
            <>
              <Col xs={12} sm={12} md={12} lg={12}>
                <h6 className="ml-1 mt-3 mb-3">Run Hour</h6>
              </Col>
              <Col xs={12} sm={12} lg={12} md={12}>
                <InputField
          ...
          {(condition) && (condition.value === 'Threshold' || condition === 'Threshold') && (
            <>
              <Col xs={12} sm={12} md={12} lg={12}>
                <h6 className="ml-1 mt-3 mb-3">Threshold</h6>
              </Col>
              <Col xs={12} sm={12} lg={12} md={12}>
                <InputField
        ...
        <Col xs={12} sm={12} lg={6} md={12}>
          <Col xs={12} sm={12} md={12} lg={12}>
            <h6 className="ml-1 mb-3">Actions</h6>
          </Col>
          <Col sm="12" md="12" xs="12" lg="12">
            <FormikAutocomplete
              name={toDo.name}
          ...
          {(to_do) && (to_do.value === 'Maintenance Order' || to_do === 'Maintenance Order') && (
            <>
              <Col xs={12} sm={12} md={12} lg={12}>
                <h6 className="ml-1 mt-3 mb-3">Maintenance Order</h6>
              </Col>
              <Col xs={12} sm={12} lg={12} md={12}>
                <FormikAutocomplete
          ...
          {(to_do) && (to_do.value === 'Alarm' || to_do === 'Alarm') && (
            <>
              <Col xs={12} sm={12} md={12} lg={12}>
                <h6 className="ml-1 mt-3 mb-3">Alarm</h6>
              </Col>
              <Col xs={12} sm={12} lg={12} md={12}>
                <InputField
                  name={alarmName.name}
          ...
          {(to_do) && (to_do.value === 'Reading Logs' || to_do === 'Reading Logs') && (
            <>
              <Col xs={12} sm={12} md={12} lg={12}>
                <h6 className="ml-1 mt-3 mb-3">Reading Logs</h6>
              </Col>
              <Col xs={12} sm={12} lg={12} md={12}>
                <DateTimeField
              ...
              <Col xs={12} sm={12} lg={12} md={12}>
                <FormikAutocomplete
                  ...
                  value={measure_id && measure_id.name ? measure_id.name : getOldData(measure_id)}
                  loading={readingsList && readingsList.loading}
                  getOptionSelected={(option, values) => option.display_name === values.display_name}
                  getOptionLabel={(option) => (typeof option === 'string' ? option : option.display_name)}
                  options={readingOptions}
