...
const unLocked = (props) => {
  const {
    quotationDetails, unlockModal, atFinish,
  } = props;
  const dispatch = useDispatch();
  const storeValue = 'purchase';
  const [modal, setModal] = useState(unlockModal);
  ...
  return (
    <Modal size="md" className="border-radius-50px modal-dialog-centered" isOpen={unlockModal}>
      <ModalHeaderComponent fontAwesomeIcon={faLockOpen} closeModalWindow={toggle} title="Unlock" response={stateChangeInfo} />
      <ModalBody>
            ...
            <CardBody data-testid="success-case" className="bg-lightblue p-3">
              <Row>
                ...
                <Col md="8" xs="8" sm="8" lg="8" className="ml-2">
                  <Row>
                    <h6 className="mb-1">{quotationData.name}</h6>
                  </Row>
                  <Row>
        ...
        <Row className="justify-content-center">
          {stateChangeInfo && (stateChangeInfo.data || stateChangeInfo.status) && (quotationDetails && !quotationDetails.loading) && (
            <SuccessAndErrorFormat response={stateChangeInfo} successMessage="This RFQ has been unlocked successfully.." />
          )}
        ...
        {quotationData.state === storeValue
          ...
          : (
            <Button
              ...
              size="sm"
              className="mr-1"
              onClick={() => handleStateChange(quotationData.id, 'button_unlock', 'unlocked')}
            >
              {((quotationDetails && quotationDetails.loading) || (stateChangeInfo && stateChangeInfo.loading && stateType === 'unlocked')) ? (
...
unLocked.propTypes = {
  ...
  unlockModal: PropTypes.oneOfType([
    PropTypes.bool,
    PropTypes.string,
  ]).isRequired,
