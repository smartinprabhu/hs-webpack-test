...
const Lock = (props) => {
  const {
    quotationDetails, lockModal, atFinish,
  } = props;
  const dispatch = useDispatch();
  const storeValue = 'done';
  const [modal, setModal] = useState(lockModal);
  ...
  return (
    <Modal size="md" className="border-radius-50px modal-dialog-centered" isOpen={lockModal}>
      <ModalHeaderComponent fontAwesomeIcon={faLock} closeModalWindow={toggle} title="Lock" response={stateChangeInfo} />
      <ModalBody>
        ...
        <Row className="justify-content-center">
          {stateChangeInfo && (stateChangeInfo.data || stateChangeInfo.status) && (quotationDetails && !quotationDetails.loading) && (
            <SuccessAndErrorFormat response={stateChangeInfo} successMessage="This RFQ has been locked successfully.." />
          )}
        ...
        {quotationData.state === storeValue
          ...
          : (
            <Button
              ...
              size="sm"
              className="mr-1"
              onClick={() => handleStateChange(quotationData.id, 'button_done', 'lock')}
            >
              {((quotationDetails && quotationDetails.loading) || (stateChangeInfo && stateChangeInfo.loading && stateType === 'lock')) ? (
