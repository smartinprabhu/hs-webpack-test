...
const CancelQuotation = (props) => {
  const {
    quotationDetails, cancelModal, atFinish,
  } = props;
  const dispatch = useDispatch();
  const storeValue = 'cancel';
  ...
  return (
    <Modal size="md" className="border-radius-50px modal-dialog-centered" isOpen={cancelModal}>
      <ModalHeaderComponent fontAwesomeIcon={faTimesCircle} closeModalWindow={toggle} title="Cancel" response={stateChangeInfo} />
      <ModalBody>
        ...
        <Row className="justify-content-center">
          {stateChangeInfo && (stateChangeInfo.data || stateChangeInfo.status) && (quotationDetails && !quotationDetails.loading) && (
            <SuccessAndErrorFormat response={stateChangeInfo} successMessage="This RFQ has been cancelled successfully.." />
          )}
        ...
        {quotationData.state === storeValue
          ...
          : (
            <Button
              ...
              size="sm"
              className="mr-1"
              onClick={() => handleStateChange(quotationData.id, 'button_cancel', 'cancel')}
            >
              {((quotationDetails && quotationDetails.loading) || (stateChangeInfo && stateChangeInfo.loading && stateType === 'cancel')) ? (
