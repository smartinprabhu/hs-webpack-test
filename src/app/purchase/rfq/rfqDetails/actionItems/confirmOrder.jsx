...
const ConfirmOrder = (props) => {
  const {
    quotationDetails, confirmModal, atFinish,
  } = props;
  ...
  return (
    <Modal size="md" className="border-radius-50px modal-dialog-centered" isOpen={confirmModal}>
      <ModalHeaderComponent fontAwesomeIcon={faCheckCircle} closeModalWindow={toggle} title="Confirm Order" response={stateChangeInfo} />
      <ModalBody>
        ...
        <Row className="justify-content-center">
          {stateChangeInfo && stateChangeInfo.data && (quotationDetails && !quotationDetails.loading) && (
            <SuccessAndErrorFormat response={stateChangeInfo} successMessage="This order has been confirmed successfully.." />
          )}
        ...
        {quotationData.state === storeValue
          ...
          : (
            <Button
              ...
              size="sm"
              className="mr-1"
              onClick={() => handleStateChange(quotationData.id, 'button_confirm', 'confirm')}
            >
              {((quotationDetails && quotationDetails.loading) || (stateChangeInfo && stateChangeInfo.loading && stateType === 'confirm')) ? (
...
ConfirmOrder.propTypes = {
  ...
  confirmModal: PropTypes.oneOfType([
    PropTypes.bool,
    PropTypes.string,
  ]).isRequired,
