...
const SendPoByEmail = (props) => {
  const {
    quotationDetails, sendPoModal, atFinish,
  } = props;
  const dispatch = useDispatch();
  const editor = useRef(null);
  const [modal, setModal] = useState(sendPoModal);
  ...
  return (
    <>
      ...
      <Modal size={(emailChangeInfo && emailChangeInfo.data) ? 'sm' : 'xl'} className="border-radius-50px modal-dialog-centered" isOpen={sendPoModal}>
...
SendPoByEmail.propTypes = {
  quotationDetails: PropTypes.oneOfType([
    PropTypes.object,
    PropTypes.array,
  ]).isRequired,
  sendPoModal: PropTypes.oneOfType([
    PropTypes.bool,
    PropTypes.string,
  ]).isRequired,
  atFinish: PropTypes.func.isRequired,
};
export default SendPoByEmail;
