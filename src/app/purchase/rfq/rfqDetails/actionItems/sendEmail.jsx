...
import {
  createComposeEmail, emailStateChange, updateRfq, getTemplateDetail, resetPrint, getVendorDetail,
...
const SendEmail = (props) => {
  const {
    quotationDetails, emailModal, atFinish,
  } = props;
  ...
  const {
    composeEmailInfo, emailChangeInfo, updateRfqInfo, printReportInfo, templateInfo, templateDetails, vendorDetails,
  ...
  const loading = (printReportInfo && printReportInfo.loading)
    ...
    || (quotationDetails && quotationDetails.loading) || (updateRfqInfo && updateRfqInfo.loading)
  ...
  return (
    <>
      ...
      <Modal size={(updateRfqInfo && updateRfqInfo.data) ? 'sm' : 'xl'} className="border-radius-50px modal-dialog-centered" isOpen={emailModal}>
        <ModalHeaderComponent title="Compose Email" fontAwesomeIcon={faEnvelope} closeModalWindow={toggle} size="lg" response={updateRfqInfo} />
        <ModalBody className="pt-0">
          {(loading || (composeEmailInfo && composeEmailInfo.data) || (emailChangeInfo && (emailChangeInfo.data || emailChangeInfo.status)) || (updateRfqInfo && updateRfqInfo.data)) ? '' : (
            <>
          ...
          <Row className="justify-content-center">
            {updateRfqInfo && updateRfqInfo.data && (quotationDetails && !quotationDetails.loading) && (
              <SuccessAndErrorFormat response={updateRfqInfo} successMessage="Email has been sent successfully.." />
            )}
...
SendEmail.propTypes = {
  ...
  emailModal: PropTypes.oneOfType([
    PropTypes.bool,
    PropTypes.string,
  ]).isRequired,
