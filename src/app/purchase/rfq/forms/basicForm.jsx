...
import {
  getStockPickingTypes, getPurchaseAgreements, getPurchaseRequests,
...
const BasicForm = (props) => {
  ...
  const {
    quotationDetails, pickingTypes, purchaseAgreementInfo, purchaseRequestInfo,
  ...
  useEffect(() => {
    if ((userInfo && userInfo.data) && (purchaseAgreementId && purchaseAgreementId.length > 0) && (vendorId && vendorId.length > 0)) {
      dispatch(getPurchaseAgreements(companies, appModels.PURCHASEAGREEMENT, vendorId[0]));
      setFieldValue('partner_id', vendorId);
      setFieldValue('requisition_id', purchaseAgreementId);
    }
  }, [userInfo]);
  ...
  const showAgreementModal = () => {
    ...
    if (partner_id && partner_id.length && partner_id.length === 0) {
      setOtherFieldValue(false);
    } else if (partner_id && partner_id.length && partner_id.length > 0) {
      setOtherFieldValue(partner_id[0]);
    } else {
      setOtherFieldValue(partner_id && partner_id.id ? partner_id.id : false);
    }
    setCompanyValue(userInfo && userInfo.data ? companies : '');
    setColumns(['id', 'name', 'user_id', 'type_id', 'state']);
    setExtraMultipleModal(true);
  };
  ...
  const purchaseAgreementDisable = (purchaseAgreementId && purchaseAgreementId.length > 0) && (vendorId && vendorId.length > 0);

  return (
    <>
      <Row className="mb-1 purchase-request-form">
        <Col xs={12} sm={6} lg={6} md={6}>
          <Col xs={12} sm={12} lg={12} md={12}>
            <FormikAutocomplete
              ...
              disabled={fieldDisable || purchaseAgreementDisable}
          ...
          <Col xs={12} sm={12} md={12} lg={12}>
            <FormikAutocomplete
              ...
              formGroupClassName="m-1"
              disabled={purchaseAgreementDisable}
              open={agreementOpen}
              oldValue={getOldData(requisition_id)}
              size="small"
              value={requisition_id && requisition_id.name ? requisition_id.name : getOldData(requisition_id)}
              onOpen={() => {
        ...
        <Col xs={12} sm={6} lg={6} md={6}>
          ...
          <Col xs={12} sm={12} md={12} lg={12}>
            <FormikAutocomplete
              ...
              size="small"
              disabled={fieldDisable}
              oldValue={getOldData(request_id)}
              value={request_id && request_id.name ? request_id.name : getOldData(request_id)}
              onOpen={() => {
                setRequestOpen(true);
                setRequestKeyword('');
              }}
