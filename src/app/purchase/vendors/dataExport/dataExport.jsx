...
import {
  getVendorExport, setInitialValues,
...
const DataExport = (props) => {
  ...
  const exportFileName = getExportFileName('Vendors')

  const [columns, setColumns] = useState(fields);

  const { userInfo } = useSelector((state) => state.user);
  const {
    vendorsCount, vendorFilters, vendorExportInfo, vendorRows,
  ...
  useEffect(() => {
    if ((userInfo && userInfo.data) && (vendorsCount && vendorsCount.length)) {
      const offsetValue = 0;
      const states = vendorFilters.statuses ? getColumnArray(vendorFilters.statuses, 'id') : [];
      const languages = vendorFilters.languages ? getColumnArrayById(vendorFilters.languages, 'id') : [];
      const customFilters = vendorFilters.customFilters ? queryGenerator(vendorFilters.customFilters) : '';
      const rows = vendorRows.rows ? vendorRows.rows : [];
      dispatch(getVendorExport(companies, appModels.PARTNER, vendorsCount.length, offsetValue, columns, states, languages, customFilters, rows));
    }
  }, [userInfo, vendorsCount]);
  ...
  const appliedFilters = filterStringGenerator(vendorFilters);

  useEffect(() => {
    if (fields && fields.length > 0 && vendorExportInfo && vendorExportInfo.data && vendorExportInfo.data.length > 0) {
  ...
  useEffect(() => {
    if (pdfHeader && pdfHeader.__wrapped__ && pdfHeader.__wrapped__.length > 0) {
      ...
      if (vendorExportInfo && vendorExportInfo.data && vendorExportInfo.data.length > 0) {
        vendorExportInfo.data.map((data) => {
          data.company_type = data.company_type && customData && customData.companyTypeNames
          && customData.companyTypeNames[data.company_type] ? customData.companyTypeNames[data.company_type].label : '';
          data.lang = data.lang && customData && customData.langugageTypes && customData.langugageTypes[data.lang] ? customData.langugageTypes[data.lang].label : '';
  ...
  useEffect(() => {
    if (exportType === 'pdf') {
      ...
      savePdfContent('Vendors', pdfHeaders, pdfBody, `${exportFileName}.pdf`, companyName, appliedFilters);
  ...
  return (
    <Row>
      <Col md="12" sm="12" lg="6">
        <div className="p-3 text-center">
          ...
          <Button
            ...
            color="dark"
            className="bg-zodiac"
            disabled={vendorExportInfo && vendorExportInfo.loading}
            onClick={() => {
              setExportType('pdf');
            }}
          >
            {(vendorExportInfo && vendorExportInfo.loading) ? (
      ...
      {vendorExportInfo && vendorExportInfo.err && (
        <Col md="12" sm="12" lg="12" xs="12">
          <SuccessAndErrorFormat response={vendorExportInfo} />
        </Col>
      )}
      <Col md="12" sm="12" lg="12" className="d-none">
        <div id="print_report">
          ...
          <br />
          {exportType === 'excel' && (
            <DataTable columns={checkFieldExists(dataFields)} data={vendorExportInfo && vendorExportInfo.data ? vendorExportInfo.data : []} propertyAsKey="id" />
          )}
        </div>
      </Col>
