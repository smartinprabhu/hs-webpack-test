...
const ConfirmOrder = (props) => {
  const {
    detail, confirmModal, atFinish,
  } = props;
  const dispatch = useDispatch();
  const storeValue = 'confirm';
  ...
  return (
    <Modal size={(stateChangeInfo && (stateChangeInfo.data || stateChangeInfo.status)) ? 'sm' : 'md'} className="border-radius-50px modal-dialog-centered" isOpen={confirmModal}>
      ...
      <ModalBody className="pt-0">
        ...
        <Row className="justify-content-center m-2">
          {stateChangeInfo && stateChangeInfo.data && (detail && !detail.loading) && (
        ...
        {requestData.state === storeValue
          ...
          : (
            <Button
              ...
              size="sm"
              className="mr-1"
              onClick={() => handleStateChange(requestData.id, 'button_confirm')}
            >
              Confirm
            </Button>
          )}
