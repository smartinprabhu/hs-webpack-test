...
const CancelRequest = (props) => {
  ...
  return (
    <Modal size={(stateChangeInfo && (stateChangeInfo.data || stateChangeInfo.status)) ? 'sm' : 'md'} className="border-radius-50px modal-dialog-centered" isOpen={cancelModal}>
      ...
      <ModalBody className="pt-0">
        ...
        <Row className="justify-content-center m-2">
          {stateChangeInfo && (stateChangeInfo.data || stateChangeInfo.status) && (detail && !detail.loading) && (
            <SuccessAndErrorFormat response={stateChangeInfo} successMessage="This purchase request has been cancelled successfully.." />
          )}
        ...
        {requestData.state === storeValue
          ...
          : (
            <Button
              ...
              size="sm"
              className="mr-1"
              onClick={() => handleStateChange(requestData.id, 'button_cancel')}
            >
              Confirm
            </Button>
          )}
