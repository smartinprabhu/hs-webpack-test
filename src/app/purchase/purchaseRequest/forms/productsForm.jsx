...
import { getTaxData, getCustomerTaxes, getMeasures } from '../../purchaseService';
...
import { getTotalTax } from '../../utils/utils';
...
const ProductsForm = (props) => {
  ...
  const { taxInfo, customerTaxesInfo } = useSelector((state) => state.purchase);
  ...
  useEffect(() => {
    if (partsSelected && partsSelected.length > 0) {
      setFieldValue('request_line', partsSelected);
    } else {
      setPartsData([]);
      setFieldValue('request_line', []);
    }
  }, [partsSelected]);
  ...
  useEffect(() => {
    ...
    setFieldValue('request_line', []);
    setFieldValue('amount_untaxed', '');
    setFieldValue('amount_tax', '');
    setFieldValue('amount_total', '');
  }, []);
  ...
  useEffect(() => {
    if (partId && (productInfo && productInfo.data)) {
      ...
      if (data.length > 0) {
        ...
        newData[partIndex].price_subtotal = data && data[0] && data[0].standard_price ? (getFloatValue(data[0].standard_price) * getFloatValue(newData[partIndex].product_qty)) : 0;
  ...
  useEffect(() => {
    if (partsSelected && partsSelected.length) {
      const withoutTaxTotal = getTotalFromArray(partsSelected && partsSelected.length ? partsSelected : [], 'price_subtotal');
      const totalTaxRs = getTotalTax(partsSelected && partsSelected.length ? partsSelected : []);
  ...
  const loadEmptyTd = () => {
    const newData = partsData;
    newData.push({
      product_id: '', date_planned: new Date(), product_uom_id: 1, product_qty: numToFloat(quantity), price_unit: numToFloat(0), taxes_id: '', price_subtotal: 0,
    });
    setPartsData(newData);
    setPartsAdd(Math.random());
  };
  ...
  const withoutTaxTotal = getTotalFromArray(partsSelected && partsSelected.length ? partsSelected : [], 'price_subtotal');
  const totalTaxRs = getTotalTax(partsSelected && partsSelected.length ? partsSelected : [], 'taxes_id', 'price_subtotal');
  const totalPay = withoutTaxTotal + totalTaxRs;
  ...
  return (
    <>
      ...
      <Row className="products-list thin-scrollbar mr-2 mt-1 mb-1">
        <Col xs={12} sm={12} md={12} lg={12}>
                ...
                <tr key={index} className="font-weight-400">
                  ...
                  <td className="p-2">
                    <DatePicker
                      ...
                      value={pl.date_planned ? moment(pl.date_planned, 'DD/MM/YYYY HH:mm:ss') : ''}
                      className="w-100"
