...
import {
  getTaxData, getCustomerTaxes, getTaxes, getRequestProduct, getMeasures,
...
const ProductUpdateForm = React.memo((props) => {
  ...
  const {
    taxInfo, requestProductOrders, taxesInfo, requestDetails, customerTaxesInfo,
  ...
  useEffect(() => {
    if (requestProductOrders && requestProductOrders.data) {
      const newData = requestProductOrders.data.map((cl) => ({
        ...cl,
        id: cl.id,
  ...
  const currentState = requestDetails && requestDetails.data ? requestDetails.data[0].state : false;
  ...
  const totalTaxRs = getTotalTax(filterNotRemovedData && filterNotRemovedData.length ? filterNotRemovedData : [], 'taxes_id', 'price_subtotal');
  const totalPay = withoutTaxTotal + totalTaxRs;
  const loading = (requestProductOrders && requestProductOrders.loading) || (taxesInfo && taxesInfo.loading);
