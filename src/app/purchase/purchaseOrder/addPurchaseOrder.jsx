...
import ProductsForm from '../rfq/forms/productsForm';
import ProductUpdateForm from '../rfq/forms/productUpdateForm';
import AdditionalForm from '../rfq/forms/additionalForm';
...
import {
  getArrayNewFormat, getArrayNewFormatUpdate, trimJsonObject, getAllowedCompanies, getDateTimeUtc,
} from '../../util/appUtils';
import { getNewRequestArray, getOrderLinesRequest } from '../utils/utils';
import {
  checkProductId, checkSheduleDate, checkProductsScheduleDate, checkRequiredFields,
} from '../rfq/utils/utils';
import ProductDetails from '../rfq/rfqDetails/products';
...
const AddPurchaseOrder = (props) => {
  const {
    afterReset, closeAddModal, editId, closeEditModal,
  } = props;
  ...
  useEffect(() => {
    if ((userInfo && userInfo.data) && ((addRfqInfo && addRfqInfo.data) || (updateRfqInfo && updateRfqInfo.data))) {
      const statusValues = [];
      const orderValues = [];
      const vendorValues = [];
      const customFilters = '';
      dispatch(getRequestQuotationList(
        companies, appModels.PURCHASEORDER, limit, offsetValue, statusValues, orderValues, vendorValues, customFilters, sortByValue, sortFieldValue, true,
      ));
      dispatch(getRequestQuotationCount(companies, appModels.PURCHASEORDER, statusValues, orderValues, vendorValues, customFilters, true));
      dispatch(getPartsData([]));
    }
  }, [userInfo, addRfqInfo, updateRfqInfo]);
  ...
  function handleSubmit(values) {
    if (quotationDetails && quotationDetails.data) {
      ...
      const postDataValues = {
        ...
        order_line: values.order_line && values.order_line.length > 0 ? getArrayNewFormatUpdate(orderData) : false,
      ...
      const requestId = values.request_id && values.request_id.id ? values.request_id.id : '';
      const requisitionId = values.requisition_id && values.requisition_id.id ? values.requisition_id.id : false;
        ...
        >
          {({
            ...
            <Form id={formId}>
                  ...
                  || (updateRfqInfo && updateRfqInfo.data) || (updateRfqInfo && updateRfqInfo.loading)) ? ('') : (
                    <ThemeProvider theme={theme}>
                      <div className="pt-1 pr-5 pl-2 pb-5 mr-2 ml-4">
                        <BasicForm formField={formField} setFieldValue={setFieldValue} setFieldTouched={setFieldTouched} />
                        {!currentState ? (
                          <>
              ...
              {(addRfqInfo && addRfqInfo.data) && (
                <SuccessAndErrorFormat response={addRfqInfo} successMessage="Purchase Order added successfully.." />
              )}
              ...
              {(updateRfqInfo && updateRfqInfo.data) && (
                <SuccessAndErrorFormat response={updateRfqInfo} successMessage="Purchase Order updated successfully.." />
              )}
              <br />
              {(quotationDetails && !quotationDetails.data && !quotationDetails.loading) && (
              ...
              {(quotationDetails && quotationDetails.data && !quotationDetails.loading) && (
                <div className="bg-lightblue sticky-button-1250drawer">
                  {(updateRfqInfo && !updateRfqInfo.data) && (
                  <Button
                    disabled={!isValid || updateRfqInfo.loading || ((partsSelected && !partsSelected.length > 0) || !checkProductId(partsSelected) || !checkRequiredFields(partsSelected))}
                    type="submit"
                    size="sm"
                     variant="contained"
                  >
