...
import { getMeasures } from '../../purchaseService';
import '../../../preventiveMaintenance/preventiveMaintenance.scss';
import {
  generateArrayFromValue, getArrayFromValuesById,
  getColumnArrayByIdWithArray,
  generateErrorMessage,
  getAllowedCompanies, getFloatValue, decimalKeyPress, numToFloat,
} from '../../../util/appUtils';
import {
  getMaintenanceCostAnalysis,
} from '../../../adminSetup/setupService';
import { checkProductId, checkRequiredFields, getRequiredMessage } from '../utils/utils';
...
const ProductsForm = (props) => {
  ...
  const [analyticKeyword, setAnalyticKeyword] = useState('');
  ...
  useEffect(() => {
    if (userInfo && userInfo.data) {
      dispatch(getMaintenanceCostAnalysis(companies, appModels.ACCOUNT, analyticKeyword));
    }
  }, [analyticKeyword]);
  ...
  useEffect(() => {
    if (maintenanceCostInfo && maintenanceCostInfo.data && maintenanceCostInfo.data.length > 0) {
      const { data } = maintenanceCostInfo;
      setAnalyticOptions(data.map((cl) => ({
        ...cl, value: cl.id, label: cl.name,
      })));
    }
  }, [maintenanceCostInfo]);
  ...
  const onAnalyticChange = (e, index) => {
    const newData = partsData;
    newData[index].account_analytic_id = e ? [e.id, e.name] : [];
    setPartsData(newData);
    setPartsAdd(Math.random());
  };

  const onAnalyticKeyWordChange = (event) => {
    setAnalyticKeyword(event.target.value);
  };
  ...
  const onUomChange = (e, index) => {
    const newData = partsData;
    newData[index].product_uom_id = e ? [e.id, e.name] : [];
    setPartsData(newData);
    setPartsAdd(Math.random());
  };
  ...
  const onScheduleDateChange = (e, index) => {
    const newData = partsData;
    newData[index].schedule_date = e;
    setPartsData(newData);
    setPartsAdd(Math.random());
  };

  const onQtyOrderChange = (e, index) => {
    const newData = partsData;
    newData[index].qty_ordered = e.target.value ? getFloatValue(e.target.value) : e.target.value;
    setPartsData(newData);
    setPartsAdd(Math.random());
  };
  ...
  const onAnalyticKeywordClear = (e, index) => {
    const newData = partsData;
    newData[index].account_analytic_id = '';
    setPartsData(newData);
    setPartsAdd(Math.random());
    setOpen(false);
  };
    ...
    <>
      ...
      <Row className="ml-0">
        <Col xs={12} sm={12} md={12} lg={12}>
          ...
          {((partsSelected && !partsSelected.length > 0) || (!checkProductId(partsData))) && (
            ...
            <tbody>
              {(partsSelected && partsSelected.length > 0 && partsSelected.map((pl, index) => (
                <tr key={index} className="font-weight-400">
                  ...
                  <td className="p-2">
                    <Input type="text" disabled autoComplete="off" name="qty_ordered" value={getFloatValue(pl.qty_ordered)} onKeyPress={decimalKeyPress} onChange={(e) => onQtyOrderChange(e, index)} />
                  </td>
                  <td className="p-2">
                    <FormControl>
                      <Autocomplete
                  ...
                  <td className="p-2">
                    <DatePicker
                      ...
                      value={pl.schedule_date ? moment(pl.schedule_date, 'DD/MM/YYYY HH:mm:ss') : ''}
                      disabledDate={disabledDate}
                      className="w-100"
                      showTime={{ format: 'HH:mm:ss' }}
                      onChange={(e) => onScheduleDateChange(e, index)}
                    />
                  </td>
                  <td className="p-2">
                    <FormControl>
                      <Autocomplete
                        ...
                        value={pl.account_analytic_id && pl.account_analytic_id.length ? pl.account_analytic_id[1] : ''}
                        getOptionSelected={(option, value) => (value.length > 0 ? option.label === value.label : '')}
                        getOptionLabel={(option) => (typeof option === 'string' ? option : option.label)}
                        options={analyticOptions}
                        onChange={(e, data) => { onAnalyticChange(data, index); }}
                        renderInput={(params) => (
                          <TextField
                            {...params}
                            onChange={onAnalyticKeyWordChange}
                            variant="outlined"
                            className={pl.account_analytic_id && pl.account_analytic_id.length ? 'without-padding custom-icons' : 'without-padding custom-icons2'}
                            placeholder="Search & Select"
                            InputProps={{
                              ...params.InputProps,
                              endAdornment: (
                                <>
                                  {(maintenanceCostInfo && maintenanceCostInfo.loading) && (analytic === index) ? <CircularProgress color="inherit" size={20} /> : null}
                                  <InputAdornment position="end">
                                    {pl.account_analytic_id && pl.account_analytic_id[0] && (
                                      <IconButton
                                        aria-label="toggle password visibility"
                                        onClick={(e, data) => { onAnalyticKeywordClear(data, index); }}
                                      >
                                        <BackspaceIcon fontSize="small" />
                                      </IconButton>
                                    )}
                      ...
                      />
                      {((maintenanceCostInfo && maintenanceCostInfo.err)
                              && (analytic === index)) && (<FormHelperText><span className="text-danger">{generateErrorMessage(maintenanceCostInfo)}</span></FormHelperText>)}
                    </FormControl>
                  </td>
                  <td className="p-2">
