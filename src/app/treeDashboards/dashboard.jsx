...
import SldDynamic from '../sustanablity/Sld/sldDynamic';
import Energy from '../sustanablity/Energy/Energy';
import { useTheme } from '../ThemeContext';
import Water from '../sustanablity/Water/water';
import SiteLevel from '../sustanablity/Water/siteWater';
import Energysite from '../sustanablity/Energy/Energysite';
import Emmision from '../sustanablity/Emissions/Emmision';
...
const Dashboard = React.memo((props) => {
  ...
  const { ninjaDashboardCode, treeDashboardInfo } = useSelector((state) => state.analytics);
  ...
  const fetchChildObjects = (node) => {
    ...
    if (node.source === '12' && node.dashboard_code && node.show === 'dashboard') {
      ...
      dispatch(getNinjaCode(node.dashboard_code, appModels.NINJABOARD));
  ...
  const closeDrawer = () => {
    if (selectedInlineData && selectedInlineData.id) {
      if (selectedInlineData.source === '12' && selectedInlineData.dashboard_code && selectedInlineData.show === 'dashboard') {
        dispatch(resetNinjaCode());
        dispatch(getNinjaCode(selectedInlineData.dashboard_code, appModels.NINJABOARD));
              ...
              <>
                ...
                <Tooltip placement="right" title={`Click the icon to view the ${node.show}`}>
                  <span aria-hidden className="ml-2" onClick={() => fetchChildObjectsData(node)}>
                    {node.show && (node.show === 'dashboard' || node.show === 'dashboard_external' || node.show === 'dashboard_external_meter') && (
                    <LeaderboardIcon
                      sx={(theme) => ({
                    ...
                    {node.show && (node.show === 'sld' || node.show === 'sld_external') && (
                    <AccountTreeIcon
                      sx={(theme) => ({
                        color: node && selectedTreeChild === node.id ? 'white' : theme.palette.primary.main,
                      })}
                    ...
                    {node.show && node.show === 'tool_scope3' && (
                    <AutoFixHighIcon
                      sx={(theme) => ({
                        color: node && selectedTreeChild === node.id ? 'white' : theme.palette.primary.main,
                      })}
    ...
    <>
      ...
      <div className="insights-box">
        ...
        >
            ...
            <Box sx={{ p: 1 }}>
              ...
              >
                ...
                <IconButton
                  ...
                  title="Collapse"
                >
                  <ChevronLeftIcon sx={themes !== 'dark' ? { color: '#ffffff' } : {}} fontSize="small" />
                </IconButton>
              </h6>
              <TreeView
          ...
          <div
            style={{ width: showSidebar ? '79.5%' : '100%', overflow: 'auto' }}
          ...
          >
            ...
            <Box sx={{ p: 0.5 }}>
              <IconButton
                ...
                title="Expand"
              >
                <ChevronRightIcon sx={themes !== 'dark' ? { color: 'white' } : {}} fontSize="small" />
              </IconButton>
            </Box>
            )}
            {selectedInlineData && selectedInlineData.target === 'inline' && (
              <Box sx={{ p: 1 }}>
                ...
                {selectedInlineData && selectedInlineData.id && selectedInlineData.show === 'sld' && (
                <SldDynamic label={selectedInlineData.label} uuid={dashboardUuid} code={selectedInlineData.dashboard_code} />
                )}
                {selectedInlineData && selectedInlineData.id && selectedInlineData.show === 'sld_external' && (
                <SldDynamic label={selectedInlineData.label} uuid={dashboardUuid} code={selectedInlineData.dashboard_code} isExternal type={selectedInlineData.ev_type} />
                )}
                  {selectedInlineData && selectedInlineData.id && selectedInlineData.show === 'dashboard_external' && selectedInlineData.ev_type === 'water' && (
                  <SiteLevel
                    uuid={dashboardUuid}
                    code={selectedInlineData.dashboard_code}
                    headerText={selectedInlineData.label}
                  />
                  )}
                {selectedInlineData && selectedInlineData.id && selectedInlineData.show === 'dashboard_external' && selectedInlineData.ev_type === 'energy' && (
                <Energysite
                  uuid={dashboardUuid}
                  code={selectedInlineData.dashboard_code}
                  headerText={selectedInlineData.label}
                ...
                {selectedInlineData && selectedInlineData.id && selectedInlineData.show === 'tool_scope3' && (
                <Emmision headerText={selectedInlineData.label} />
                )}
                {selectedInlineData && selectedInlineData.id && selectedInlineData.show === 'dashboard_external_meter' && selectedInlineData.ev_type === 'energy' && (
                <Energy uuid={dashboardUuid} code="ENERGYV3" equipmentId={selectedInlineData.equipment_id} headerText={selectedInlineData.label} />
                )}
                {selectedInlineData && selectedInlineData.id && selectedInlineData.show === 'dashboard_external_meter' && selectedInlineData.ev_type === 'water' && (
                <Water uuid={dashboardUuid} code="WATERV3" equipmentId={selectedInlineData.equipment_id} headerText={selectedInlineData.label} />
                )}
