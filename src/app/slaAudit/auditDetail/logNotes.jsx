...
const LogNotes = () => {
  ...
  const loading = slaAuditDetails && slaAuditDetails.loading;
  const isErr = slaAuditDetails && slaAuditDetails.err;
  const inspDeata = slaAuditDetails && slaAuditDetails.data && slaAuditDetails.data.length ? slaAuditDetails.data[0] : false;
        ...
        {isChecklist && (
          <div>
            <Table responsive className="mb-0 mt-2 font-weight-400 border-0 assets-table" width="100%">
              ...
              <tbody>
                {getRow(slaAuditDetails.data.length > 0 ? slaAuditDetails.data[0].status_logs : [])}
              </tbody>
            </Table>
            <hr className="m-0" />
          </div>
        )}
        <DetailViewFormat detailResponse={slaAuditDetails} />
