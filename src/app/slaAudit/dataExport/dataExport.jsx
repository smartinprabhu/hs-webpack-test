...
const DataExport = (props) => {
  ...
  useEffect(() => {
    if ((userInfo && userInfo.data) && (slaAuditCount && slaAuditCount.length)) {
      const offsetValue = 0;
      const customFiltersQuery = slaAuditFilters && slaAuditFilters.customFilters ? queryGeneratorV1(slaAuditFilters.customFilters) : '';
      dispatch(getSlaAuditExport(companies, appModels.SLAAUDIT, slaAuditCount.length, offsetValue, apiFields, customFiltersQuery, rows, sortedValue.sortBy, sortedValue.sortField));
    }
  }, [userInfo, slaAuditCount]);
  ...
  const appliedFilters = filterStringGenerator(slaAuditFilters);
  ...
  useEffect(() => {
    if (fields && fields.length > 0 && slaAuditExportInfo && slaAuditExportInfo.data && slaAuditExportInfo.data.length > 0) {
  ...
  useEffect(() => {
    if (exportType === 'pdf') {
      ...
      savePdfContent('SLA Audit', pdfHeaders, pdfBody, `${exportFileName}.pdf`, companyName, appliedFilters);
  ...
  return (
    <Row>
      <Col md="12" sm="12" lg="6">
        <div className="p-3 text-center">
          ...
          <Button
            ...
            color="dark"
            className="bg-zodiac"
            disabled={slaAuditExportInfo && slaAuditExportInfo.loading}
            onClick={() => {
              setExportType('pdf');
            }}
          >
            {(slaAuditExportInfo && slaAuditExportInfo.loading) ? (
      ...
      {slaAuditExportInfo && slaAuditExportInfo.err && (
        <Col md="12" sm="12" lg="12" xs="12">
          <SuccessAndErrorFormat response={slaAuditExportInfo} />
        </Col>
      )}
      <Col md="12" sm="12" lg="12" className="d-none">
        <div id="print_report">
          ...
          {exportType === 'excel'
            ? (
              <DataTable
                columns={slaAuditExportInfo && slaAuditExportInfo.data && slaAuditExportInfo.data.length ? checkFieldExists(dataFields) : []}
                data={slaAuditExportInfo && slaAuditExportInfo.data ? slaAuditExportInfo.data : []}
                propertyAsKey="id"
              />
            )
            : ''}
