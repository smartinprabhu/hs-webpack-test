...
export function getSlaStateLabel(staten, hasSecondApprove, isSecondApproved) {
  if (complianceActions && complianceActions.status[staten]) {
    return (
      hasSecondApprove ? (
        <>
          {staten === 'Approved' && (
          <Tooltip title={isSecondApproved ? 'Second Level Approved' : 'First Level Approved'} placement="top">
            <Badge
              color={staten === 'Approved' && hasSecondApprove && !isSecondApproved ? 'warning' : complianceActions.status[staten].color}
        ...
        <>
          {staten === 'Approved' && (
          <Tooltip title="Approved (Second Level Approval Not Required)" placement="top">
            <Badge
              color={complianceActions.status[staten].color}
              className="badge-text no-border-radius"
              pill
...
export function getSlaStatusText(staten, hasSecondApprove, isSecondApproved) {
  let res = '';
  if (complianceActions && complianceActions.status[staten]) {
    if (staten === 'Approved') {
      if (hasSecondApprove) {
        if (isSecondApproved) {
          res = 'Approved (Second Level)';
        } else {
          res = 'Approved (First Level)';
        }
      } else {
        res = 'Approved (Second Level Approval Not Required)';
      }
    } else {
      res = complianceActions.status[staten].label;
    }
  }
  return res;
}

export function getSlaStateLabelAudit(staten) {
  if (complianceActions && complianceActions.status[staten]) {
    return (
      <Badge
        color={complianceActions.status[staten].color}
