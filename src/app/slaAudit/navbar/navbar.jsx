...
import TrackerIcon from '@images/slaAudit/slaAudit.svg';
import searchActiveIcon from '@images/icons/searchGrey.svg';
import buildingTrackerIcon from '@images/slaAudit/slaAuditBlue.svg';
...
import {
  getSlaAuditList, getSlaAuditCount, getSlaAuditFilters, resetAddSlaAuditInfo,
  getSlaAuditDetail,
} from '../auditService';
...
const Navbar = (props) => {
  ...
  const [currentTab, setActive] = useState('SLA Audit');
  ...
  const {
    slaAuditInfo, slaAuditCount, slaAuditCountErr, slaAuditCountLoading, addSlaAuditInfo,
  ...
  useEffect(() => {
    if (complianceList) {
      ...
      history.push({ pathname: '/sla-audits', state: { id: complianceList.id } });
    }
  }, [complianceList]);

  useEffect(() => {
    if (searchOpen) {
      dispatch(getSlaAuditList(companies, appModels.SLAAUDIT, limit, offset, false, 'DESC', 'create_date', keyword));
      dispatch(getSlaAuditCount(companies, appModels.SLAAUDIT, [], keyword));
      setTotalCount(0);
    }
  ...
  const loading = (slaAuditCountLoading) || (slaAuditInfo && slaAuditInfo.loading);
  const error = (slaAuditInfo && slaAuditInfo.err) || (slaAuditCountErr);

  useEffect(() => {
    if ((slaAuditInfo && slaAuditInfo.err) || (slaAuditCountErr)) {
      setTotalCount(0);
    }
  }, [slaAuditInfo, slaAuditCountErr]);

  useEffect(() => {
    if (slaAuditCount && slaAuditCount.length) {
      setTotalCount(slaAuditCount.length);
    } else {
      setTotalCount(0);
    }
  }, [slaAuditCount]);
  ...
  useEffect(() => {
    if (addSlaAuditInfo && addSlaAuditInfo.data && addSlaAuditInfo.data.length && id === 'Insights') {
            ...
            <>
              {menuList && menuList.map((menu) => (
                sideNav && sideNav.data && sideNav.data[menu] && (
                  <NavItem key={menu} onClick={() => { dispatch(getSlaAuditFilters([], [], [], [])); }}>
              ...
              <Col md="6" sm="12" lg="6" className="">
                ...
                <Popover trigger="legacy" className="search-popover" placement="bottom" isOpen={searchOpen} target="asset-search" toggle={() => setSearchOpen(!searchOpen)}>
                  <PopoverBody>
                    <Row>
                      <Nav>
                        <NavLink className="insights-nav-link" onClick={() => setActive('SLA Audit')} href="#">SLA Audit</NavLink>
                      </Nav>
                    </Row>
                    <hr className="mb-2 mt-1" />
                    {currentTab === 'SLA Audit' ? (
                      <>
                        {slaAuditInfo && slaAuditInfo.loading && (
                          <Loader />
                        )}
                        {slaAuditInfo && slaAuditInfo.err && (
                          <ErrorContent errorTxt={generateErrorMessage(slaAuditInfo)} />
                        )}
                        {slaAuditInfo && slaAuditInfo.data && slaAuditInfo.data.length && slaAuditInfo.data.map((sv) => (
