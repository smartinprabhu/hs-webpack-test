...
import {
  getPantryDashboard, getPantryFilters,
} from '../pantryService';
import {
  ...
  useEffect(() => {
    if (userInfo && userInfo.data && currentTab && currentTab !== 'Custom') {
      ...
      if (dates.length > 0) {
        const start = dates[0];
        const end = dates[1];
        dispatch(getPantryDashboard(start, end));
      } else {
        const monthDates = getDatesOfQuery('This month');
...
useEffect(() => {
  ...
  const total = getTotal(pantryDashboard && pantryDashboard.data ? pantryDashboard.data : []);

  const gridData = generateArrayFromValue(pantryDashboard && pantryDashboard.data ? pantryDashboard.data : [], 'ks_dashboard_item_type', 'ks_tile');
              ...
              <CardFooter className="bg-med-blue border-0 pt-0">
                {actions.datasets && actions.datasets.length && actions.datasets.length > 0 && actions.datasets[0] !== 0 && actions.code ? (
                  <Button
                     variant="contained"
                    size="sm"
                    onClick={() => onLoadPantry(customData.dashboardStatus[actions.code].tags)}
