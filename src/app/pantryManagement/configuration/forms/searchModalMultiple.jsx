...
const SearchModalMutliple = (props) => {
  ...
  const handleTableCellChange = (event) => {
    ...
    if (checked) {
      ...
      let items = [];
      if ((fieldName === 'spaces_ids')) {
        items = [{ id: values.id, path_name: values.path_name }];
      }
    ...
    for (let i = 0; i < arrayData.length; i += 1) {
      tableTr.push(
        <tr className={selectedId === arrayData[i].id ? 'text-info cursor-pointer' : 'cursor-pointer'} key={i}>
            ...
            {(fieldName === 'spaces_ids') && (
              <div className="checkbox">
                <Input
                  ...
                  className="ml-0"
                  name={fieldName === 'spaces_ids' ? arrayData[i].path_name : arrayData[i].name}
          ...
          {(fieldName === 'spaces_ids') && (
            <>
              <td><span className="font-weight-400">{getDefaultNoValue(arrayData[i].space_name)}</span></td>
              <td><span className="font-weight-400">{getDefaultNoValue(arrayData[i].path_name)}</span></td>
            </>
          )}
        </tr>,
      );
          ...
          <CardBody className="bg-color-white p-1 m-0">
            <Row className="pl-2 pr-2 pb-0 pt-0">
              ...
              <Col sm="12" md="3" lg="3" xs="12" className="pl-0">
                <div className="float-right">
                  <FormGroup>
                    <Input type="input" name="search" id="exampleSearch" placeholder="Search" onKeyDown={onSearchChange} onChange={onSearchChange} className="subjectticket bw-2 mt-2" />
                  </FormGroup>
                </div>
              </Col>
            </Row>
            ...
            <Modal isOpen={modalAlert} toggle={toggleAlert} size="sm">
              ...
              <ModalBody>
                Maximum allowed
                {' '}
                {fieldName === 'spaces_ids' ? 'spaces' : 'equipments'}
                {' '}
                : 10
              </ModalBody>
              <ModalFooter>
