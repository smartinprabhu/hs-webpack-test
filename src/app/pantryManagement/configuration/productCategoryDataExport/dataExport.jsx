...
import {
  ...
  getAllowedCompanies, queryGeneratorV1, getCompanyTimezoneDate, filterStringGeneratorDynamic, getDefaultNoValue,
} from '../../../util/appUtils';
...
const DataExport = (props) => {
  ...
  const exportFileName = getExportFileName('Product Category')
  ...
  const appliedFilters = filterStringGeneratorDynamic(productCategoryFilters);
  ...
  useEffect(() => {
    if ((userInfo && userInfo.data) && (productCategoryCount && productCategoryCount.length)) {
      ...
      dispatch(getProductCategoryListExport(companies, appModels.PRODUCTCATEGORY, productCategoryCount.length, offsetValue, InventoryModule.inventoryProductCategoryApiFields, customFiltersQuery, rows, sortedValue.sortBy, sortedValue.sortField));
    }
  }, [userInfo, productCategoryCount]);
  ...
  useEffect(() => {
    if (exportType === 'pdf') {
      ...
      savePdfContent('Product Category', pdfHeaders, pdfBody, `${exportFileName}.pdf`, companyName, appliedFilters);
      dispatch(setInitialValues(false, false, false, false, false));
    } else if (exportType === 'excel') {
      exportTableToExcel('print_report_product_category', exportFileName);
  ...
  return (
    <Row>
      ...
      <Col md="12" sm="12" lg="12" className="d-none">
        <div id="print_report_product_category">
          {exportType === 'excel' && (
          <table align="center">
            <tbody>
              <tr><td style={{ textAlign: 'center' }} colSpan={5}><b>Product Category Report</b></td></tr>
              <tr>
                <td>Company</td>
