...
import DataExport from '../reports/dataExport/inventoryReportExport';
import {
  generateErrorMessage, getDefaultNoValue, extractNameObject, getListOfOperations
} from '../../util/appUtils';
import { groupByQuantity } from '../utils/utils';
import { groupByMultiple } from '../../util/staticFunctions';
import { getCompanyTimezoneDate } from '../../util/appUtils'
import {
  getPantryFilters,
} from '../pantryService';
import {
...
import InventoryReportPrint from './dataExport/inventoryReportPrint';
...
const InventoryReport = (props) => {
  ...
  const { pantryFilters, reports } = useSelector((state) => state.pantry);
  const {
    reportId,
  ...
  const pantryFilterList = pantryFilters && pantryFilters.customFilters && pantryFilters.customFilters.filter((item) => item.key === 'pantry_id');
    ...
    <Card className="p-2 mb-2 bg-lightblue h-100">
      <CardBody className="p-1 bg-color-white m-0 side-filters-list thin-scrollbar">
        ...
        {dateFilterList && dateFilterList.length !== 0 && reports && reports.data && reports.data.length > 0 ? (
          <>
            <Row className="pt-2" id='export-inventory-pantry-reports'>
            ...
            <div className="d-none">
              <InventoryReportPrint reportName={reportName} selectedReportDate={selectedReportDate} />
            </div>
          </>
        ) : (
          <>
            {(((reports && reports.loading) || (reports && reports.err) || (reports && reports.data)) || (pantryFilterList && pantryFilterList.length === 0) ? '' : (
