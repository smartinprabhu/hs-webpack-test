...
export function checkProduct(array){
  let result = false;
  let count = 0;
  const arrayNew = array && array.length && array.filter((item) => !item.isRemove);
  for (let i = 0; i < arrayNew.length; i += 1) {
    if ((arrayNew[i].product_id && arrayNew[i].product_id.length > 0) && (arrayNew[i].ordered_qty > 0)) {
      count += 1;
    }
  }
...
export function checkRemovedProduct(array) {
  let result = false;
...
export function getOrderStateLabel(staten) {
