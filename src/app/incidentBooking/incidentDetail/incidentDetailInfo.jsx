  ...
  const IncidentDetailInfo = (props) => {
    ...
    const switchActionItem = (action) => {
      if (action.displayname !== 'Print Investigation Report' && action.displayname !== 'Print Lessons Learn Report') {
        ...
        dispatch(getEquipmentDocument(viewData.id, appModels.HXINCIDENT, appModels.DOCUMENT));
    ...
    const checkActionAllowed = (actionName) => {
      ...
      if (actionName === 'Complete Recommendation') {
        if (vrState !== 'Analyzed' || !isValidUser() || !viewData.probability_id.is_analysis_required) {
          allowed = false;
        }
      }
      if (actionName === 'Resolve') {
        ...
        if (vrState === 'Acknowledged' && !viewData.probability_id.is_analysis_required) {
          allowed = true;
        }
        if (vrState === 'Analyzed' && !viewData.probability_id.is_analysis_required) {
          allowed = true;
        }
        if (vrState === 'Reported' && !configData.is_acknowledge_required && !viewData.probability_id.is_analysis_required) {
          allowed = true;
        }
      }
    ...
    function getStateName() {
      ...
      if (viewData.state === 'Reported' && !configData.is_acknowledge_required && viewData.probability_id.is_analysis_required) {
      ...
      } else if (viewData.state === 'Acknowledged' && !viewData.probability_id.is_analysis_required) {
        res = '';
      } else if (viewData.state === 'Reported' && !configData.is_acknowledge_required && !viewData.probability_id.is_analysis_required) {
        res = '';
        ...
        res = '';
      } else if (viewData.state === 'Analyzed' && viewData.probability_id.is_analysis_required && !isValidUser()) {
        res = "User don't have the authority to do recommendation.";
      } else if (viewData.state === 'Remediated' && viewData.probability_id.is_analysis_required && !isValidUser()) {
        res = "User don't have the authority to do recommendation.";
      } else if (viewData.state === 'Analyzed' && !viewData.probability_id.is_analysis_required && !isValidUser()) {
        res = customData.status.Resolved.msg;
      } else if (viewData.state === 'Remediated' && !viewData.probability_id.is_analysis_required && !isValidUser()) {
      ...
      !loading && viewData && (
        <>
          ...
          <Col md="12" sm="12" lg="12" className="d-none">
            <InvestigationReport detailData={viewData} isAnalysis={viewData.probability_id.is_analysis_required} isValidate={configData.is_validation_required} />
          </Col>
          <Col md="12" sm="12" lg="12" className="d-none">
