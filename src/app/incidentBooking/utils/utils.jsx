...
export const getChecklistData = (data) => {
  const newData = data.map((cl) => ({
    id: cl.id,
    answer: cl.answer,
    rich_text: cl.rich_text,
  }));
  return newData;
};

export const getChecklistUpdateData = (data) => {
  // const verData = data.filter((item) => item.type === 'Rich Text');
  const newData = data.map((cl) => ({
    ...cl,
    isNew: 'no',
  }));
...
export const getChecklistUpdateErrData = (data) => {
  // const verData = data.filter((item) => item.type === 'Rich Text');
  const newData = data.map((cl) => ({
    ...cl,
    isNew: 'yes',
...
export const getChecklistOtherUpdateData = (data) => {
  // const verData = data.filter((item) => item.type !== 'Rich Text');
  const newData = data.map((cl) => ({
    ...cl,
    isNew: 'no',
...
export const getChecklistOtherUpdateErrData = (data) => {
  // const verData = data.filter((item) => item.type !== 'Rich Text');
  const newData = data.map((cl) => ({
    ...cl,
    isNew: 'yes',
...
export function getArrayNewFormatUpdateChecklist(array) {
  const newArray = [];
  for (let i = 0; i < array.length; i += 1) {
    const newData = [array[i].id ? 1 : 0, array[i].id, { answer: array[i].answer, rich_text: array[i].rich_text }];
