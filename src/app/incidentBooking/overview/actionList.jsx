...
import { getComplianceFilters, resetAddComplianceInfo, resetComplianceTemplate } from '../complianceService';
...
const ActionList = ({ setPopoverModal }) => {
  ...
  useEffect(() => {
    if (userInfo && userInfo.data) {
      ...
      const applies = [];
      const customFilters = [];
      dispatch(getComplianceFilters(statuses, category, applies, customFilters));
    }
  }, [userInfo]);
  ...
  return (
    <Card className="p-2 mt-2 rounded-0 border-0">
      ...
      <CardBody className="p-0 ml-3">
            ...
            <React.Fragment key={actions.id}>
              {(allowedOperations.includes(actionCodes[actions.name])) && (
                <Col sm="6" md="6" lg="6" xs="12" key={actions.id} className="pr-0 mb-2">
                  {actions.name === 'Add Compliance Obligation'
                    ? (
                      <Button
                        id={`action${index}`}
                        onClick={() => { setPopoverModal(false); showAddComplianceModal(true); }}
        ...
        <Modal size={(addAssetInfo && addAssetInfo.data) ? 'sm' : 'xl'} className="border-radius-50px modal-dialog-centered" isOpen={addComplianceModal}>
          <ModalHeaderComponent title="Add Compliance Obligation" imagePath={assetIcon} closeModalWindow={() => { showAddComplianceModal(false); onReset(); }} response={addAssetInfo} />
