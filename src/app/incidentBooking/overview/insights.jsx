...
import {
  getSlaAuditDashboard, getConsumptionTrackerFilters,
} from '../ctService';
import {
...
const Insights = () => {
  ...
  useEffect(() => {
    if (userInfo && userInfo.data && currentTab && currentTab !== 'Custom') {
      ...
      if (dates.length > 0) {
        const start = dates[0];
        const end = dates[1];
        dispatch(getSlaAuditDashboard(start, end));
      } else {
  ...
  const onLoadTracker = (tags) => {
    if (tags) {
      setValue(tags);
      dispatch(getConsumptionTrackerFilters(tags));
    }
  };

  const total = getTotal(slaAuditDashboard && slaAuditDashboard.data ? slaAuditDashboard.data : []);

  if (values.length > 0) {
    return (<Redirect to="/consumption-trackers" />);
  }

  const isUserError = userInfo && userInfo.err;
  const loading = (userInfo && userInfo.loading) || (slaAuditDashboard && slaAuditDashboard.loading);
  const userErrorMsg = generateErrorMessage(userInfo);
  const errorMsg = (slaAuditDashboard && slaAuditDashboard.err) ? generateErrorMessage(slaAuditDashboard) : userErrorMsg;

  const gridData = generateArrayFromValue(slaAuditDashboard && slaAuditDashboard.data ? slaAuditDashboard.data : [], 'ks_dashboard_item_type', 'ks_tile');
