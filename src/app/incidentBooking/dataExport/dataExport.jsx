...
const DataExport = (props) => {
  ...
  const exportFileName = getExportFileName('hx_incidents');
  ...
  useEffect(() => {
    if ((userInfo && userInfo.data) && (incidentHxCount && incidentHxCount.length)) {
      ...
      dispatch(getIncidentsExport(companies, appModels.HXINCIDENT, incidentHxCount.length, offsetValue, apiFields, customFiltersQuery, rows, sortedValue.sortBy, sortedValue.sortField));
    }
  }, [userInfo, incidentHxCount]);

  function checkFieldExists(dataColumns) {
    let dataColumnsList = dataColumns;
    if (columns.some((selectedValue) => selectedValue.includes('assigned_id'))) {
      dataColumnsList = dataColumnsList.concat(tableFields.assignFields);
    }
    if (columns.some((selectedValue) => selectedValue.includes('sub_category_id'))) {
      dataColumnsList = dataColumnsList.concat(tableFields.subCatFields);
    }
    ...
    if (columns.some((selectedValue) => selectedValue.includes('reported_on'))) {
      dataColumnsList = dataColumnsList.concat(tableFields.reportedOnFields);
    }
    ...
    if (pdfHeader && pdfHeader.__wrapped__ && pdfHeader.__wrapped__.length > 0) {
      ...
      if (incidentHxExportInfo && incidentHxExportInfo.data && incidentHxExportInfo.data.length > 0) {
        incidentHxExportInfo.data.map((data) => {
          data.target_closure_date = getCompanyTimezoneDate(data.target_closure_date, userInfo, 'datetime');
  ...
  useEffect(() => {
    if (exportType === 'pdf') {
      ...
      savePdfContent('HX Incidents', pdfHeaders, pdfBody, `${exportFileName}.pdf`, companyName, appliedFilters);
