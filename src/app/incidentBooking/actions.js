...
export function getIncidentsCountInfo(company, model, states, customFilters, globalFilter, keyword) {
  let payload = `domain=[["company_id","in",[${company}]]`;
  if (keyword && keyword.length) {
    payload = `${payload},["name","ilike","${keyword}"]`;
  }
...
export function getHxIncidentConfigInfo(id, modelName) {
  const fields = '["id","category_access","incident_type_access","priority_access","is_sign_off_required","has_incident_type","severity_access","services_access","has_attachment_for_reporting","has_sub_category","has_services_impacted","is_acknowledge_required",("sign_off_role_role_id", ["id", "name"]),("acknowledge_role_id", ["id", "name"]),"is_validation_required",("validator_role_id", ["id", "name"])]';
...
export function updateIncidentNoLoadInfo(id, model, result, noload) {
  const payload = {
    ids: `[${id}]`, values: result,
  };
  return {
    [CALL_API]: {
      endpoint: `write/${model}`,
      types: [!noload ? UPDATE_INCIDENT_NO_INFO : 'UPDATE_INCIDENT_NO_INFO_NO', !noload ? UPDATE_INCIDENT_NO_INFO_SUCCESS : 'UPDATE_INCIDENT_NO_INFO_NO_SUCCESS', !noload ? UPDATE_INCIDENT_NO_INFO_FAILURE : 'UPDATE_INCIDENT_NO_INFO_NO_FAILURE'],
      method: 'PUT',
      payload,
    },
  };
