...
import {
  getChecklistAlarms, getChecklistAlarmsCount, updateAlarm, resetUpdateAlarm,
  resetAlarm,
...
const ChecklistAlarms = (props) => {
  ...
  const {
    alarmsCheckListInfo, alarmsChecklistCount, alarmsChecklistCountLoading, alarmUpdateInfo,
  ...
  useEffect(() => {
    if (userInfo && userInfo.data) {
      dispatch(getChecklistAlarms(companies, userInfo.data.id, appModels.ALARMS, modelName, limit, offset, statusValues, priorityValues, customFilters));
    }
  ...
  useEffect(() => {
    if (userInfo && userInfo.data) {
      setAlarmsList([]);
      dispatch(getChecklistAlarmsCount(companies, userInfo.data.id, appModels.ALARMS, modelName, statusValues, priorityValues, customFilters));
    }
  }, [userInfo, modelName]);
  ...
  useEffect(() => {
    if (alarmsCheckListInfo) {
      if (alarmsCheckListInfo.data) {
        if (alarmsCheckListInfo.data.length) {
          if (alarmsCheckListInfo.data[0].model_name !== modelName) {
            setAlarmsList(alarmsCheckListInfo.data);
          } else {
            setAlarmsList([]);
          }
        } else {
        ...
        if (alarmUpdateInfo && alarmUpdateInfo.err) {
          setAlarmsList([]);
        }
  ...
  }, [alarmsCheckListInfo, alarmsChecklistCount, modelName]);

  const totalDataCount = alarmsChecklistCount && alarmsChecklistCount.length ? alarmsChecklistCount.length : 0;
  ...
  const onOrderIDLoad = (id) => {
    ...
    setOrder(true);
    setRedirect(true); */
    dispatch(getInspectionDetail(companies, appModels.INSPECTIONCHECKLISTLOGS, id));
    setEventModal(true);
  };

  const isUserError = userInfo && userInfo.err;
  const loading = (userInfo && userInfo.loading) || (alarmsCheckListInfo && alarmsCheckListInfo.loading) || (alarmsChecklistCountLoading);
  const userErrorMsg = generateErrorMessage(userInfo);
  const errorMsg = (alarmsCheckListInfo && alarmsCheckListInfo.err) ? generateErrorMessage(alarmsCheckListInfo) : userErrorMsg;
      ...
      <CardBody className="alarms-list p-0 thin-scrollbar">
        <Row className="mb-2">
          <Col md="12" xs="12" sm="12" lg="12">
            {!loading && (alarmsCheckListInfo && alarmsCheckListInfo.data && alarmsCheckListInfo.data[0].model_name !== modelName) && (
              <>
            ...
            <Col md="12" sm="12" xs="12" lg="12">
              <Card className="border-left-dark bg-thinblue overflow-hidden">
                <CardBody className="pt-2 pb-2">
                  {(activeId === actions.id && ((alarmUpdateInfo && alarmUpdateInfo.loading) || (alarmsCheckListInfo && alarmsCheckListInfo.loading))) ? (
                    <Loader />
                  )
                    : (
                      <>
                        ...
                        <Row>
                          ...
                          {(actions.model_name === 'mro.order' || actions.model_name === 'hx.inspection_checklist_log') && actions.record && (
                            <Col md="12" xs="12" sm="12" lg="12" className="ml-2">
                              <span className="font-weight-700 mr-2">
                                #
                                {actions.record_name}
                        ...
                        <Row>
                          <Col md="12" xs="12" sm="12" lg="12">
                            {getActions(actions.state, actions.mro_alarm_id ? actions.mro_alarm_id[0] : actions.id, actions.id)}
                          </Col>
                        </Row>
                      </>
                    )}
        ...
        {((alarmsCheckListInfo && alarmsCheckListInfo.err) || isUserError) && (
...
export default ChecklistAlarms;
