...
import {
  getExtraSelection, getExtraSelectionCount, getExtraSelectionMultiple, getExtraSelectionMultipleCount,
...
const SearchModal = (props) => {
  ...
  const {
    listDataInfo, listDataCountInfo, listDataCountLoading, listDataMultipleInfo, listDataMultipleCountLoading, listDataMultipleCountInfo,
  } = useSelector((state) => state.ticket);

  useEffect(() => {
    if (modelName && fields) {
      if (fieldName === 'copy_company') {
        const searchValueMultiple = `[["name", "ilike", "${searchValue}"]]`;
        const fieldsNew = '["id", "name", ["allowed_module_ids", ["id", "name", "code"]]]';
  ...
  useEffect(() => {
    if (modelName) {
      if (fieldName === 'copy_company') {
        const searchValueMultiple = `[["name", "ilike", "${searchValue}"]]`;
        dispatch(getExtraSelectionMultipleCount(company, modelName, fields, searchValueMultiple, 'search'));
      } else {
  ...
  const totalDataCount1 = listDataMultipleCountInfo && listDataMultipleCountInfo.length ? listDataMultipleCountInfo.length : 0;
  ...
  const handleRowClick = (data) => {
    ...
    if (fieldName === 'copy_company') {
      items = { id: data.id, name: data.name, allowed_module_ids: data.allowed_module_ids };
    }
  ...
  const loading1 = (listDataMultipleInfo && listDataMultipleInfo.loading) || (listDataMultipleCountLoading);
        ...
        <Card className="p-2 bg-lightblue h-100">
          <CardBody className="bg-color-white p-1 m-0">
              ...
              <Col sm="12" md="9" lg="9" xs="12">
                <div className="mt-3">
                  <span className="p-0 font-weight-600 font-medium mr-2">
                    Total :
                    {' '}
                    {fieldName === 'copy_company' ? totalDataCount1 : totalDataCount}
                  </span>
                </div>
              </Col>
            ...
            <div className="pl-1 pr-1 pt-0 comments-list thin-scrollbar">
              {(fieldName !== 'copy_company' && listDataInfo && listDataInfo.data) && (
                <Table responsive>
              ...
              {(fieldName === 'copy_company' && listDataMultipleInfo && listDataMultipleInfo.data) && (
                <Table responsive>
                  <thead className="bg-gray-light">
                    <tr>
                      <th className="p-2 min-width-100">
