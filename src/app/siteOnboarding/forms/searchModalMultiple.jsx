...
const SearchModalMutliple = (props) => {
  ...
  useEffect(() => {
    if (fieldName === 'allowed_module_ids') {
  ...
  useEffect(() => {
    if (modelName && fields) {
      if (fieldName === 'allowed_module_ids') {
        const searchValueMultiple = `[["name", "ilike", "${searchValue}"],["code", "ilike", "${searchValue}"]]`;
  ...
  const handleTableCellChange = (event) => {
    ...
    if (checked) {
      ...
      let items = [];
      if ((fieldName === 'allowed_module_ids')) {
        items = [{ id: values.id, name: values.name }];
      }
  ...
  const checkDefaultValue = (id) => {
    let val = false;
    const dataList = defaultModule.filter((item) => item.id === id);
    val = !(dataList.length > 0);
    return val;
  };
    ...
    for (let i = 0; i < arrayData.length; i += 1) {
      tableTr.push(
        <tr className={selectedId === arrayData[i].id ? 'text-info cursor-pointer' : 'cursor-pointer'} key={i}>
          <td className="w-5">
            {(fieldName === 'allowed_module_ids') && (
              checkDefaultValue(arrayData[i].id) && (
              <Checkbox
                sx={{
                  transform: 'scale(0.9)',
                  padding: '0px',
                ...
                className="ml-0"
                name={fieldName === 'allowed_module_ids' ? arrayData[i].name : arrayData[i].name}
          ...
          {(fieldName === 'allowed_module_ids') && (
            <>
              ...
              <td><span className="font-weight-400">{getDefaultNoValue(arrayData[i].code)}</span></td>
            </>
          )}
        </tr>,
      );
          ...
          <CardBody className="bg-color-white p-1 m-0">
            ...
            {/* <Modal isOpen={modalAlert} toggle={toggleAlert} size="sm">
              ...
              <ModalBody>
                Maximum allowed
                {' '}
                {fieldName === 'allowed_module_ids' ? 'spaces' : 'equipments'}
                {' '}
                : 10
              </ModalBody>
              <ModalFooter>
...
SearchModalMutliple.propTypes = {
  ...
  defaultModule: PropTypes.oneOfType([
    PropTypes.array,
    PropTypes.object,
    PropTypes.string,
  ]).isRequired,
