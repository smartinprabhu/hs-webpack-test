  ...
  function handleSubmit(values) {
    ...
    if (editPageIndex && !editId) {
      if (selectedData && selectedData.length > 0) {
        ...
        postData.sla_state = extractValueObjects(values.sla_state);
        postData.is_requestee = values.is_requestee;
        postData.is_send_email = values.is_send_email;
        postData.mail_template_id = values.mail_template_id && values.mail_template_id.id ? values.mail_template_id : '';
        ...
        // const newArrData = [...new Map(arr.map((item) => [item.sla_state, item])).values()];
        setFieldVal(arr);
      }
    } else if (editId) {
      const postData = {
        sla_state: extractValueObjects(values.sla_state),
