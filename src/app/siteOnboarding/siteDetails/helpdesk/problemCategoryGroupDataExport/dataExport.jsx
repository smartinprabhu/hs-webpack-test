...
const DataExport = (props) => {
  ...
  const appliedFilters = filterStringGeneratorDynamic(problemCategoryGroupFilters);
  ...
  useEffect(() => {
    if ((userInfo && userInfo.data) && (problemCategoryGroupCount && problemCategoryGroupCount.length)) {
      const offsetValue = 0;
      const customFiltersQuery = problemCategoryGroupFilters && problemCategoryGroupFilters.customFilters ? queryGeneratorV1(problemCategoryGroupFilters.customFilters) : '';
      dispatch(getProblemCategoryGroupListExport(companies, appModels.TICKETCATEGORYGROUP, problemCategoryGroupCount.length, offsetValue, apiFields, customFiltersQuery, rows, sortedValue.sortBy, sortedValue.sortField));
    }
  }, [userInfo, problemCategoryGroupCount]);
  ...
  useEffect(() => {
    if (fields && fields.length > 0 && pcgExportListInfo && pcgExportListInfo.data && pcgExportListInfo.data.length > 0) {
  ...
  const getNewVisitArray = (array) => {
    ...
    if (array && array.length > 0) {
      for (let i = 0; i < array.length; i += 1) {
        const val = array[i];
        val.type_category = typeCtegoryLabelFunction(val.type_category);
        val.is_all_category = val.is_all_category ? 'Yes' : 'No';
  ...
  useEffect(() => {
    if (pdfHeader && pdfHeader.__wrapped__ && pdfHeader.__wrapped__.length > 0) {
      ...
      const exportData = pcgExportListInfo && pcgExportListInfo.data && pcgExportListInfo.data.length > 0 ? getNewVisitArray(pcgExportListInfo.data) : [];
  ...
  useEffect(() => {
    if (exportType === 'pdf') {
      ...
      savePdfContent('Problem Category', pdfHeaders, pdfBody, 'HSense-problemcategory.pdf', companyName, appliedFilters);
      dispatch(setInitialValues(false, false, false, false, false));
    } else if (exportType === 'excel') {
      const currentDate = getLocalTime(new Date());
      const title = `ProblemCategoryGroup_on_${currentDate}`;
      exportTableToExcel('print_report_problem_category_group', title);
  ...
  return (
    <Row>
      <Col md="12" sm="12" lg="6">
        <div className="p-3 text-center">
          ...
          <Button
            ...
            color="dark"
            className="bg-zodiac"
            disabled={pcgExportListInfo && pcgExportListInfo.loading}
            onClick={() => {
              setExportType('pdf');
            }}
          >
            {(pcgExportListInfo && pcgExportListInfo.loading) ? (
      ...
      {pcgExportListInfo && pcgExportListInfo.err && (
        <Col md="12" sm="12" lg="12" xs="12">
          <SuccessAndErrorFormat response={pcgExportListInfo} />
        </Col>
      )}
      <Col md="12" sm="12" lg="12" className="d-none">
        <div id="print_report_problem_category_group">
          {exportType === 'excel' && (
            <table align="center">
              <tbody>
                <tr>
          ...
          {exportType === 'excel' && (
            <DataTable
              columns={pcgExportListInfo && pcgExportListInfo.data && pcgExportListInfo.data.length ? checkFieldExists(dataFields) : []}
              data={pcgExportListInfo && pcgExportListInfo.data ? pcgExportListInfo.data : []}
              propertyAsKey="id"
            />
          )}
        </div>
