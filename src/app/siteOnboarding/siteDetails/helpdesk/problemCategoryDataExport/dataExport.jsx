...
const DataExport = (props) => {
  ...
  const appliedFilters = filterStringGeneratorDynamic(problemCategoryFilters);
  ...
  useEffect(() => {
    if ((userInfo && userInfo.data) && (problemCategoryCount && problemCategoryCount.length)) {
      const offsetValue = 0;
      const customFiltersQuery = problemCategoryFilters && problemCategoryFilters.customFilters ? queryGeneratorV1(problemCategoryFilters.customFilters) : '';
      dispatch(getProblemCategoryListExport(companies, appModels.TICKETCATEGORY, problemCategoryCount.length, offsetValue, apiFields, customFiltersQuery, rows, sortedValue.sortBy, sortedValue.sortField));
    }
  }, [userInfo, problemCategoryCount]);
  ...
  useEffect(() => {
    if (pdfHeader && pdfHeader.__wrapped__ && pdfHeader.__wrapped__.length > 0) {
      ...
      const exportData = pcExportListInfo && pcExportListInfo.data && pcExportListInfo.data.length > 0 ? getNewVisitArray(pcExportListInfo.data) : [];
  ...
  useEffect(() => {
    if (exportType === 'pdf') {
      ...
      const title = `ProblemCategory_on_${currentDate}`;
      exportTableToExcel('print_report_pcategory', title);
      ...
      <Col md="12" sm="12" lg="6">
        <div className="p-3 text-center">
          ...
          <Button
            ...
            color="dark"
            className="bg-zodiac"
            disabled={pcExportListInfo && pcExportListInfo.loading}
            onClick={() => {
              setExportType('pdf');
            }}
          >
            {(pcExportListInfo && pcExportListInfo.loading) ? (
