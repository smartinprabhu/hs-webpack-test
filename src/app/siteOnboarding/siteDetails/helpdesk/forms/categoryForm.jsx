...
const ProductForm = (props) => {
  ...
  useEffect(() => {
    if (problem_category_ids && problem_category_ids.length > 0) {
      setPartsData(problem_category_ids);
    }
  }, [problem_category_ids]);
  ...
  useEffect(() => {
    if (editId && (pcInfo && pcInfo.data && pcInfo.data.length && pcInfo.data[0].problem_category_ids && pcInfo.data[0].problem_category_ids.length)
      && (updateProductCategoryInfo && !updateProductCategoryInfo.err)) {
      const newArrData = pcInfo.data[0].problem_category_ids.map((cl) => ({
        ...cl,
        id: cl.id,
        name: cl.name,
      ...
      setFieldValue('problem_category_ids', newArrData);
  ...
  useEffect(() => {
    if (partsData) {
      setFieldValue('problem_category_ids', partsData);
    }
  }, [partsData]);
    ...
    <>
      ...
      <Card className="no-border-radius mb-2 mt-2">
        <CardBody className="p-0 bg-porcelain">
          <p className="ml-2 mb-1 mt-1 font-weight-600 font-side-heading">Problem Category</p>
        </CardBody>
      </Card>
      <Row className="">
