...
const SearchModalMutliple = (props) => {
  ...
  const handleTableCellChange = (event) => {
    ...
    if (checked) {
      ...
      let items = [];
      if ((fieldName === 'recipients_ids' || fieldName === 'category_id' || fieldName === 'location_ids' || fieldName === 'space_category_id' || fieldName === 'cat_user_ids' || fieldName === 'access_group_ids')) {
    ...
    for (let i = 0; i < arrayData.length; i += 1) {
      tableTr.push(
        <tr className={selectedId === arrayData[i].id ? 'text-info cursor-pointer' : 'cursor-pointer'} key={i}>
          <td className="w-5">
            {(fieldName === 'recipients_ids' || fieldName === 'category_id' || fieldName === 'location_ids' || fieldName === 'space_category_id' || fieldName === 'cat_user_ids' || fieldName === 'access_group_ids') && (
                ...
                <Table responsive>
                  <thead className="bg-gray-light">
                    <tr>
                      ...
                      {(fieldName === 'cat_user_ids') && (
                        <th className="p-2 min-width-100">
                          Name
                        </th>
                      )}
                      {fieldName === 'access_group_ids' && (
                        <th className="p-2 min-width-100">
                          Name
                        </th>
                      )}
