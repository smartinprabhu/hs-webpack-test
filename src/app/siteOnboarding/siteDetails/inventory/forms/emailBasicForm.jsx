...
const InventoryEmailBasicForm = React.memo((props) => {
  ...
  const {
    recipients_ids, is_recipients, request_state, code,
  } = formValues;
  ...
  const getType = () => {
    ...
    const existingRequests = selectedData.filter((item) => item.request_state === (request_state && request_state.value ? request_state.value : request_state));
    const typelist = getColumnArrayById(existingRequests, 'code');
  ...
  const getRequest = () => {
    ...
    const existingRequests = selectedData.filter((item) => item.code === (code && code.value ? code.value : code));
    const reqlist = getColumnArrayById(existingRequests, 'request_state');
    const filteredLIst = array.filter((item) => !reqlist.includes(item.value));
    return filteredLIst;
  };

  const request = (!editId || !editPageIndex) ? getColumnArrayById(selectedData, 'request_state') : '';
  const requestOption = (!editId || !editPageIndex) ? getRequest() : customData.requestStatus;
  const type = (!editId || !editPageIndex) ? getColumnArrayById(selectedData, 'code') : '';
        ...
        <Col xs={12} sm={6} lg={6} md={6}>
          <Col xs={12} sm={12} lg={12} md={12}>
            <MuiAutoComplete
              ...
              oldvalue={getRequestLabel(request_state)}
              value={request_state && request_state.label ? request_state.label : getRequestLabel(request_state)}
          ...
          <Col xs={12} sm={12} lg={12} md={12}>
            <MuiAutoComplete
              ...
              oldvalue={getTypeLabel(code)}
              value={code && code.label ? code.label : getTypeLabel(code)}
              getOptionSelected={(option, value) => option.label === value.label}
              getOptionLabel={(option) => (typeof option === 'string' ? option : option.label)}
              options={typeOptions}
              renderInput={(params) => (
          ...
          <Col xs={12} sm={12} md={12} lg={12} className="ml-2px pl-3">
            <MuiCheckboxField
              ...
              name={recipientsAdvance.name}
              label={recipientsAdvance.label}
            />
          </Col>
          {is_recipients === true ? (
