...
import { getPartsData, getChecklistData } from '../../../../preventiveMaintenance/ppmService';
  ...
  const handleRowClick = (data) => {
    ...
    if (arrayValues && arrayValues.length > 0 && arrayIndex >= 0) {
      ...
      if (fields[0] === 'path_name') {
        newData[arrayIndex][fieldName] = [data.id, data.path_name];
      } else {
        newData[arrayIndex][fieldName] = [data.id, data.name];
      }
      if (operationData === 'parts') {
        dispatch(getPartsData(newData));
      } else if (operationData === 'checklist') {
        dispatch(getChecklistData(newData));
      }
    } else {
      let items = {};
      if (fields[0] === 'path_name') {
        items = { id: data.id, [serchFieldValue]: data.path_name };
      } else {
        items = { id: data.id, [serchFieldValue]: data.name };
      }
...
SearchModal.propTypes = {
  ...
  operationData: PropTypes.oneOfType([
    PropTypes.array,
    PropTypes.bool,
    PropTypes.string,
  ]),
