...
const SearchModalMutliple = (props) => {
  ...
  const [checkedRows, setCheckRows] = useState(oldCheckedRows);
  ...
  useEffect(() => {
    if (fieldName === 'recipients_ids') {
      setCheckRows(oldRecipientsData);
    }
    if (fieldName === 'approval_recipients_ids') {
      setCheckRows(oldApprovalData);
    }
  }, []);
  ...
  const handleTableCellChange = (event) => {
    ...
    if (checked) {
      ...
      const arr = [...items, ...oldCheckedRows];
      setCheckedRows(arr);
    } else {
      setCheckedRows(oldCheckedRows.filter((item) => item.id !== values.id));
    ...
    for (let i = 0; i < arrayData.length; i += 1) {
      tableTr.push(
        <tr className={selectedId === arrayData[i].id ? 'text-info cursor-pointer' : 'cursor-pointer'} key={i}>
          <td className="w-5">
            {(fieldName === 'recipients_ids' || fieldName === 'approval_recipients_ids') && (
...
SearchModalMutliple.propTypes = {
  ...
  oldCheckedRows: PropTypes.oneOfType([
    PropTypes.array,
    PropTypes.object,
    PropTypes.string,
  ]).isRequired,
  oldRecipientsData: PropTypes.oneOfType([
    PropTypes.array,
    PropTypes.object,
    PropTypes.string,
  ]).isRequired,
  oldApprovalData: PropTypes.oneOfType([
    PropTypes.array,
    PropTypes.object,
    PropTypes.string,
  ]).isRequired,
...
SearchModalMutliple.defaultProps = {
