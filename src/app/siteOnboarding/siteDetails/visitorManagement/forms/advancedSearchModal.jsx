...
const AdvancedSearchModal = (props) => {
  ...
  useEffect(() => {
    if (modelName && fields) {
      const sortField = (fieldName === 'prescreen_survey' || fieldName === 'feedback_survey') ? 'title' : 'name';
  ...
  const handleRowClick = (data) => {
    ...
    if ((fieldName === 'prescreen_survey' || fieldName === 'feedback_survey')) {
        ...
        <tr className={selectedId === assetData[i].id ? 'text-info cursor-pointer' : 'cursor-pointer'} onClick={() => handleRowClick(assetData[i])} key={i}>
          <td className="p-2">
            <span className="font-weight-400">
              {fieldName === 'parent_id'
                ? getDefaultNoValue(assetData[i].display_name) : (fieldName === 'prescreen_survey' || fieldName === 'feedback_survey')
              ...
              {(listDataInfo && listDataInfo.data) && (
                <Table responsive>
                  <thead className="bg-gray-light">
                    <tr>
                      {(fieldName === 'prescreen_survey' || fieldName === 'feedback_survey')
                        ? (
