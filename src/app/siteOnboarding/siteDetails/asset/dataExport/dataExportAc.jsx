...
const DataExport = (props) => {
  ...
  const appliedFilters = filterStringGeneratorDynamic(assetCategoryFilters);
  ...
  useEffect(() => {
    if ((userInfo && userInfo.data) && (assetCategoryCount && assetCategoryCount.length)) {
      const offsetValue = 0;
      const customFiltersQuery = assetCategoryFilters && assetCategoryFilters.customFilters ? queryGeneratorV1(assetCategoryFilters.customFilters) : '';
      dispatch(getAssetCategoryListExport(companies, appModels.EQUIPMENTCATEGORY, assetCategoryCount.length, offsetValue, apiFields, customFiltersQuery, rows, sortedValue.sortBy, sortedValue.sortField));
    }
  }, [userInfo, assetCategoryCount]);
  ...
  useEffect(() => {
    if (fields && fields.length) {
      let array = [];
      fields.map((field) => {
        const exportField = tableFields.acFields.find((tableField) => tableField.property === field);
  ...
  useEffect(() => {
    if (fields && fields.length > 0 && acExportListInfo && acExportListInfo.data && acExportListInfo.data.length > 0) {
  ...
  useEffect(() => {
    if (pdfHeader && pdfHeader.__wrapped__ && pdfHeader.__wrapped__.length > 0) {
      ...
      const exportData = acExportListInfo && acExportListInfo.data && acExportListInfo.data.length > 0 ? getNewVisitArray(acExportListInfo.data) : [];
  ...
  useEffect(() => {
    if (exportType === 'pdf') {
      ...
      savePdfContent('Asset Category', pdfHeaders, pdfBody, 'HSense-assetcategory.pdf', companyName, appliedFilters);
      dispatch(setInitialValues(false, false, false, false, false));
    } else if (exportType === 'excel') {
      const currentDate = getLocalTime(new Date());
      const title = `AssetCategory_on_${currentDate}`;
      exportTableToExcel('print_report_asset', title);
      if (afterReset) afterReset();
      dispatch(setInitialValues(false, false, false, false, false));
    }
  }, [exportType]);

  const loading = acExportListInfo && acExportListInfo.loading;
  ...
  return (
    <Row>
      ...
      {acExportListInfo && acExportListInfo.err && (
        <Col md="12" sm="12" lg="12" xs="12">
          <SuccessAndErrorFormat response={acExportListInfo} />
        </Col>
      )}
      <Col md="12" sm="12" lg="12" className="d-none">
        <div id="print_report_asset">
          {exportType === 'excel' && (
            <table align="center">
              <tbody>
                <tr>
          ...
          {exportType === 'excel' && (
            <DataTable
              columns={acExportListInfo && acExportListInfo.data && acExportListInfo.data.length ? checkFieldExists(dataFields) : []}
              data={acExportListInfo && acExportListInfo.data ? acExportListInfo.data : []}
              propertyAsKey="id"
            />
          )}
        </div>
