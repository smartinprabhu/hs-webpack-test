...
const DataExport = (props) => {
  ...
  useEffect(() => {
    if ((userInfo && userInfo.data) && (productCategoryCount && productCategoryCount.length)) {
      ...
      dispatch(getProductCategoryListExport(companies, appModels.PRODUCTCATEGORY, productCategoryCount.length, offsetValue, apiFields, customFiltersQuery, rows, sortedValue.sortBy, sortedValue.sortField));
    }
  ...
  useEffect(() => {
    if (exportType === 'pdf') {
      ...
      savePdfContent('Product Category', pdfHeaders, pdfBody, 'HSense-productcategory.pdf', companyName, appliedFilters);
      dispatch(setInitialValues(false, false, false, false, false));
    } else if (exportType === 'excel') {
      const currentDate = getLocalTime(new Date());
      const title = `ProductCategory_on_${currentDate}`;
      exportTableToExcel('print_report_problem', title);
