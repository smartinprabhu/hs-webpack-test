...
const DataExport = (props) => {
  ...
  const appliedFilters = filterStringGeneratorDynamic(spaceCategoryFilters);
  ...
  useEffect(() => {
    if ((userInfo && userInfo.data) && (spaceCategoryCount && spaceCategoryCount.length)) {
      const offsetValue = 0;
      const customFiltersQuery = spaceCategoryFilters && spaceCategoryFilters.customFilters ? queryGeneratorV1(spaceCategoryFilters.customFilters) : '';
      dispatch(getSpaceCategoryListExport(companies, appModels.ASSETCATEGORY, spaceCategoryCount.length, offsetValue, apiFields, customFiltersQuery, rows, sortedValue.sortBy, sortedValue.sortField));
    }
  }, [userInfo, spaceCategoryCount]);
  ...
  useEffect(() => {
    if (fields && fields.length > 0 && scExportListInfo && scExportListInfo.data && scExportListInfo.data.length > 0) {
  ...
  useEffect(() => {
    if (pdfHeader && pdfHeader.__wrapped__ && pdfHeader.__wrapped__.length > 0) {
      ...
      const exportData = scExportListInfo && scExportListInfo.data && scExportListInfo.data.length > 0 ? getNewVisitArray(scExportListInfo.data) : [];
  ...
  useEffect(() => {
    if (exportType === 'pdf') {
      ...
      savePdfContent('Space Category', pdfHeaders, pdfBody, 'HSense-spacecategory.pdf', companyName, appliedFilters);
      dispatch(setInitialValues(false, false, false, false, false));
    } else if (exportType === 'excel') {
      const currentDate = getLocalTime(new Date());
      const title = `SpaceCategory_on_${currentDate}`;
      exportTableToExcel('print_report_space', title);
      if (afterReset) afterReset();
      dispatch(setInitialValues(false, false, false, false, false));
    }
  }, [exportType]);

  const loading = scExportListInfo && scExportListInfo.loading;
  ...
  return (
    <Row>
      ...
      {scExportListInfo && scExportListInfo.err && (
        <Col md="12" sm="12" lg="12" xs="12">
          <SuccessAndErrorFormat response={scExportListInfo} />
        </Col>
      )}
      <Col md="12" sm="12" lg="12" className="d-none">
        <div id="print_report_space">
          {exportType === 'excel' && (
            <table align="center">
              <tbody>
                <tr>
          ...
          {exportType === 'excel' && (
            <DataTable
              columns={scExportListInfo && scExportListInfo.data && scExportListInfo.data.length ? checkFieldExists(dataFields) : []}
              data={scExportListInfo && scExportListInfo.data ? scExportListInfo.data : []}
              propertyAsKey="id"
            />
          )}
        </div>
