...
const AdvancedSearchModal = (props) => {
  ...
  const handleRowClick = (data) => {
    ...
    if (fieldName === 'parent_id' && modelName === 'mro.equipment.category') {
      items = { id: data.id, path_name: data.path_name };
    }
    if (fieldName === 'commodity_id') {
        ...
        <tr className={selectedId === assetData[i].id ? 'text-info cursor-pointer' : 'cursor-pointer'} onClick={() => handleRowClick(assetData[i])} key={i}>
          <td className="p-2">
            <span className="font-weight-400">
              {
                fieldName === 'parent_id' && modelName === 'mro.equipment.category' ? getDefaultNoValue(assetData[i].path_name) : fieldName === 'commodity_id' ? getDefaultNoValue(assetData[i].display_name) : getDefaultNoValue(assetData[i].name)
              }
            </span>
