...
const SearchModalMutliple = (props) => {
  ...
  useEffect(() => {
    ...
    if (fieldName === 'mail_template_id') {
      setCheckRows(oldMailData);
    }
  }, []);
  ...
  useEffect(() => {
    if (modelName && fields) {
      ...
      if (fieldName === 'mail_template_id') {
        const searchValueMultiple = `["&",["model_id","=","dw2.inspection_log"],["name", "ilike", "${searchValue}"]]`;
        dispatch(getExtraSelectionMultiple(company, modelName, limit, offset, fields, searchValueMultiple));
      }
    }
  ...
  const handleTableCellChange = (event) => {
    ...
    if (checked) {
      ...
      let items = [];
      if ((fieldName === 'recipients_ids' || fieldName === 'mail_template_id')) {
    ...
    for (let i = 0; i < arrayData.length; i += 1) {
      tableTr.push(
        <tr className={selectedId === arrayData[i].id ? 'text-info cursor-pointer' : 'cursor-pointer'} key={i}>
          <td className="w-5">
            {(fieldName === 'recipients_ids' || fieldName === 'mail_template_id') && (
...
SearchModalMutliple.propTypes = {
  ...
  oldMailData: PropTypes.oneOfType([
    PropTypes.array,
    PropTypes.object,
    PropTypes.string,
  ]).isRequired,
