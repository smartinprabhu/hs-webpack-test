...
import { checkActionItems } from '../util/appUtils';

const DrawerHeader = (props) => {
  const {
    title, pathName, imagePath, closeDrawer, isEditable, onNext, onPrev, onEdit, isButtonDisabled, loadingText, ExtraHeaderComponent, actionText, actionAllowed,
  } = props;
      ...
      <Col md={12} sm={12} xs={12} lg={12} className="m-0 p-0">
        <h5 className="m-0 p-0">
          ...
          <span className="float-right">
            ...
            {loadingText && (
            <span className="font-tiny mr-3">{loadingText}</span>
            )}
            {(isEditable && checkActionItems(actionAllowed, actionText)) && (
              <Button
                variant="contained"
                size="sm"
                onClick={onEdit}
