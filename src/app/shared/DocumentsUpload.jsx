...
const DocumentsUpload = ({ saveData, model, limit }) => {
  ...
  return (
    <>
        ...
        {uploadPhoto && uploadPhoto.length && uploadPhoto.length > 0 ? (
          <>
            {uploadPhoto.map((fl, index) => (
              <Col sm="2" md="2" lg="2" xs="2" className="position-relative mb-3" key={fl.name}>
      ...
      <Modal isOpen={modalAlert} toggle={toggleAlert} size="sm">
        ...
        <ModalFooter>
          <Button variant="contained" color="primary" onClick={() => { setFinish(true); setDocumentReady(Math.random()); setModalAlert(false); }}>Ok</Button>
          {' '}
