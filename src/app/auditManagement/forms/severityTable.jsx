...
const SeverityTable = (props) => {
  ...
  const loading = (hxSeverities && hxSeverities.loading) || (hxIncidentConfig && hxIncidentConfig.loading);
  ...
  return (
    <div>
            ...
            <tr>
              <td className="p-2 sticky-td">
                ...
                <span className="text-info cursor-pointer">
                  <FontAwesomeIcon onClick={() => showDesc(hs.description, hs.name)} onMouseOver={() => showDesc(hs.description, hs.name)} className="ml-1" size="sm" icon={faInfoCircle} />
                </span>
              </td>
      ...
      <Dialog size="lg" fullWidth open={descModal}>
        <DialogHeader title={modalName} fontAwesomeIcon={faInfoCircle} onClose={() => { setDescModal(false) }} />
        <DialogContent>
          <Markup content={truncateFrontSlashs(truncateStars(desc))} />
        </DialogContent>
      </Dialog>
