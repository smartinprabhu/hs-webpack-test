...
const Questions = (props) => {
  ...
  function handleSubmit(values) {
    if (editPageIndex) {
      if (questionData && questionData.length > 0) {
        ...
        data.has_attachment = values.has_attachment;
        data.helper_text = values.helper_text;
        data.applicable_score = values.applicable_score ? parseFloat(values.applicable_score) : 0.00;
        data.procedure = values.procedure;
        data.risk_level = values.risk_level && values.risk_level.value ? values.risk_level.value : values.risk_level;
        data.question_group_id = values.question_group_id;
        ...
        data.applicable_standard_ids = values.applicable_standard_ids && values.applicable_standard_ids.length > 0 ? [[6, 0, getColumnArrayById(values.applicable_standard_ids, 'id')]] : [[6, 0, []]];
      ...
      const standardIds = values.applicable_standard_ids && values.applicable_standard_ids.length > 0 ? [[6, 0, getColumnArrayById(values.applicable_standard_ids, 'id')]] : [[6, 0, []]];
      ...
      const questionGroupId = values.question_group_id;
      ...
      postData.applicable_standard_ids = standardIds;
  ...
  return (
    <Dialog size="xl" fullWidth open={model}>
      <DialogHeader title={(!editQuestionId || !editPageIndex) ? 'Create a Item' : 'Update a Item'} onClose={() => { toggle(); }} />
      <DialogContent>
