...
import {
  Dialog, DialogContent, DialogContentText, Typography, ListItemText, Input,
} from '@mui/material';
...
import {
  getSurveyQuestions, getSurveyQuestionsGroups, getSurveyOptions, getAuditStandards, getChoiceData, getMatrixData, getChoiceOptions, getMatrixOptions,
...
const AddQuestions = (props) => {
  ...
  const {
    ...
    matrix_subtype, question_group_id, display_mode, applicable_standard_ids, risk_level, column_nb, parent_id, based_on_ids, validation_min_date, validation_max_date,
  } = formValues;
  ...
  useEffect(() => {
    if ((editQuestionId || editPageIndex) && auditStandards && auditStandards.data && auditStandards.data.length > 0) {
      if (applicable_standard_ids && applicable_standard_ids.length > 0) {
        const filteredData = auditStandards.data.filter((item) => applicable_standard_ids.includes(item.id));
        if (filteredData && filteredData.length) {
          setFieldValue('applicable_standard_ids', filteredData);
        }
      }
    }
  }, [editQuestionId, editPageIndex, auditStandards]);
