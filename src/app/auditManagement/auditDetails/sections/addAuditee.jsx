...
const AddAuditee = React.memo(({
  ...
  const { hxAuditors, hxRoleIds, hxAuditUpdate } = useSelector((state) => state.hxAudits);
  ...
  useEffect(() => {
    if (userInfo && userInfo.data) {
      dispatch(getHxAuditNames(companies, appModels.TEAMMEMEBERS, teamMemberKeyword, [], 'member'));
    }
  }, [userInfo, teamMemberKeyword]);

  useEffect(() => {
    if (userInfo && userInfo.data && roleOpen) {
      dispatch(getHxAuditRoles(companies, appModels.HXAUDITROLES, roleKeyword, [], 'auditee'));
    }
  ...
  const showAdvancedModal = () => {
    ...
    setOldValues(formValues.team_members_id && formValues.team_members_id.id ? formValues.team_members_id.id : '');
  };
  ...
  const createNC = () => {
    if (auditId) {
      ...
      postData.team_members_id = formValues.team_members_id ? extractValueObjects(formValues.team_members_id) : false;
      postData.role_id = formValues.role_id ? extractValueObjects(formValues.role_id) : false;
      let postDataValues = {
        auditees_ids: [[lineId ? 1 : 0, lineId || 0, postData]],
      };
      if (deleteId) {
        postDataValues = {
          auditees_ids: [[2, lineId, false]],
        };
      }
  ...
  const isDisable = !((formValues.team_members_id && formValues.team_members_id.id) && (formValues.role_id && formValues.role_id.id));

  const teamMembersOptions = extractOptionsObject(hxAuditors, formValues.team_members_id);
      ...
      <DialogContent>
        <DialogContentText id="alert-dialog-description">
          ...
          >
              ...
              <Col md="12" xs="12" sm="12" lg="12">
                <FormGroup className="mb-1">
                  ...
                  <Autocomplete
                    ...
                    open={auditorOpen}
                    size="small"
                    value={formValues.team_members_id && formValues.team_members_id.name ? formValues.team_members_id.name : ''}
                    onOpen={() => {
                      setAuditorOpen(true);
                      setTeamMemberKeyword('');
                    }}
                    ...
                    renderInput={(params) => (
                      <TextField
                        ...
                        className={((formValues.team_members_id && formValues.team_members_id.id))
                          ...
                          endAdornment: (
                            <>
                              ...
                              <InputAdornment position="end">
                                {((formValues.team_members_id && formValues.team_members_id.id) || (teamMemberKeyword && teamMemberKeyword.length > 0)) && (
                                <IconButton
            ...
            <Row>
              <Col xs={12} sm={12} md={12} lg={12} className="mb-2">
                <p className="font-family-tab text-center">
                  ...
                  {' '}
                  {editData.team_members_id && editData.team_members_id.name ? editData.team_members_id.name : ''}
                </p>
              </Col>
            </Row>
            )}
            ...
            <SuccessAndErrorFormat response={hxAuditUpdate} successMessage={`The Auditee ${lineId ? `${deleteId ? 'deleted' : 'updated'}` : 'added'} successfully.`} />
            )}
            <hr className="mb-0" />
          </Box>
