...
import {
  Dialog, DialogContent, DialogContentText, Typography, Input,
} from '@mui/material';
...
import customData from '../../../survey/data/customData.json';
import SearchModal from '../../configuration/forms/searchModal';

import {
  getSurveyQuestions, getSurveyQuestionsGroups, getAuditStandards, getSurveyOptions, getChoiceData, getMatrixData, getChoiceOptions, getMatrixOptions,
} from '../../../survey/surveyService';
import {
  decimalKeyPress, generateErrorMessage,
  arraySortByString, a11yProps,
  decimalKeyPressDown,
...
const AddQuestions = (props) => {
  const {
    ...
    groupId, setFieldValue, editData,
  } = props;
  ...
  const {
    ...
    matrix_subtype, question_group_id, display_mode, risk_level, column_nb, parent_id, applicable_standard_ids, based_on_ids, validation_min_date, validation_max_date,
  } = formValues;
  ...
  const {
    surveyQuestionInfo, auditStandards, surveyOptionInfo, surveyQtnGroups, choiceSelected, matrixSelected, choiceOptionInfo, matrixOptionInfo,
  } = useSelector((state) => state.survey);
  const { userInfo } = useSelector((state) => state.user);

  const { hxAuditConfig } = useSelector((state) => state.hxAudits);
  ...
  useEffect(() => {
    if (groupId) {
      setFieldValue('question_group_id', groupId);
    }
  }, [groupId]);
  ...
  useEffect(() => {
    if (editData && parent_id.labels_ids) {
  ...
  useEffect(() => {
    (async () => {
      if (userInfo.data) {
        const accessLevel = getCompanyAccessLevel(configData && configData.applicable_standards_access ? configData.applicable_standards_access : '', userInfo);
        await dispatch(getAuditStandards(accessLevel, appModels.AUDITSTANDARD));
      }
    })();
  }, []);
  ...
  useEffect(() => {
    (async () => {
      if (userInfo.data && groupOpen) {
        await dispatch(getSurveyQuestionsGroups(appModels.MROQUESTIONGROUP, groupKeyword));
      }
    })();
  ...
  const onQuestionGroupClear = () => {
    setGroupKeyword(null);
    setGroupOpen(false);
    setFieldValue('question_group_id', '');
  };
  ...
  const onStandarClear = () => {
    setFieldValue('applicable_standard_ids', '');
  };
  ...
  const loadEmptyTd = () => {
    ...
    newData.push({
      value: '', quizz_mark: 0.00, sequence: 0, favicon: '', color: '', emoji: '', is_not_acceptable: false, is_ncr: false, is_remark_required: false,
    });
  ...
  const onScoreChange = (e, index) => {
    const newData = choiceData;
    newData[index].quizz_mark = e.target.value;
    setChoiceData(newData);
    setChoiceAdd(Math.random());
  };
  ...
  if (surveyQtnGroups && surveyQtnGroups.loading) {
    groupOptions = [{ name: 'Loading..' }];
  }
  if (surveyQtnGroups && surveyQtnGroups.data) {
    groupOptions = surveyQtnGroups.data;
  }
  if (question_group_id && question_group_id.length && question_group_id.length > 0) {
    const oldId = [{ id: question_group_id[0], name: question_group_id[1] }];
    const newArr = [...groupOptions, ...oldId];
    groupOptions = [...new Map(newArr.map((item) => [item.id, item])).values()];
  }
  if (surveyQtnGroups && surveyQtnGroups.err) {
    groupOptions = [];
  }
  ...
  if (auditStandards && auditStandards.loading) {
    standardOptions = [{ name: 'Loading..' }];
  }
  if (auditStandards && auditStandards.data) {
    standardOptions = auditStandards.data;
  }
  if (auditStandards && auditStandards.err) {
    standardOptions = [];
  }
  ...
  return (
    <>
            ...
            {!groupId ? (
              <Col xs={12} sm={7} md={7} lg={7}>
                <MuiAutoComplete
                  ...
                  name={questionGroupId.name}
                  label={questionGroupId.label}
                  open={groupOpen}
                  size="small"
                  oldValue={question_group_id}
                  value={question_group_id && question_group_id.name ? question_group_id.name : getOldData(question_group_id)}
                  onOpen={() => {
                    setGroupOpen(true);
                    setGroupKeyword('');
                  }}
                  ...
                  onChange={(e, data) => setFieldValue('question_group_id', data)}
                  renderInput={(params) => (
                    <TextField
                      {...params}
                      variant="standard"
                      ...
                      InputProps={{
                        ...
                        endAdornment: (
                          <>
                            {surveyQtnGroups && surveyQtnGroups.loading ? <CircularProgress color="inherit" size={20} /> : null}
                            <InputAdornment position="end">
                              {((getOldData(question_group_id)) || (question_group_id && question_group_id.id) || (groupKeyword && groupKeyword.length > 0)) && (
                              <IconButton onClick={onQuestionGroupClear}>
                                <IoCloseOutline size={22} fontSize="small" />
                              </IconButton>
                              )}
                              <IconButton onClick={onQuestionGRoupSearch}>
          ...
          <Row>
            <Col xs={12} sm={6} md={6} lg={6}>
              <MuiTextField
                ...
                name={applicableScore.name}
                label={applicableScore.label}
                setFieldValue={setFieldValue}
            ...
            <Col xs={12} sm={6} md={6} lg={6}>
              <MuiAutoComplete
                ...
                size="small"
                oldValue={risk_level}
                value={risk_level && risk_level.label ? risk_level.label : risk_level}
                onOpen={() => {
                  setRiskOpen(true);
                }}
                onClose={() => {
          ...
          <Row>
            <Col xs={12} sm={12} md={6} lg={6}>
              <MuiAutoComplete
                ...
                limitTags={3}
                name={applicableStandardIds.name}
                label={applicableStandardIds.label}
                value={applicable_standard_ids && applicable_standard_ids.length > 0 ? applicable_standard_ids : []}
                loading={(auditStandards && auditStandards.loading)}
                options={standardOptions}
                getOptionLabel={(option) => (typeof option === 'string' ? option : option.name)}
                onChange={(e, data) => { setFieldValue('applicable_standard_ids', data); }}
                renderInput={(params) => (
                  <TextField
                    {...params}
                    variant="standard"
                    ...
                    InputProps={{
                      ...
                      endAdornment: (
                        <>
                          {auditStandards && auditStandards.loading ? <CircularProgress color="inherit" size={20} /> : null}
                          <InputAdornment position="end">
                            {((applicable_standard_ids && applicable_standard_ids.length > 0)) && (
                            <IconButton onClick={onStandarClear}>
                              <IoCloseOutline size={22} fontSize="small" />
                            </IconButton>
                            )}
                          </InputAdornment>
                  ...
                  <Col xs={12} sm={12} md={12} lg={12}>
                    <>
                      ...
                      <div aria-hidden="true" className="font-weight-800 d-inline text-lightblue cursor-pointer mt-1 mb-1" onClick={loadEmptyTd}>
                        ...
                        <span className="mr-2 font-family-tab">Add an Choice</span>
                      </div>
                    </>
                  </Col>
                </Row>
                  ...
                  <Col xs={12} sm={12} md={12} lg={12}>
                    <Table responsive className="m-0">
                      <thead className="bg-lightblue">
                        <tr>
                          <th className="p-2 min-width-200 border-0 font-family-tab">Choices</th>
                          <th className="p-2 min-width-160 border-0 font-family-tab">Fav Icon</th>
                          <th className="p-2 min-width-160 border-0 font-family-tab">Color</th>
                          <th className="p-2 min-width-160 border-0 font-family-tab">Emoji</th>
                          <th className="p-2 min-width-160 border-0 font-family-tab">Not Acceptable ?</th>
                          <th className="p-2 min-width-160 border-0 font-family-tab">Is NCR ?</th>
                          <th className="p-2 min-width-160 border-0 font-family-tab">Is Remarks Required ?</th>
                          <th className="p-2 min-width-160 border-0 font-family-tab">Score</th>
                          <th className="p-2 min-width-100 border-0 font-family-tab">
                              ...
                              <tr key={index} className="font-weight-400">
                                ...
                                <td className="p-2">
                                  <InputGroup size="sm">
                                    ...
                                    <Input type="text" value={pl.color ? pl.color : ''} disabled />
                                  </InputGroup>
                                </td>
                                <td className="p-2">
                                  <InputGroup size="sm">
                                    ...
                                    <Input type="text" value={pl.emoji ? pl.emoji : ''} disabled />
                                  </InputGroup>
                                </td>
                                <td className="pl-2 pr-2" style={{ verticalAlign: 'middle' }}>
                                  <Checkbox
                                    checked={pl.is_not_acceptable}
                                    onChange={(e) => handleSpocChange(e, index, 'is_not_acceptable')}
                                    size="small"
                                    className="p-0"
                                ...
                                <td className="pl-2 pr-2" style={{ verticalAlign: 'middle' }}>
                                  <Checkbox
                                    checked={pl.is_ncr}
                                    onChange={(e) => handleSpocChange(e, index, 'is_ncr')}
                                    size="small"
                                    className="p-0"
                                    inputProps={{ 'aria-label': 'controlled' }}
                                  />
                                ...
                                <td className="pl-2 pr-2" style={{ verticalAlign: 'middle' }}>
                                  <Checkbox
                                    checked={pl.is_remark_required}
                                    onChange={(e) => handleSpocChange(e, index, 'is_remark_required')}
                                    size="small"
                                    className="p-0"
                                    inputProps={{ 'aria-label': 'controlled' }}
                                  />
                                ...
                                <td className="p-2">
                                  <Input type="text" autoComplete="off" maxLength="5" name="value" value={pl.quizz_mark} onKeyDown={decimalKeyPressDown} onChange={(e) => onScoreChange(e, index)} />
                                </td>
                                <td className="p-2">
                  ...
                  <Col xs={12} sm={12} md={12} lg={12}>
                    <>
                      ...
                      <div aria-hidden="true" className="font-weight-800 d-inline text-lightblue cursor-pointer mt-1 mb-1" onClick={loadEmptyRowTd}>
                        <img src={addIcon} className="mr-2 mb-1" alt="addequipment" height="15" width="15" />
                        <span className="mr-2">Add a Matrix</span>
                      </div>
                    </>
                  </Col>
                </Row>
          ...
          <Row className="pr-3 pt-3">
            ...
            {constMandatory
              ? (
                <Col xs={12} sm={6} md={6} lg={6} className="ml-3">
                  <MuiTextField setFieldValue={setFieldValue} name={errorMessage.name} label={errorMessage.label} inputProps={{ maxLength: 100 }} type="text" />
                </Col>
              ) : ''}
            {' '}
            ...
            {validationRequired && qtype.toString() === 'textbox'
              ? (
                <Row>
                  <Col xs={12} sm={6} md={6} lg={6}>
                    <MuiTextField setFieldValue={setFieldValue} label={expectedMin.label} name={expectedMin.name} type="text" />
                  </Col>
                  <Col xs={12} sm={6} md={6} lg={6}>
                    <MuiTextField setFieldValue={setFieldValue} label={expectedMax.label} name={expectedMax.name} type="text" />
                  </Col>
                  <Col xs={12} sm={12} md={12} lg={12}>
                    <MuiTextField setFieldValue={setFieldValue} label={validationErrorMsg.label} name={validationErrorMsg.name} type="text" />
                  </Col>
                </Row>
              ) : ''}
      ...
      <Dialog size="lg" fullWidth open={extraModal}>
        <DialogHeader title="Sections" imagePath={false} onClose={() => { setExtraModal(false); }} />
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            <SingleSearchModal
              modelName={appModels.MROQUESTIONGROUP}
              afterReset={() => { setExtraModal(false); }}
              fieldName="question_group_id"
              fields={['id', 'name']}
              company={false}
