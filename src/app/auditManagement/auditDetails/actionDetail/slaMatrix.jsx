...
const SlaMatrix = () => {
  const { hxAuditActionDetail } = useSelector((state) => state.hxAudits);
  const { userInfo } = useSelector((state) => state.user);

  const loading = hxAuditActionDetail && hxAuditActionDetail.loading;
  const isErr = hxAuditActionDetail && hxAuditActionDetail.err;
  const inspDeata = hxAuditActionDetail && hxAuditActionDetail.data && hxAuditActionDetail.data.length ? hxAuditActionDetail.data[0] : false;
        ...
        {isChecklist && (
          <div>
            <Table responsive className="mb-0 mt-2 font-weight-400 border-0 assets-table" width="100%">
              ...
              <tbody>
                {getRow(hxAuditActionDetail.data.length > 0 ? hxAuditActionDetail.data[0].escalation_matrix_ids : [])}
              </tbody>
            </Table>
            <hr className="m-0" />
          </div>
        )}
        <DetailViewFormat detailResponse={hxAuditActionDetail} />
