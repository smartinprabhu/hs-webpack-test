import {
  getEmployeesInfo, getEmployeesCount, getCategoryGroupsInfo,
  getEmployeeDetails, getNeighbourhoodsInfo, updateEmployeeDataInfo,
  loadNeighbourhoodsInfo,
} from './actions';

export const EMPLOYEE_FILTERS = 'EMPLOYEE_FILTERS';
export const RESET_EMPLOYEE_UPDATE = 'RESET_EMPLOYEE_UPDATE';

export function getEmployeeList(company, model, limit, offset, states, customFilters, sortByValue, sortFieldValue) {
  return (dispatch) => {
    dispatch(getEmployeesInfo(company, model, limit, offset, states, customFilters, sortByValue, sortFieldValue));
  };
}
