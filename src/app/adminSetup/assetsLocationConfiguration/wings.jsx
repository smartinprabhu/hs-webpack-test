...
import { WingColumns } from '../../commonComponents/gridColumnsEditable';
...
const AdminSites = () => {
  ...
  useEffect(() => {
    if (
      ...
      const customFiltersQuery = equipmentFilters && equipmentFilters.customFilters
        ...
        : '';
      dispatch(
        getLocationExport(companies, appModels.LOCATION, false, 'Wing', sortedValue.sortBy, sortedValue.sortField, limit, offsetValue, rowselected, customFiltersQuery, globalFilter),
      );
    }
  }, [startExport]);
  ...
  useEffect(() => {
    if ((createBulkInfo && createBulkInfo.data) || (updateBulkInfo && updateBulkInfo.data) || (bulkUploadInfo && bulkUploadInfo.data) || (createSpaceInfo && createSpaceInfo.data)) {
      ...
      dispatch(getLocationCount(companies, appModels.LOCATION, 'Wing', customFiltersList, globalFilter));
      dispatch(getLocationData(companies, appModels.LOCATION, false, 'Wing', sortedValue.sortBy, sortedValue.sortField, limit, offset, customFiltersList, globalFilter));
    }
  ...
  const floorSelection = (paramsEdit) => (
    <EditSelection
      ...
      advanceSearchHeader="Building/Floor List"
      advanceSearchFieldName="parent_id"
      isRenderOption
    />
  );

  const tableColumns = WingColumns(editModeRow, floorSelection, setErrorMessage);
    ...
    <Box className="insights-box">
      <CommonGrid
        ...
        bulkOption={{
          enable: true,
          text: 'Bulk Upload',
          title: 'Wing Bulk Upload',
          template: locationTemplate,
          modalTitle: 'Wing Bulk Upload',
          modalMsg: 'Wings are uploaded successfully...',
      ...
      >
        <DrawerHeader
          headerName="Create Wing"
          subTitle="Create Wings for the Building/Floor"
          imagePath={false}
