...
import locationTemplate from '@images/templates/space_template.xlsx';
import {
  Badge,
} from 'reactstrap';
...
import { SpaceColumns } from '../../commonComponents/gridColumnsEditable';
import { resetAssetCategory } from '../../preventiveMaintenance/ppmService';
import {
  getAllCompanies, debounce,
  formatFilterData,
...
const AdminSites = () => {
  ...
  useEffect(() => {
    if (
      ...
      const customFiltersQuery = equipmentFilters && equipmentFilters.customFilters
        ...
        : '';
      dispatch(
        getLocationExport(companies, appModels.LOCATION, false, 'Space', sortedValue.sortBy, sortedValue.sortField, limit, offsetValue, rowselected, customFiltersQuery, globalFilter),
      );
    }
  }, [startExport]);
  ...
  useEffect(() => {
    if ((createBulkInfo && createBulkInfo.data) || (updateBulkInfo && updateBulkInfo.data) || (bulkUploadInfo && bulkUploadInfo.data) || (createSpaceInfo && createSpaceInfo.data)) {
      ...
      dispatch(getLocationCount(companies, appModels.LOCATION, 'Space', customFiltersList, globalFilter));
      dispatch(getLocationData(companies, appModels.LOCATION, false, 'Space', sortedValue.sortBy, sortedValue.sortField, limit, offset, customFiltersList, globalFilter));
    }
  ...
  const tableColumns = SpaceColumns(editModeRow, floorSelection, setErrorMessage);
  ...
  return (
    <Box className="insights-box">
      ...
      >
        <DrawerHeader
          headerName="Create Space"
          subTitle="Create Spaces for the Building/Floor/Wing"
          imagePath={false}
