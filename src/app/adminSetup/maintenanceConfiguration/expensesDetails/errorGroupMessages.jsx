                      ...
                      <Card className="border-0">
                        <CardHeader id={`heading${getQueryInKey(mg)}`} className="p-2 bg-lightgrey border-0">
                          <Button
                            ...
                            onClick={() => setActive(getQueryInKey(mg))}
                            aria-expanded={currentTab}
                            aria-controls={`collapse${getQueryInKey(mg)}`}
                          >
                            <span className="collapse-heading font-weight-800 text-capital">
                              {getQueryInKey(mg)}
                              {'     '}
                              (
                        ...
                        >
                          <div className="p-2 form-modal-scroll thin-scrollbar">
                            {mg && getMessages(mg, getQueryInKey(mg)).map((ms) => (
                              <p className={ms.type === 'error' ? 'text-danger' : 'text-info'} key={ms.field}>
                                <FontAwesomeIcon
                                  color={ms.type === 'error' ? 'danger' : 'primary'}
                                  className="mr-2"
                                  icon={faInfoCircle}
                                />
                                <span className={ms.type === 'error' ? 'text-danger' : 'text-info'}>
                                  {ms.message}
                                  {'   '}
