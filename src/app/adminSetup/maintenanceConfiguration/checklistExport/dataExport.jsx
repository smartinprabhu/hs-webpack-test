...
const DataExport = (props) => {
  ...
  const exportFileName = getExportFileName('Checklists')
  const [columns, setColumns] = useState(fields);
  const [dataFields, setDataFields] = useState([]);

  useEffect(() => {
    if (fields && fields.length) {
      exportingFieldsArray(tableFields.fields, fields, dataFields, setDataFields);
    }
  }, []);

  const { userInfo } = useSelector((state) => state.user);
  const {
    checklistCount, checklistExportData, checkListFilters,
  ...
  useEffect(() => {
    if ((userInfo && userInfo.data) && (checklistCount)) {
      const offsetValue = 0;
      const customFiltersData = checkListFilters.customFilters ? queryGenerator(checkListFilters.customFilters) : '';
      dispatch(getChecklistsExport(companies, appModels.PPMCHECKLIST, getTotalCount(checklistCount), offsetValue, WorkPermitModule.checkListApiFields, customFiltersData, rows, sortedValue.sortBy, sortedValue.sortField));
    }
  }, [userInfo, checklistCount]);
  ...
  const appliedFilters = filterStringGeneratorAdminSetup(checkListFilters);

  useEffect(() => {
    if (fields && fields.length > 0 && checklistExportData && checklistExportData.data && checklistExportData.data.length > 0) {
  ...
  useEffect(() => {
    if (pdfHeader && pdfHeader.__wrapped__ && pdfHeader.__wrapped__.length > 0) {
      ...
      const exportData = checklistExportData && checklistExportData.data && checklistExportData.data.length > 0 ? getNewArray(checklistExportData.data) : [];
      if (exportData) {
        exportData.map((data) => {
  ...
  useEffect(() => {
    if (exportType === 'pdf') {
      ...
      savePdfContent('Checklist', pdfHeaders, pdfBody, `${exportFileName}.pdf`, companyName, appliedFilters);
      dispatch(setInitialValues(false, false, false, false));
    } else if (exportType === 'excel') {
      exportTableToExcel('print_report_main_checklist', exportFileName);
      if (afterReset) afterReset();
      dispatch(setInitialValues(false, false, false, false));
    }
  }, [exportType]);

  const loading = checklistExportData && checklistExportData.loading;
  ...
  return (
    <Row>
      ...
      {checklistExportData && checklistExportData.err && (
        <Col md="12" sm="12" lg="12" xs="12">
          <SuccessAndErrorFormat response={checklistExportData} />
        </Col>
      )}
      <Col md="12" sm="12" lg="12" className="d-none">
        <div id="print_report_main_checklist">
          {exportType === 'excel' && (
          <table align="center">
            <tbody>
            <tr><td style={{ textAlign: 'center' }} colSpan={5}><b>Checklist Report</b></td></tr>
              <tr>
                <td>Company</td>
          ...
          <br />
          {exportType === 'excel' && (
            <DataTable columns={checkFieldExists(dataFields)} data={checklistExportData && checklistExportData.data ? checklistExportData.data : []} propertyAsKey="id" />
          )}
        </div>
      </Col>
