...
const SearchModalSingle = (props) => {
  ...
  useEffect(() => {
    if (modelName && fields) {
      ...
      if (searchValue && fieldName === 'parent_id') {
  ...
  const handleRowClick = (data) => {
    setSelected(data.id);
    const items = { id: data.id, name: data.group_id && data.group_id.length ? data.group_id[1] : '' };
    ...
    for (let i = 0; i < assetData.length; i += 1) {
      tableTr.push(
        <tr className={selectedId === assetData[i].id ? 'text-info cursor-pointer' : 'cursor-pointer'} onClick={() => handleRowClick(assetData[i])} key={i}>
          {fieldName === 'parent_id'
            ? <td className="p-2"><span className="font-weight-400">{getDefaultNoValue(assetData[i].group_id && assetData[i].group_id.length > 0 ? assetData[i].group_id[1] : '')}</span></td> : ''}
