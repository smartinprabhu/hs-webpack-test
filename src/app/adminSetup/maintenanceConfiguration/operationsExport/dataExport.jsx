...
const DataExport = (props) => {
  ...
  const exportFileName = getExportFileName('PPM_Operations')
  ...
  useEffect(() => {
    if (pdfHeader && pdfHeader.__wrapped__ && pdfHeader.__wrapped__.length > 0) {
      ...
      const exportData = operationExportData && operationExportData.data && operationExportData.data.length > 0 ? getNewArray(operationExportData.data) : [];
      if (exportData) {
        exportData.map((data) => {
  ...
  useEffect(() => {
    if (exportType === 'pdf') {
      ...
      savePdfContent('PPM Operation', pdfHeaders, pdfBody, `${exportFileName}.pdf`, companyName, appliedFilters);
      ...
      <Col md="12" sm="12" lg="12" className="d-none">
        <div id="print_report_operation">
          {exportType === 'excel' && (
            <table align="center">
              <tbody>
                <tr><td style={{ textAlign: 'center' }} colSpan={5}><b>PPM Operation Report</b></td></tr>
                <tr>
                  <td>Company</td>
