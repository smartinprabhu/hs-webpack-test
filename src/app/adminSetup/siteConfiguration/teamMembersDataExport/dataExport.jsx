...
const DataExport = (props) => {
  ...
  const exportFileName =  getExportFileName('Team_Members')
  const [columns, setColumns] = useState(fields);

  const { userInfo } = useSelector((state) => state.user);
  const {
    teamMemberCount, teamMembersExportInfo,
  ...
  useEffect(() => {
    if ((userInfo && userInfo.data) && (teamMemberCount)) {
      const offsetValue = 0;
      dispatch(getTeamMembersExport(companies, appModels.TEAMMEMEBERS, getTotalCount(teamMemberCount), offsetValue, columns, searchValue, rows));
    }
  }, [userInfo, teamMemberCount]);

  function checkFieldExists(dataColumns) {
    let dataColumnsList = dataColumns;
    if (columns.some((selectedValue) => selectedValue.includes('vendor_id'))) {
      dataColumnsList = dataColumnsList.concat(tableFields.entityFields);
    }
    if (columns.some((selectedValue) => selectedValue.includes('employee_id_seq'))) {
      dataColumnsList = dataColumnsList.concat(tableFields.employeeFields);
    }
    if (columns.some((selectedValue) => selectedValue.includes('hr_department'))) {
      dataColumnsList = dataColumnsList.concat(tableFields.departmentFields);
    }
    if (columns.some((selectedValue) => selectedValue.includes('resource_calendar_id'))) {
      dataColumnsList = dataColumnsList.concat(tableFields.shiftFields);
    }
  ...
  useEffect(() => {
    if (fields && fields.length > 0 && teamMembersExportInfo && teamMembersExportInfo.data && teamMembersExportInfo.data.length > 0) {
  ...
  useEffect(() => {
    if (pdfHeader && pdfHeader.__wrapped__ && pdfHeader.__wrapped__.length > 0) {
      ...
      if (teamMembersExportInfo && teamMembersExportInfo.data && teamMembersExportInfo.data.length > 0) {
        teamMembersExportInfo.data.map((data) => {
  ...
  useEffect(() => {
    if (exportType === 'pdf') {
      ...
      savePdfContent('Maintenance Team Members', pdfHeaders, pdfBody, `${exportFileName}.pdf`, companyName);
  ...
  return (
    <Row>
      <Col md="12" sm="12" lg="6">
        <div className="p-3 text-center">
          ...
          <Button
            ...
            color="dark"
            className="bg-zodiac"
            disabled={teamMembersExportInfo && teamMembersExportInfo.loading}
            onClick={() => {
              setExportType('pdf');
            }}
          >
            {(teamMembersExportInfo && teamMembersExportInfo.loading) ? (
      ...
      {teamMembersExportInfo && teamMembersExportInfo.err && (
        <Col md="12" sm="12" lg="12" xs="12">
          <SuccessAndErrorFormat response={teamMembersExportInfo} />
        </Col>
      )}
      <Col md="12" sm="12" lg="12" className="d-none">
        <div id="print_report">
          ...
          <table align="center">
            <tbody>
              <tr><td style={{ textAlign: 'center' }} colSpan={5}><b>Maintenance Team Members Report</b></td></tr>
              <tr>
                <td>Company</td>
          ...
          <br />
          {exportType === 'excel' && (
            <DataTable columns={checkFieldExists(dataFields)} data={teamMembersExportInfo && teamMembersExportInfo.data ? teamMembersExportInfo.data : []} propertyAsKey="id" />
          )}
        </div>
      </Col>
