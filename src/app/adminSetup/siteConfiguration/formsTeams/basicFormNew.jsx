...
const BasicForm = (props) => {
  ...
  const {
    maintenanceCostInfo, teamCategoryInfo, workingTimeInfo, allowedCompanies,
  ...
  useEffect(() => {
    if (spaceName && spaceName.data && spaceName.data.length > 0) {
      ...
      <Row>
        ...
        {/* <Col xs={12} sm={6} md={6} lg={6}>
          <MuiAutoComplete
            ...
            name={formField.companyId.name}
            label={formField.companyId.label}
            className="bg-white"
            open={companyOpen}
            size="small"
            oldValue={values.company_id && values.company_id.length && values.company_id.length > 0 ? values.company_id[1] : ''}
            value={values.company_id && values.company_id.name ? values.company_id.name : values.company_id && values.company_id.length && values.company_id.length > 0 ? values.company_id[1] : ''}
            onOpen={() => {
              setCompanyOpen(true);
            }}
            onClose={() => {
