...
const LogNotes = () => {
  ...
  const loading = tankerTransactionDetail && tankerTransactionDetail.loading;
  const isErr = tankerTransactionDetail && tankerTransactionDetail.err;
  const inspDeata = tankerTransactionDetail && tankerTransactionDetail.data && tankerTransactionDetail.data.length ? tankerTransactionDetail.data[0] : false;
        ...
        {isChecklist && (
          <div>
            <Table responsive className="mb-0 mt-0 font-weight-400 border-0 assets-table" width="100%">
              ...
              <tbody>
                {getRow(tankerTransactionDetail.data.length > 0 ? tankerTransactionDetail.data[0].status_logs : [])}
              </tbody>
            </Table>
            <hr className="m-0" />
          </div>
        )}
        <DetailViewFormat detailResponse={tankerTransactionDetail} />
