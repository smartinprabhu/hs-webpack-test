...
const SearchModalSingle = (props) => {
  ...
  const [checkedRows, setCheckRows] = useState(fieldName === 'commodity' ? oldCommodityValues : oldVendorValues);
  ...
  useEffect(() => {
    if (fieldName === 'commodity') {
      setCheckRows(oldCommodityValues);
    }
    if (fieldName === 'vendor_id') {
      setCheckRows(oldVendorValues);
    }
  }, []);
  ...
  useEffect(() => {
    if (modelName && fields) {
      ...
      if (fieldName === 'vendor_id') {
        searchValueMultiple = `["&",["company_id","in",[${company}]],["supplier", "=", true],["parent_id", "=", false]`;
      }

      if (searchValue && fieldName === 'commodity') {
        searchValueMultiple = `${searchValueMultiple},["name","ilike","${searchValue}"]`;
      }
      if (searchValue && fieldName === 'vendor_id') {
  ...
  const handleAdd = () => {
    if (checkedRows) {
      if (fieldName === 'commodity') {
        onCommodityChange(checkedRows);
      }
      if (fieldName === 'vendor_id') {
        onVendorChange(checkedRows);
      }
    }
...
SearchModalSingle.propTypes = {
  ...
  oldCommodityValues: PropTypes.array.isRequired,
  // eslint-disable-next-line react/forbid-prop-types
  oldVendorValues: PropTypes.array.isRequired,
