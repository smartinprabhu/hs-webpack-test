...
import pantryOrderBlue from '@images/icons/mailroomBlue.svg';
...
import {
  geTankerDashboard, getTankerFilters,
} from '../tankerService';
import {
...
const Insights = () => {
  ...
  const dashboardName = 'Tanker Transactions';

  const { userInfo } = useSelector((state) => state.user);
  const { tankerDashboard } = useSelector((state) => state.tanker);
  ...
  useEffect(() => {
    if (userInfo && userInfo.data && currentTab && currentTab !== 'Custom') {
      ...
      if (dates.length > 0) {
        const start = dates[0];
        const end = dates[1];
        dispatch(geTankerDashboard(start, end, dashboardName));
      } else {
  ...
  const total = getTotal(tankerDashboard && tankerDashboard.data ? tankerDashboard.data : []);

  const gridData = generateArrayFromValue(tankerDashboard && tankerDashboard.data ? tankerDashboard.data : [], 'ks_dashboard_item_type', 'ks_tile');
  const gridCards = gridData.sort((a, b) => a.sequence - b.sequence);

  const isUserError = userInfo && userInfo.err;
  const isUserLoading = userInfo && userInfo.loading;
  const userErrorMsg = generateErrorMessage(userInfo);
  const errorMsg = (tankerDashboard && tankerDashboard.err) ? generateErrorMessage(tankerDashboard) : userErrorMsg;

  if (values.length > 0) {
    return (<Redirect to="/commodity/operations" />);
  }
  ...
  return (
    <>
      ...
      <Row className="m-0 pt-1 pb-3 pr-3 pl-3 tankers-card row">
              ...
              <Tooltip title={`${actions.name}(${actions.datasets && actions.datasets[0] ? actions.datasets && actions.datasets[0] : 0})`}>
                <CardBody id="Tooltip-Insights" className="pb-1 pl-5 pr-5 pt-0">
                  <CircularProgressbarWithChildren
                    value={actions.datasets && actions.code && actions.code !== 'AT' ? newpercalculate(total, actions.datasets) : '100'}
                    strokeWidth={9}
                    styles={buildStyles({
                      textColor: '#3a4354',
                  ...
                  >
                    ...
                    {actions.datasets && actions.code && actions.code !== 'AT'
                      ? (
              ...
              <CardFooter className="bg-med-blue border-0 pt-0">
                {actions.datasets && actions.datasets.length && actions.datasets.length > 0 && actions.datasets[0] !== 0 && actions.code ? (
                  <Button
                     variant="contained"
                    size="sm"
                    onClick={() => onLoadTankers(customData.dashboardStatus[actions.code] ? customData.dashboardStatus[actions.code] : false)}
      ...
      {((tankerDashboard && tankerDashboard.loading) || (isUserLoading)) && (
        <div className="mb-4 mt-2">
          <Loader />
        </div>
      )}
      {((tankerDashboard && tankerDashboard.err) || (isUserError)) && (
