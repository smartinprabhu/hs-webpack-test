...
const AdvancedSearchModal = (props) => {
  ...
  useEffect(() => {
    if (modelName && fields) {
      ...
      dispatch(getExtraSelection(company, modelName, limit, offset, fields, searchValue, otherFieldName, otherFieldValue, sortField, fieldName === 'service_category_id' || fieldName === 'category_id' ? company : false));
    }
  }, [modelName, offset, searchValue]);

  useEffect(() => {
    if (modelName) {
      dispatch(getExtraSelectionCount(company, modelName, searchValue, otherFieldName, otherFieldValue, fieldName === 'service_category_id' || fieldName === 'category_id' ? company : false));
    }
  ...
  const handleRowClick = (data) => {
    ...
    if (fieldName === 'space_id') {
      ...
      items = {
        id: data.id, name: data.name, amc_start_date: data.amc_start_date, amc_end_date: data.amc_end_date,
      };
    } else if (fieldName === 'category_id') {
      items = { id: data.id, name: data.name, sub_category_ids: data.sub_category_ids };
    } else {
        ...
        <tr className={selectedId === assetData[i].id ? 'text-info cursor-pointer' : 'cursor-pointer'} onClick={() => handleRowClick(assetData[i])} key={i}>
          <td className="p-2">
            <span className="font-weight-400">
              {fieldName === 'space_id'
                ? getDefaultNoValue(assetData[i].path_name) : getDefaultNoValue(assetData[i].name)}
            </span>
          </td>
          {fieldName === 'equipment_id'
            ? (
              <td className="p-2">
                <span className="font-weight-400">
                  {getDefaultNoValue(assetData[i].location_id ? assetData[i].location_id[1] : '')}
                </span>
              </td>
            )
            : ''}
