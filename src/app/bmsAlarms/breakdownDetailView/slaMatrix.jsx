    ...
    for (let i = 0; i < assetData.length; i += 1) {
      tableTr.push(
        <tr key={i}>
          ...
          <td className="p-2">
            {' '}
            {getDefaultNoValue(getCompanyTimezoneDate(assetData[i].level_date, userInfo, 'datetime'))}
          </td>
        ...
        {isChecklist && (
          <div>
            <Table responsive className="mb-0 mt-2 font-weight-400 border-0 assets-table" width="100%">
              ...
              <tbody>
                {getRow(trackerDetails.data.length > 0 ? trackerDetails.data[0].escalation_matrix_ids : [])}
              </tbody>
            </Table>
            <hr className="m-0" />
          </div>
