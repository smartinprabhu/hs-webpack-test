...
const DataExport = (props) => {
  ...
  const exportFileName = getExportFileName('Consumption Repot')
  const { userInfo } = useSelector((state) => state.user);
  const {
    typeId,
  } = useSelector((state) => state.ppm);

  const dataFields = tableFields.ConsumptionReportdHeadings;
  ...
  useEffect(() => {
    if (pdfHeader && pdfHeader.__wrapped__ && pdfHeader.__wrapped__.length > 0) {
      ...
      if (assetsList && assetsList.data && assetsList.data.length > 0) {
        assetsList.map((data) => {
  ...
  useEffect(() => {
    if (exportType === 'pdf') {
      ...
      savePdfContent('Consumption Report', pdfHeaders, pdfBody, `${exportFileName}.pdf`, companyName, appliedFilters);
      ...
      <Col md="6" sm="6" lg="6" xs="12">
        <div className="p-1 text-center">
          ...
          <Button
            ...
            color="dark"
            className="bg-zodiac"
            disabled={assetsList && assetsList.data && !assetsList.data.length}
            size="sm"
            onClick={() => {
              setExportType('pdf');
            }}
      ...
      <Col md="12" sm="12" lg="12" xs="12">
        <div className="hidden-div" id="print_report">
          ...
          {exportType === 'excel'
            ? (
              <DataTable
                columns={assetsList && assetsList.data && assetsList.data.length ? dataFields : []}
