...
import { getMaterialsIssuedInfo } from '../inventoryService';
import {
...
const MaterialsIssuedReport = () => {
  ...
  const loading = materialIssued && materialIssued.loading;
  ...
  useEffect(() => {
    if (date && date.length) {
      ...
      dispatch(getMaterialsIssuedInfo(start, end));
    }
  }, [date]);
  ...
  function headers() {
    ...
    const array = DataHeaders.MaterialIssuedStartHeadings.concat(headings, DataHeaders.MaterialIssuedEndHeadings);
    return array;
  }

  useEffect(() => {
    if (materialIssued && materialIssued.data) {
      const array = [];
      materialIssued.data.map((data) => {
        let finalObj = {};
    ...
    <>
      <Row className="pt-0">
        ...
        <Col md="12" sm="12" lg={collapse ? 11 : 9} xs="12" className={collapse ? 'filter-margin-left-align pt-2 pr-2 pl-1 ' : 'pl-1 pt-2 pr-2'}>
          <Card className="p-2 mb-2 bg-lightblue h-100 material-issued-table-card">
            <CardBody className="p-1 bg-color-white m-0">
              <Row className="p-2">
                ...
                <Col md="1" xs="12" sm="1" lg="1">
                  {!loading && materialIssued && materialIssued.data && date && date.length && (
                  <div className="float-right">
                    <ExportList response={!loading && materialIssued && materialIssued.data && date && date.length > 0} />
              ...
              <Row className="pt-2">
                <Col md="12" sm="12" lg="12" xs="12">
                  {date ? (
                    <>
                      {materialIssued && materialIssued.loading && (
                        <Loader />
                      )}
                      {materialIssued && materialIssued.data && (
                        ...
                        <div className="thin-scrollbar">
                          ...
                          <Table responsive bordered>
                            <thead className="bg-gray-light">
                              <tr>
                                {customData.MaterialIssuedStartHeadings && customData.MaterialIssuedStartHeadings.map((head) => (
                                  <th className="min-width-120" key={head.label}>
                                    {head.label}
                                  </th>
                                ))}
                                ...
                                {customData.MaterialIssuedEndHeadings && customData.MaterialIssuedEndHeadings.map((head) => (
                                  <th className="min-width-120" key={head.label}>
                                    {head.label}
                                  </th>
                                ))}
                            ...
                            <tbody>
                              {materialIssued && materialIssued.data && materialIssued.data.length && materialIssued.data.map((data) => (
                                <tr>
                                  <>
                                    <td><span className="font-weight-400">{getDefaultNoValue(data['Sl No'])}</span></td>
                                    <td><span className="font-weight-400">{getDefaultNoValue(data['Material Name'])}</span></td>
                                    <td><span className="font-weight-400">{getDefaultNoValue(data.UOM)}</span></td>
                                    <td><span className="font-weight-400">{getDefaultNoValue(data['Vendor Name'])}</span></td>
                                    <td><span className="font-weight-400">{getDefaultNoValue(data['Opening Stock'])}</span></td>
                                    <td><span className="font-weight-400">{getDefaultNoValue(data['Stock Ordered'])}</span></td>
                                    <td><span className="font-weight-400">{getDefaultNoValue(data['Total Stock'])}</span></td>
                                    ...
                                    <td className="stickyemp-td"><span className="font-weight-400">{getDefaultNoValue(data['Total Consumption'])}</span></td>
                                    <td className="stickyemp-td"><span className="font-weight-400">{getDefaultNoValue(data['Closing Stock'])}</span></td>
                                    <td className="stickyemp-td"><span className="font-weight-400">{getDefaultNoValue(data['Units/Qty'])}</span></td>
                                    <td className="stickyemp-td"><span className="font-weight-400">{getDefaultNoValue(data['Unit Rate'].toFixed(2))}</span></td>
                                    <td className="stickyemp-td"><span className="font-weight-400">{getDefaultNoValue(data['Total Cost'].toFixed(2))}</span></td>
                                  </>
                                </tr>
                              ))}
                            </tbody>
...
export default MaterialsIssuedReport;
