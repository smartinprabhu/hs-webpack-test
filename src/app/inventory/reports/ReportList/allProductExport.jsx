...
const AllProductExport = (props) => {
  const {
    ppmStatusInfo, isShowArc, typeId, selectedDate, isdownloadRequest, downloadDetails, dateSelection,
  } = props;

  const { userInfo } = useSelector((state) => state.user);
  const { inventoryOverview } = useSelector((state) => state.inventory);
  const {
    selectedReportDate,
  ...
  let isData = inventoryOverview && inventoryOverview.data && inventoryOverview.data.data ? inventoryOverview.data.data : false;
  isData = isdownloadRequest ? downloadDetails : isData;

  return (
    <>
      <div id="export-allProduct-report">
        <table align="center">
          <tbody>
            <tr>
              <td colSpan={15}><b> Inventory Overview Report</b></td>
            </tr>
            <tr>
              <td>Date</td>
              <td colSpan={15}>
        ...
        {!isdownloadRequest && (
          <Col md="12" xs="12" sm="12" lg="12" className="mt-2 ml-2">
            ...
            <span className="font-weight-500">{isShowArc ? 'Yes' : 'No'}</span>
          </Col>
        )}
        </Row> */ }
...
export default AllProductExport;
