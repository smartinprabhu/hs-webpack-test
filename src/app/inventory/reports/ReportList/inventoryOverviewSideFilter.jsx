...
import {
  getInventoryOverview, resetInventoryOverview, getInventoryOverviewCount, getInventoryOverviewExport,
...
const InterviewOverviewReport = (props) => {
  const {
    setTableHeaders,
    tableHeaders,
    selectedDate,
    ...
    setShowArc,
    showExport,
    limit, currentPage, setPage, offset, setOffset,
    filterOpen,
    setFilterOpen,
    resetFilters,
    setResetFilters,
  ...
  const [customFilters, setCustomFilters] = useState({
    date: [null, null], productCategoryId: [], vendorId: [], departmentValue: [], productId: [],
  });
  ...
  const { inventoryOverview, inventoryOverviewCount } = useSelector((state) => state.inventory);
  ...
  //     if (start && end) {
  //       dispatch(getInventoryOverviewExport(start, end, product, productCateg, vendor, depart, isShowArc));
  //     }
  //   }
  // }, [showExport]);

  useEffect(() => {
    if (userInfo && userInfo.data && userInfo.data.company && selectedDate && selectedDate !== 'Custom' && (productCategoryId && productCategoryId.length > 0)) {
      let start = '';
      let end = '';
      // const todayDate = (new Date(), userInfo);
      ...
      if (dates.length > 0) {
        ...
        // dispatch(getInventoryOverviewCount(start, end, product, productCateg, vendor, depart));
        dispatch(getInventoryOverview(start, end, product, productCateg, vendor, depart, limit, offset, isShowArc));
        // dispatch(getInventoryOverviewExport(start, end, product, productCateg, vendor, depart));
      }
    }
  }, [userInfo, customFilters]);

  /* useEffect(() => {
    if (userInfo && userInfo.data && userInfo.data.company && selectedDate && selectedDate !== 'Custom') {
      let start = '';
      let end = '';
      // const todayDate = (new Date(), userInfo);
      //   const todayDate = (new Date(), userInfo, 'datetimesecs');
      ...
      if (dates.length > 0) {
        ...
        dispatch(getInventoryOverviewCount(start, end, product, productCateg, vendor, depart, isShowArc));
      }
      // dispatch(getInventoryOverviewExport(start, end, product, productCateg, vendor, depart));
    }
  }, [userInfo, customFilters]); */
  ...
  useEffect(() => {
    if ((userInfo && userInfo.data && userInfo.data.company) && selectedDate && selectedDate === 'Custom' && (productCategoryId && productCategoryId.length > 0)) {
      dispatch(resetInventoryOverview());
      // dispatch(getSelectedReportDate());
      let start = '';
      let end = '';
  ...
  const handleArcCheckboxChange = (event) => {
    setShowArc(event.target.value);
    setOffset(0);
    setPage(0);
  };
  ...
  useEffect(() => {
    if (productCategoryInfo && productCategoryInfo.data && productCategoryInfo.data.length && productCategoryOpen && ((selectedDate && selectedDate !== 'Custom') || (selectedDate && selectedDate === 'Custom' && date && date.length > 1))) {
  ...
  useEffect(() => {
    if (productCategoryInfo && productCategoryInfo.data && productCategoryInfo.data.length && ((selectedDate && selectedDate !== 'Custom') || (selectedDate && selectedDate === 'Custom' && date && date.length > 1))) {
  ...
  useEffect(() => {
    if (partnersInfo && partnersInfo.data && partnersInfo.data.length && vendorOpen) {
      const newArrData = partnersInfo.data.map((cl) => ({
        id: cl.preferred_vendor && cl.preferred_vendor.length ? cl.preferred_vendor[0] : false,
        name: cl.preferred_vendor && cl.preferred_vendor.length ? cl.preferred_vendor[1] : false,
      }));
      setVendorOptions(getArrayFromValuesById(newArrData, isAssociativeArray(vendorId || []), 'id'));
  ...
  useEffect(() => {
    if (productsData && productsData.data && productsData.data.length && productOpen && ((selectedDate && selectedDate !== 'Custom') || (selectedDate && selectedDate === 'Custom' && date && date.length > 1))) {
  ...
  useEffect(() => {
    if (productsData && productsData.data && productsData.data.length && ((selectedDate && selectedDate !== 'Custom') || (selectedDate && selectedDate === 'Custom' && date && date.length > 1))) {
  ...
  useEffect(() => {
    (async () => {
      if (userInfo && userInfo.data && ((selectedDate && selectedDate !== 'Custom') || (selectedDate && selectedDate === 'Custom' && date && date.length > 1))) {
        ...
        if (start && end) {
          await dispatch(getProductCategoryInfo(companies, appModels.PRODUCTCATEGORY, productCategoryKeyword, 'report', start, end, 'overview'));
        }
      }
    })();
  ...
  useEffect(() => {
    (async () => {
      if (userInfo && userInfo.data && ((selectedDate && selectedDate !== 'Custom') || (selectedDate && selectedDate === 'Custom' && date && date.length > 1))) {
        ...
        if (start && end) {
          await dispatch(getProductsInfo(productsListId || companies, appModels.PRODUCT, productKeyword, 'report', start, end, 'overview', getColumnArrayById(productCategoryId, 'name')));
        }
      }
    })();
  }, [userInfo, selectedDate, date, productCategoryId]);

  useEffect(() => {
    (async () => {
      if (userInfo && userInfo.data && vendorOpen) {
        await dispatch(getPartners(productsListId || companies, appModels.PARTNER, 'group', vendorKeyword, false, true));
      }
    })();
  ...
  const onProductCategChange = (data) => {
  ...
  const showProductCategModal = () => {
    setModelValue(appModels.PRODUCTCATEGORY);
    setFieldName('product_categ_ids');
    setModalName('Product Category List');
  ...
  const arcOptions = [{ value: 'True', label: 'Yes' }, { value: 'False', label: 'No' }];
  ...
  <>
    ...
    },
    {
      ...
      component:
  <>
    {arcOptions.map((ac, index) => (
      <FormControlLabel
        control={(
          <Checkbox
            type="checkbox"
            id={`checkboxarc${index}`}
            value={ac.value}
            name={ac.label}
            disabled={inventoryOverview.loading}
            checked={isShowArc === ac.value}
            onChange={handleArcCheckboxChange}
          />
              )}
        label={ac.label}
      />
  ...
  const onApplyFilters = () => {
    setFilterOpen(false);
    setCustomFilters({
      date, productCategoryId, departmentValue, vendorId, productId, selectedDate,
    });
    dispatch(getTypeId({
      date, productCategoryId, vendorId, departmentValue, productId, selectedDate,
    }));
  };
    ...
    <>
      <Drawer anchor="right" open={filterOpen} PaperProps={{ sx: { width: '30%' } }}>
        <ReportsFilterDrawer
          ...
          isDisabled={!((selectedDate && selectedDate !== 'Custom' || (selectedDate === 'Custom' && date && date.length && date[0] !== null && date[1] !== null)) && (productCategoryId && productCategoryId.length > 0))}
        />
      </Drawer>
      ...
      <Dialog maxWidth="xl" open={extraModal}>
        ...
        <DialogContent sx={{ width: '1000px' }}>
          {/* <SearchModalMultiple
            ...
            oldSpaceValues={productCategoryId}
          /> */}
          <SearchModalMultiple
            modelName={modelValue}
            afterReset={() => { setExtraModal(false); }}
...
export default InterviewOverviewReport;
