...
import { getInwardSummary, resetInwardSummary } from '../../inventoryService';
import { getEquipmentList, getSpaceAllSearchList } from '../../../helpdesk/ticketService';
import { getProductCategoryInfo, getProductsInfo } from '../../../purchase/purchaseService';
import {
  getPartners,
...
const FilterInwardReport = (props) => {
  const {
    setTableHeaders,
    tableHeaders,
    selectedDate,
  ...
  useEffect(() => {
    if (userInfo && userInfo.data && userInfo.data.company && selectedDate && selectedDate !== 'Custom') {
      ...
      if (dates.length > 0) {
        ...
        dispatch(getSelectedReportDate(getDatesOfQueryInventoryReport(selectedDate, userInfo, start, end)));
      }
      dispatch(getInwardSummary(start, end, product, productCateg, vendor, depart));
    }
  }, [customFilters]);
  ...
  useEffect(() => {
    if ((userInfo && userInfo.data && userInfo.data.company) && selectedDate && selectedDate === 'Custom' && (date && date.length)) {
      dispatch(resetInwardSummary());
      dispatch(getSelectedReportDate());
      let start = '';
      let end = '';
  ...
  useEffect(() => {
    if (productCategoryInfo && productCategoryInfo.data && productCategoryInfo.data.length && productCategoryOpen) {
  ...
  useEffect(() => {
    if (productsData && productsData.data && productsData.data.length && productOpen) {
  ...
  useEffect(() => {
    (async () => {
      if (userInfo && userInfo.data && productCategoryOpen) {
        // await dispatch(getBuildings(companies, appModels.SPACE, productCategoryKeyword));
        await dispatch(getProductCategoryInfo(companies, appModels.PRODUCTCATEGORY, productCategoryKeyword));
      }
    })();
  }, [userInfo, productCategoryKeyword, productCategoryOpen]);

  useEffect(() => {
    (async () => {
      if (userInfo && userInfo.data && departmentOpen) {
        await dispatch(getDepartments(companies, appModels.DEPARTMENT1, departmentKeyword));
      }
    })();
  ...
  useEffect(() => {
    (async () => {
      if (userInfo && userInfo.data && productOpen) {
        await dispatch(getProductsInfo(productsListId || companies, appModels.PRODUCT, productKeyword));
      }
    })();
  ...
  //     dispatch(getInwardSummary(start, end, preventiveFor, assetId, commenceDate));
  ...
  const showProductModal = () => {
    ...
    setColumns(['id', 'name', 'unique_code']);
...
export default FilterInwardReport;
