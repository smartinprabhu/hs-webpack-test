...
import { getConsumptionSummary, resetConsumptionSummary } from '../../inventoryService';
import { getEquipmentList, getSpaceAllSearchList } from '../../../helpdesk/ticketService';
import {
  getProductCategoryInfo, getVendorGroups, getProductsInfo, productFilters,
...
const FilterConsumptionReport = (props) => {
  const {
    setTableHeaders,
    tableHeaders,
    selectedDate,
  ...
  const [date, changeDate] = useState(customFilters.data);
  ...
  useEffect(() => {
    if (userInfo && userInfo.data && userInfo.data.company && selectedDate && selectedDate !== 'Custom' && opTypeValue && opTypeValue.id) {
      ...
      dispatch(getConsumptionSummary(start, end, product, productCateg, vendor, depart, opTypeValue.id));
    }
  ...
  useEffect(() => {
    if ((userInfo && userInfo.data && userInfo.data.company) && selectedDate && selectedDate === 'Custom' && opTypeValue && opTypeValue.id) {
      dispatch(resetConsumptionSummary());
      dispatch(getSelectedReportDate());
      let start = '';
      let end = '';
  ...
  //       }
  //       dispatch(getSelectedReportDate(getDatesOfQueryInventoryReport(selectedDate, userInfo)));
  //       dispatch(getConsumptionSummary(start, end, preventiveFor, assetId, commenceDate));
  //     }
  //   }, [userInfo, date, productCategoryId, vendorId, departmentValue]);
      ...
      <FormControlLabel
        control={(
          <Checkbox
            ...
            value={tp.label}
            name={tp.label}
            disabled={consumptionSummary.loading}
...
export default FilterConsumptionReport;
