...
import InwardStockDetailPrint from './ReportList/InwardStockDetailPrint';
...
const InwardDetailReport = React.memo((props) => {
  const {
  ...
  const loading = ((userInfo && userInfo.loading) || (inwardDetailSummary && inwardDetailSummary.loading));
  ...
  const isError = inwardDetailSummary && inwardDetailSummary.data ? !inwardDetailSummary.data.status : false;
  // const isNoData = (isData && isData.headers && isData.headers.length > 0 && isData.stock_details && isData.stock_details.length > 0) ? 'true' : 'false';
  const qlIndex = 1;
  ...
  useEffect(() => {
    if (exportType === 'excel') {
      exportTableToExcel('export-inward-detail-report', 'Inward Detail Report')
  ...
  const statingFields = ['sl_no', 'product_name', 'unique_code', 'specification', 'brand', 'department', 'category', 'uom', 'vendor_name',]
  const endingFields = ['inward_stock', 'unit_rate', 'total_cost']

  useEffect(() => {
    if (inwardDetailSummary?.data?.data?.headers) {
      const fieldsArray = []
      const dateFieldsArray = []
      inwardDetailSummary.data.data.stock_details.map((stock, stockIndex) => {
        stock.date_list.map((dateList, index) => {
          stock[`date_list${index}`] = dateList
          if (stockIndex === 0) {
            dateFieldsArray.push(`date_list${index}`)
          }
        })
      })
      const finalArray = [...statingFields, ...dateFieldsArray, ...endingFields]
      inwardDetailSummary.data.data.headers.map((header, index) => {
        const obj = {
          field: finalArray[index],
          headerName: header,
          width: header === 'Sl No' ? 60 : 180,
          editable: true,
          valueGetter: (params) => statingFields.includes(finalArray[index]) ? getDefaultNoValue(params.value) : params.value ? params.value.toFixed(2) : 0,
  ...
  return (
    <Box sx={{ fontFeatureSettings: "Suisse Intl", position: 'sticky' }} >
      ...
      {isData && isData.stock_details && isData.stock_details.length > 0 && inwardDetailSummary && !inwardDetailSummary.err && !inwardDetailSummary.loading && selectedReportDate1 && selectedDate && SummaryColumns.length > 0 && (
        <CommonGrid
          ...
          loading={inwardDetailSummary && inwardDetailSummary.loading}
          err={inwardDetailSummary && inwardDetailSummary.err}
          noHeader={true}
          disableFilters={true}
      ...
      <div className="d-none">
        <InwardStockDetailPrint typeId={typeId} inwardDetailSummary={inwardDetailSummary} showObservations={showObservations} tableHeaders={tableHeaders} selectedDate={selectedDate} />
      </div>
      {loading && (
      ...
      {(inwardDetailSummary && inwardDetailSummary.err) && (
        <ErrorContent errorTxt={generateErrorMessage(inwardDetailSummary)} />
      )}
...
InwardDetailReport.propTypes = {
  collapse: PropTypes.bool,
  afterReset: PropTypes.func.isRequired,
  reportName: PropTypes.string.isRequired,
};
InwardDetailReport.defaultProps = {
  collapse: false,
};

export default InwardDetailReport;
