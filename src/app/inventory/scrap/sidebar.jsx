...
import {
  getScrapsList, getScrapsCount, getScrapFilters,
  getScrapDetail,
...
const Sidebar = (props) => {
  ...
  const {
    scrapFilters, scrapCount, scrapsInfo,
    addScrapInfo, updateScrapInfo, actionResultInfo,
  ...
  useEffect(() => {
    if (scrapsInfo && scrapsInfo.data && id) {
      const arr = [...scrollDataList, ...scrapsInfo.data];
  ...
  useEffect(() => {
    if (removedStatusValue && removedStatusValue !== 0) {
      ...
      if (checkItems.filter((item) => item.id !== removedStatusValue) && checkItems.filter((item) => item.id !== removedStatusValue).length === 0) {
        dispatch(getScrapFilters(checkItems.filter((item) => item.id !== removedStatusValue)), checkDatesItems, customFiltersList);
      }
    }
  }, [removedStatusValue]);
  ...
  useEffect(() => {
    if (removedDateValue && removedDateValue !== 0) {
      ...
      if (checkDatesItems.filter((item) => item.id !== removedDateValue) && checkDatesItems.filter((item) => item.id !== removedDateValue).length === 0) {
        dispatch(getScrapFilters(checkItems, checkDatesItems.filter((item) => item.id !== removedDateValue)), customFiltersList);
      }
    }
  }, [removedDateValue]);
  ...
  useEffect(() => {
    if ((checkItems && checkItems.length > 0) || (checkDatesItems && checkDatesItems.length > 0)) {
      dispatch(getScrapFilters(checkItems, checkDatesItems, customFiltersList));
    }
  ...
  useEffect(() => {
    if (userInfo && userInfo.data && (scrapFilters && (scrapFilters.statuses || scrapFilters.dates || scrapFilters.customFilters))) {
      const statusValues = scrapFilters.statuses ? getColumnArrayById(scrapFilters.statuses, 'id') : [];
      const dateValues = scrapFilters.dates ? getColumnArrayById(scrapFilters.dates, 'id') : [];
      const customFilters = scrapFilters.customFilters ? queryGenerator(scrapFilters.customFilters) : '';
      dispatch(getScrapsCount(companies, appModels.STOCKSCRAP, statusValues, dateValues, customFilters));
    }
  }, [userInfo, scrapFilters]);
  ...
  useEffect(() => {
    if (userInfo && userInfo.data && (scrapFilters && (scrapFilters.statuses || scrapFilters.dates || scrapFilters.customFilters))) {
      ...
      const dateValues = scrapFilters.dates ? getColumnArrayById(scrapFilters.dates, 'id') : [];
      const customFilters = scrapFilters.customFilters ? queryGenerator(scrapFilters.customFilters) : '';
      dispatch(getScrapsList(companies, appModels.STOCKSCRAP, limit, offsetValue, statusValues, dateValues, customFilters, sortByValue, sortFieldValue));
    }
  }, [userInfo, offsetValue, sortByValue, sortFieldValue, scrapFilters, scrollTop]);
  ...
  useEffect(() => {
    if (userInfo && userInfo.data && id === 0) {
      ...
      const dateValues = scrapFilters.dates ? getColumnArrayById(scrapFilters.dates, 'id') : [];
      const customFilters = scrapFilters.customFilters ? queryGenerator(scrapFilters.customFilters) : '';
      dispatch(getScrapsList(companies, appModels.STOCKSCRAP, limit, offset, statusValues, dateValues, customFilters, sortByValue, sortFieldValue));
    }
  }, [id]);

  useEffect(() => {
    if ((userInfo && userInfo.data) && ((addScrapInfo && addScrapInfo.data) || (updateScrapInfo && updateScrapInfo.data))) {
      const statusValues = scrapFilters.statuses ? getColumnArrayById(scrapFilters.statuses, 'id') : [];
      const dateValues = scrapFilters.dates ? getColumnArrayById(scrapFilters.dates, 'id') : [];
      const customFilters = scrapFilters.customFilters ? queryGenerator(scrapFilters.customFilters) : '';
      dispatch(getScrapsCount(companies, appModels.STOCKSCRAP, statusValues, dateValues, customFilters));
  ...
  const handleCheckboxChange = (event) => {
    ...
    if (checked) {
      ...
      if (checkItems.filter((item) => item.id !== value) && checkItems.filter((item) => item.id !== value).length === 0) {
        dispatch(getScrapFilters(checkItems.filter((item) => item.id !== value), checkDatesItems, customFiltersList));
      }
    }
    setOffsetValue(0);
  ...
  const handleDateCheckboxChange = (event) => {
    ...
    if (checked) {
      ...
      if (checkDatesItems.filter((item) => item.id !== value) && checkDatesItems.filter((item) => item.id !== value).length === 0) {
        dispatch(getScrapFilters(checkItems, checkDatesItems.filter((item) => item.id !== value), customFiltersList));
      }
    }
    setOffsetValue(0);
    if (afterReset) afterReset();
  ...
  const handleCustomFilterClose = (value) => {
    ...
    dispatch(getScrapFilters(checkItems, checkDatesItems, customFiltersList.filter((item) => item.key !== value)));
    setOffsetValue(0);
    if (afterReset) afterReset();
    setScrollData([]);
  };
  ...
  const onScroll = (e) => {
    ...
    const total = scrapCount && scrapCount.length ? scrapCount.length : 0;
    const scrollListCount = scrollDataList && scrollDataList.length ? scrollDataList.length : 0;
    if ((scrapsInfo && !scrapsInfo.loading) && bottom && (total !== scrollListCount) && (total >= offsetValue)) {
  ...
  const statusValues = scrapFilters && scrapFilters.statuses ? getColumnArrayById(scrapFilters.statuses, 'id') : [];
  const dateValues = scrapFilters && scrapFilters.dates ? getColumnArrayById(scrapFilters.dates, 'id') : [];

  const statusList = scrapFilters && scrapFilters.statuses ? scrapFilters.statuses : [];
  const dateList = scrapFilters && scrapFilters.dates ? scrapFilters.dates : [];
        ...
        <>
          ...
          {id ? (
            <>
              <div>
                ...
                <div onScroll={onScroll} className={scrollDataList && scrollDataList.length > 9 ? 'height-100 table-scrollable thin-scrollbar' : ''}>
                  ...
                  {(scrapsInfo && scrapsInfo.err) && (
                    <Card className="mr-2 ml-2 mb-2 border-nepal-1px">
                      <CardBody className="p-2">
                        <ErrorContent errorTxt={generateErrorMessage(scrapsInfo)} />
                      </CardBody>
                    </Card>
                  )}
                  ...
                  {((scrapsInfo && scrapsInfo.loading) || (userInfo && userInfo.loading)) && (
                    <Loader />
                  )}
                </div>
              </div>
            ...
            <CardBody className="ml-2 p-0 mt-2 position-relative side-filters-list thin-scrollbar">
              ...
              <Row className="m-0">
                <Col md="8" xs="8" sm="8" lg="8" className="p-0">
                  <p className="m-0 font-weight-800 collapse-heading">BY EXPECTED DATE</p>
                </Col>
                <Col md="4" xs="4" sm="4" lg="4" className="text-right p-0">
