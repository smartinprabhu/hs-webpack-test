...
import {
  getScrapsExport, setInitialValues,
...
const DataExport = (props) => {
  ...
  const exportFileName = getExportFileName('Inventory_Scraps');
  const [columns, setColumns] = useState(fields);
  const [dataFields, setDataFields] = useState([]);

  const { userInfo } = useSelector((state) => state.user);
  const {
    scrapCount, scrapFilters, scrapsExportInfo, scrapRows,
  ...
  useEffect(() => {
    if ((userInfo && userInfo.data) && (scrapCount && scrapCount.length)) {
      const offsetValue = 0;
      const states = scrapFilters.statuses ? getColumnArrayById(scrapFilters.statuses, 'id') : [];
      const dates = scrapFilters.dates ? getColumnArrayById(scrapFilters.dates, 'id') : [];
      const customFilters = scrapFilters.customFilters ? queryGeneratorV1(scrapFilters.customFilters) : '';
      const rows = scrapRows.rows ? scrapRows.rows : [];
      dispatch(getScrapsExport(companies, appModels.STOCKSCRAP, scrapCount.length, offsetValue, InventoryModule.scrapApiFields, customFilters, rows, sortedValue.sortBy, sortedValue.sortField));
    }
  }, [userInfo, scrapCount, sortedValue, scrapFilters]);

  function checkFieldExists(dataColumns) {
    let dataColumnsList = dataColumns;
    if (columns.some((selectedValue) => selectedValue.includes('picking_id'))) {
      dataColumnsList = dataColumnsList.concat(tableFields.pickFields);
    }
    /* if (columns.some((selectedValue) => selectedValue.includes('date_expected'))) {
      dataColumnsList = dataColumnsList.concat(tableFields.dateFields);
    }
    if (columns.some((selectedValue) => selectedValue.includes('origin'))) {
      dataColumnsList = dataColumnsList.concat(tableFields.originFields);
    } */
  ...
  const appliedFilters = filterStringGenerator(scrapFilters);

  useEffect(() => {
    if (fields && fields.length > 0 && scrapsExportInfo && scrapsExportInfo.data && scrapsExportInfo.data.length > 0) {
  ...
  useEffect(() => {
    if (exportType === 'pdf') {
      ...
      savePdfContent('Inventory Scraps', pdfHeaders, pdfBody, `${exportFileName}.pdf`, companyName, appliedFilters);
      dispatch(setInitialValues(false, false, false, false));
    } else if (exportType === 'excel') {
      const currentDate = getLocalTime(new Date());
      const title = `Inventory_Scraps_On_${currentDate}`;
  ...
  return (
    <Row>
      <Col md="12" sm="12" lg="6">
        <div className="p-3 text-center">
          ...
          <Button
            ...
            color="dark"
            className="bg-zodiac"
            disabled={scrapsExportInfo && scrapsExportInfo.loading}
            onClick={() => {
              setExportType('pdf');
            }}
          >
            {(scrapsExportInfo && scrapsExportInfo.loading) ? (
      ...
      {scrapsExportInfo && scrapsExportInfo.err && (
        <Col md="12" sm="12" lg="12" xs="12">
          <SuccessAndErrorFormat response={scrapsExportInfo} />
        </Col>
      )}
      <Col md="12" sm="12" lg="12" className="d-none">
        <div id="print_report">
          ...
          <table align="center">
            <tbody>
              <tr><td style={{ textAlign: 'center' }} colSpan={5}><b>Inventory Scraps Report</b></td></tr>
              <tr>
                <td>Company</td>
          ...
          <br />
          {exportType === 'excel' && (
            <DataTable columns={checkFieldExists(dataFields)} data={scrapsExportInfo && scrapsExportInfo.data ? scrapsExportInfo.data : []} propertyAsKey="id" />
          )}
        </div>
      </Col>
