...
import {
  resetUpdateLocation, getLocation
} from '../../inventoryService';
const appModels = require('../../../util/appModels').default;

import actionCodes from '../../data/actionCodes.json';

const tabs = ["Location Overview"];

const checkLocationStatus = (val) => {
  return (
    <Box>
...
const LocationDetail = () => {
  ...
  const { locationDetails } = useSelector((state) => state.inventory);
  ...
  const detailData = locationDetails && (locationDetails.data && locationDetails.data.length > 0) ? locationDetails.data[0] : '';
  const loading = locationDetails && locationDetails.loading;
  ...
  function checkActionAllowed(actionName) {
    let allowed = true;
    const whState = locationDetails && locationDetails.data ? locationDetails.data[0].active : '';
    if (whState === 'done') {
      allowed = false;
    }
  ...
  const onUpdateReset = () => {
    if (detailData && detailData.id) {
      dispatch(getLocation(detailData.id, appModels.STOCKLOCATION));
    }
      ...
      <>
        <Box>
          ...
          <DetailViewHeader
            ...
            status={
              detailData.active
                ? checkLocationStatus(detailData.active)
                : "-"
            }
            subHeader={
              <>
          ...
          >
                        ...
                        [
                          {
                            ...
                            value: getDefaultNoValue(detailData.usage),
                          },
                          {
                            property: "Company",
            ...
            >
              <DetailViewRightPanel
                ...
                panelOneValue1={getDefaultNoValue((detailData.location_id))}
          ...
          >
            ...
            <DrawerHeader
              headerName="Update Location"
              imagePath={InventoryBlue}
              onClose={() => setEditLink(false)}
            />
            <AddLocation afterReset={() => { onUpdateReset(); }} editId={editId} />
          </Drawer>
          ...
          {/* <Dialog maxWidth={'md'} open={actionModal} >
            ...
            <CardBody data-testid="success-case" className="bg-lightblue p-3">
                  ...
                  <Row>
                    <Col md="12" xs="12" sm="12" lg="12" className="p-0">
                      ...
                      <span className="font-weight-400">
                        {LocationStatusJson(isResult && (locationDetails && !locationDetails.loading) ? getAlertStatus(actionText) : detailData.state)}
                      </span>
                    </Col>
                  </Row>
                </Col>
              ...
              {isResult && (locationDetails && !locationDetails.loading) && (
              ...
              {((locationDetails && locationDetails.loading) || (loading)) && (
...
{isResult && (
  <Button color="primary" disabled={locationDetails.loading} onClick={toggle}>Ok</Button>
