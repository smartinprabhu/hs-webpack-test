  ...
  useEffect(() => {
    if (modelName && fields) {
      ...
      if (fieldName === 'location_id') {
        const searchValueMultiple = `["&",["company_id","in",[${company}]],["name", "ilike", "${searchValue}"],["usage","not in",["supplier", "production"]]]`;
        dispatch(getExtraSelectionMultiple(company, modelName, limit, offset, fields, searchValueMultiple));
      }
      if (fieldName === 'default_location_src_id') {
        const searchValueMultiple = `["&",["company_id","in",[${company}]],["name", "ilike", "${searchValue}"]]`;
        dispatch(getExtraSelectionMultiple(company, modelName, limit, offset, fields, searchValueMultiple));
      }
      if (fieldName === 'default_location_dest_id') {
        const searchValueMultiple = `["&",["company_id","in",[${company}]],["name", "ilike", "${searchValue}"]]`;
        dispatch(getExtraSelectionMultiple(company, modelName, limit, offset, fields, searchValueMultiple));
      }
    }
  ...
  useEffect(() => {
    if (modelName && fields) {
      if ((addVendorInfo && addVendorInfo.data)) {
        const searchValueMultiple = `["&",["company_id","in",[${company}]], ["parent_id", "=", false],"|","|",["email", "ilike", "${searchValue}"],["name", "ilike", "${searchValue}"],["mobile", "ilike", "${searchValue}"]]`;
        dispatch(getExtraSelectionMultiple(company, modelName, limit, offset, fields, searchValueMultiple));
            ...
            <Row className="pl-2 pr-2 pb-0 pt-0">
              ...
              <Col sm="12" md="2" lg="2" xs="12" className="pl-0">
                <div className="float-right">
                  <FormGroup>
                    <Input type="input" name="search" id="exampleSearch" placeholder="Search.." onChange={(e) => onSearchChange(e, 'search')} onKeyDown={onSearchChange} className="subjectticket bw-2 mt-2" />
                  </FormGroup>
                </div>
              </Col>
            </Row>
