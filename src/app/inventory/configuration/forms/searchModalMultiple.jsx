...
const SearchModalMutliple = (props) => {
  ...
  useEffect(() => {
    if (fieldName === 'approval_user_role_ids') {
      setCheckRows(oldServiceData);
    }
  }, []);
    ...
    for (let i = 0; i < arrayData.length; i += 1) {
      tableTr.push(
        <tr className={selectedId === arrayData[i].id ? 'text-info cursor-pointer' : 'cursor-pointer'} key={i}>
          <td className="w-5">
            {(fieldName === 'approval_user_role_ids') && (
              <FormGroup>
          ...
          {(fieldName === 'approval_user_role_ids') && (
            <td className='pt-4'><span className="font-weight-400">{getDefaultNoValue(arrayData[i].name)}</span></td>
          )}
        </tr>,
      );
    }
          ...
          <CardBody className="bg-color-white p-1 m-0">
            ...
            <Modal isOpen={modalAlert} toggle={toggleAlert} size="sm">
              ...
              <ModalBody>
                Maximum allowed
                {' '}
                {fieldName === 'approval_user_role_ids' ? 'spaces' : 'equipments'}
                {' '}
                : 10
              </ModalBody>
              <ModalFooter>
...
SearchModalMutliple.propTypes = {
  ...
  oldServiceData: PropTypes.oneOfType([
    PropTypes.array,
    PropTypes.object,
    PropTypes.string,
  ]).isRequired,
