...
const ActionValidate = (props) => {
  ...
  const { stockValidateInfo } = useSelector((state) => state.inventory);
  const { userInfo } = useSelector((state) => state.user);

  const isResult = stockValidateInfo && stockValidateInfo.data;
  const loading = stockValidateInfo && stockValidateInfo.loading;
  const isError = stockValidateInfo && stockValidateInfo.err;
  ...
  const handleStateChange = (id) => {
    dispatch(setValidateStock(id, false));
  };
  ...
  return (
    <Modal size="md" className="border-radius-50px modal-dialog-centered" isOpen={actionModal}>
      <ModalHeaderComponent fontAwesomeIcon={faIcons[actionText]} closeModalWindow={toggle} title={`${actionText} Stock Audit`} response={stockValidateInfo} />
      <ModalBody>
        ...
        <Row className="justify-content-center">
          {isResult && (details && !details.loading) && (
            <SuccessAndErrorFormat response={stockValidateInfo} successMessage={`This Stock Audit has been ${getAlertText(actionText)} successfully..`} />
          )}
          {isError && (
          <SuccessAndErrorFormat response={stockValidateInfo} />
          )}
