...
const AdvancedSearchModal = (props) => {
  ...
  useEffect(() => {
    if (modelName && fields) {
      ...
      if (assetType) {
        searchValueMultiple = `${searchValueMultiple},["type_category","=","${assetType}"]`;
      }

      if (categoryId) {
        if (typeof categoryId === 'object') {
          searchValueMultiple = `${searchValueMultiple},["${assetType === 'equipment' ? 'category_id' : 'asset_category_id'}","in",${JSON.stringify(categoryId)}]`;
        } else {
          searchValueMultiple = `${searchValueMultiple},["${assetType === 'equipment' ? 'category_id' : 'asset_category_id'}","=",${categoryId}]`;
        }
      }
              ...
              <Col sm="12" md="5" lg="5" xs="12" className="text-right">
                {/* {isAddTeam
                  && (fieldName === 'maintenance_team_id'
                    || fieldName === 'approval_authority_id' || fieldName === 'ehs_authority_id' || fieldName === 'security_office_id' || fieldName === 'reviewer_id') && (
