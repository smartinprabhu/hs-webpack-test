...
import {
  geWorkPermitDashboard, getWorkPermitFilters,
...
const Insights = () => {
  ...
  const dashboardName = 'Work Permit';
  ...
  useEffect(() => {
    if (userInfo && userInfo.data && currentTab && currentTab !== 'Custom') {
      ...
      if (dates.length > 0) {
        const start = dates[0];
        const end = dates[1];
        dispatch(geWorkPermitDashboard(start, end, dashboardName));
      } else {
  ...
  const total = getTotal(workPermitDashboard && workPermitDashboard.data ? workPermitDashboard.data : []);

  const gridData = generateArrayFromValue(workPermitDashboard && workPermitDashboard.data ? workPermitDashboard.data : [], 'ks_dashboard_item_type', 'ks_tile');
            ...
            >
              ...
              <Tooltip title={`${actions.name}(${actions.datasets && actions.datasets[0] ? actions.datasets && actions.datasets[0] : 0})`}>
                <CardBody id="Tooltip-Insights" className="pb-1 pl-5 pr-5 pt-0">
                  <CircularProgressbarWithChildren
                    value={actions.datasets && actions.code && actions.code !== 'AP' ? newpercalculate(total, actions.datasets) : '100'}
                    strokeWidth={9}
                    styles={buildStyles({
                      textColor: '#3a4354',
              ...
              <CardFooter className="bg-med-blue border-0 pt-0">
                {actions.datasets && actions.datasets.length && actions.datasets.length > 0 && actions.datasets[0] !== 0 && actions.code ? (
                  ...
                  : (
                    <Button color="secondary" size="sm" disabled className="bg-white  text-dark rounded-pill  mb-1">
                      <small className="text-center font-weight-800">
                        Go to
                        {' '}
                        {truncate(actions.name, 10)}
