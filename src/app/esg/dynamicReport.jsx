...
import esgNav from './navbar/navlist.json';
...
const DynamicReports = (props) => {
  ...
  if (headerTabs) {
    const tabsDef = getTabs(headerTabs[0].menu, esgNav.data);
    let dynamicList = headerTabs[0].menu.filter((item) => !(esgNav && esgNav.data && esgNav.data[item.name]) && item.name !== 'Help');
