...
const LogNotes = () => {
  ...
  const wpConfig = btConfigInfo && btConfigInfo.data && btConfigInfo.data.length ? btConfigInfo.data[0] : false;

  const loading = trackerDetails && trackerDetails.loading;
  const isErr = trackerDetails && trackerDetails.err;
  const inspDeata = trackerDetails && trackerDetails.data && trackerDetails.data.length ? trackerDetails.data[0] : false;
        ...
        <tr key={i}>
          ...
          <td className="p-2">
            <div className="display-flex-div content-center">
              <p className="m-0">
                {getDefaultNoValue(assetData[i].to_state)}
              </p>
            </div>
          </td>
        ...
        {isChecklist && (
          <div>
            <Table responsive className="mb-0 mt-0 font-weight-400 border-0 assets-table" width="100%">
              ...
              <tbody>
                {getRow(trackerDetails.data.length > 0 ? trackerDetails.data[0].status_logs : [])}
              </tbody>
            </Table>
            <hr className="m-0" />
          </div>
        )}
        <DetailViewFormat detailResponse={trackerDetails} />
