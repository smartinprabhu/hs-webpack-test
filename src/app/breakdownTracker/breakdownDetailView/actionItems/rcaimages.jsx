...
import {
  getUploadRCAImageInfo,
} from '../../breakdownService';
import {
  mergeArray, isMemorySizeLarge, getFileExtension, isArrayValueExistsData,
} from '../../../util/appUtils';
...
const RacImages = () => {
  ...
  const {
    uploadPhotoRca, attachmentCategoryInfo,
  ...
  useEffect(() => {
    if (finish) {
      if (isMemorySizeLarge(filesList)) {
        dispatch(getUploadRCAImageInfo(filesList));
      } else {
        setimgSize(true);
        setErrorShow(true);
      }
  ...
  const irCategory = irCategoryList && (isArrayValueExistsData(irCategoryList, 'name', 'RCA'));

  const handleFilesRCA = (files) => {
    setimgValidation(false);
    setimgSize(false);
    const appendFileList = [...filesList, ...mergeArray(files.base64, files.fileList, companyId, appModels.BREAKDOWNTRACKER, false, irCategory)];
  ...
  const onRemovePhoto = (id) => {
    ...
    dispatch(getUploadRCAImageInfo(filesList.filter((item, index) => index !== id)));
  };
  ...
  return (
    <>
        ...
        <Col md="12" lg="12">
          ...
          <span className="float-right">
            {filesList.length < filesLimit && (
              <ReactFileReader
                handleFiles={handleFilesRCA}
                multipleFiles
                elementId="fileUploadsRCA"
      ...
      <Row className="mb-5">
        {uploadPhotoRca && uploadPhotoRca.length && uploadPhotoRca.length > 0 ? (
          <>
            {uploadPhotoRca.map((fl, index) => (
