...
const Images = (addModal) => {
  ...
  //const filesLimit = userInfo && userInfo.data && userInfo.data.maintenance_setting && userInfo.data.maintenance_setting.helpdesk_attachment_limit ? userInfo.data.maintenance_setting.helpdesk_attachment_limit : 5;
  const filesLimit = 3;
  ...
  useEffect(() => {
    if (addModal && uploadPhoto && !uploadPhoto.length) {
      setFilesList([]);
    }
  }, [addModal]);
  ...
  const handleFiles = (files) => {
    ...
    if (files !== undefined && totalLength <= filesLimit) {
      setFilesList([...filesList, ...mergeArray(files.base64, files.fileList, companyId, appModels.BREAKDOWNTRACKER)]);
      // setModalAlert(true);
        ...
        {uploadPhoto && uploadPhoto.length && uploadPhoto.length > 0 ? (
          <>
            {uploadPhoto.map((fl, index) => (
              <Col sm="4" md="4" lg="4" xs="4" className="position-relative mb-2" key={fl.name}>
                {(getFileExtension(fl.datas_fname) === 'png' || getFileExtension(fl.datas_fname) === 'svg' || getFileExtension(fl.datas_fname) === 'jpeg' || getFileExtension(fl.datas_fname) === 'jpg') && (
                  <>
                    ...
                    />
                    <div className="position-absolute topright-img-close-small">
                      <img
                        aria-hidden="true"
                        src={closeCircleIcon}
                        className="cursor-pointer"
