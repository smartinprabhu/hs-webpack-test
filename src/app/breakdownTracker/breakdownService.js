import {
  ...
  getTrackerActs, getSubmittedTos, getTrackerLog, getClosureDurationInfo, getTrackerEvidences, getTrackerTemplateDetails, complianceStateChangeInfo,
  getTrackerDashboardInfo, getCategoryGroupsInfo, getTrackerCategorys, getTrackerReports, getRaisedLists, getServiceImpacteds,
  getServiceCategorys, getEquipmentLists, setComplianceReinitiateInfo, getOnHoldRequestInfo, onHoldApprovalInfo, getServiceCategoryGroupsInfo, getRaisedByGroupsInfo, getStatusByGroupsInfo, getAttachmentCategoryLists,
} from './actions';
...
export const RESET_CLOSURE_DURATION_INFO = 'RESET_CLOSURE_DURATION_INFO';
export const RESET_BT_OH_REQUEST_INFO = 'RESET_BT_OH_REQUEST_INFO';
export const RESET_BT_OH_APPROVAL_INFO = 'RESET_BT_OH_APPROVAL_INFO';
export const RESET_COMPLIANCE_REINITIATE_INFO = 'RESET_COMPLIANCE_REINITIATE_INFO';

export function getTrackerList(company, model, limit, offset, customFilters, sortByValue, sortFieldValue, globalFilter, keyword) {
  return (dispatch) => {
    dispatch(getTrackerListInfo(company, model, limit, offset, customFilters, sortByValue, sortFieldValue, globalFilter, keyword));
  };
}
...
export function getTrackerCount(company, model, customFilters, globalFilter, keyword) {
  return (dispatch) => {
    dispatch(getTrackerCountInfo(company, model, customFilters, globalFilter, keyword));
  };
}
...
export function getEquipmentList(company, model, keyword, assetCategory) {
  return (dispatch) => {
    dispatch(getEquipmentLists(company, model, keyword, assetCategory));
  };
}
...
export function getClosureDuration(critical, priority, service) {
  return (dispatch) => {
    dispatch(getClosureDurationInfo(critical, priority, service));
  };
}

export function resetCloseDuration(result) {
  return {
    type: RESET_CLOSURE_DURATION_INFO,
    payload: result,
  };
}
...
export function resetOnHoldRequest(result) {
  return {
    type: RESET_BT_OH_REQUEST_INFO,
    payload: result,
  };
}

export function onHoldApproval(id, model, args, method, onHoldRejectRemarks) {
  return (dispatch) => {
    dispatch(onHoldApprovalInfo(id, model, args, method, onHoldRejectRemarks));
  };
}

export function resetOnHoldApproval(result) {
  return {
    type: RESET_BT_OH_APPROVAL_INFO,
    payload: result,
  };
}

export function setComplianceReinitiate(id, state, modelName, args, date) {
  return (dispatch) => {
    dispatch(setComplianceReinitiateInfo(id, state, modelName, args, date));
  };
}

export function resetComplianceReinitiate(result) {
  return {
    type: RESET_COMPLIANCE_REINITIATE_INFO,
    payload: result,
  };
}
