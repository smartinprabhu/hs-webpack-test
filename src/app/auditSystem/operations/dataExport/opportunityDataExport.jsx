...
import { getStateText } from '../../utils/utils';
...
const DataExport = (props) => {
  ...
  const exportFileName = getExportFileName('Audits_Report')
  const [columns, setColumns] = useState(fields);
  const [pdfBody, setPdfBody] = useState([]);
  const dataFields = tableFields.opportunityfields;
  ...
  const appliedFilters = filterStringGeneratorDynamic(opportunitiesFilters);
  ...
  useEffect(() => {
    if ((userInfo && userInfo.data) && (opportunitiesCount && opportunitiesCount.length)) {
      const offsetValue = 0;
      const customFiltersQuery = opportunitiesFilters && opportunitiesFilters.customFilters ? queryGeneratorWithUtc(opportunitiesFilters.customFilters, false, userInfo.data) : '';
      dispatch(getAuditExport(companies, appModels.SYSTEMOPPORTUNITIES, opportunitiesCount.length, offsetValue, columns, customFiltersQuery, rows, sortBy, sortField));
    }
  }, [userInfo, opportunitiesCount, opportunitiesFilters]);
  ...
  function checkFieldExists(dataColumns) {
    ...
    if (columns.some((selectedValue) => selectedValue.includes('create_date'))) {
      dataColumnsList = dataColumnsList.concat(tableFields.createOpportunityField);
    }
  ...
  useEffect(() => {
    if (fields && fields.length > 0 && auditsExport && auditsExport.data && auditsExport.data.length > 0) {
    ...
    if (pdfHeader && pdfHeader.__wrapped__ && pdfHeader.__wrapped__.length > 0) {
      ...
      if (auditsExport && auditsExport.data && auditsExport.data.length > 0) {
        auditsExport.data.map((data) => {
          data.date = getCompanyTimezoneDate(data.date, userInfo, 'datetime');
  ...
  useEffect(() => {
    if (exportType === 'pdf') {
      ...
      savePdfContent('Audits', pdfHeaders, pdfBody, `${exportFileName}.pdf`, companyName, appliedFilters);
  ...
  const loading = auditsExport && auditsExport.loading;
  ...
  return (
    <Row>
      <Col md="12" sm="12" lg="6">
        <div className="p-3 text-center">
          ...
          <Button
            ...
            color="dark"
            className="bg-zodiac"
            disabled={loading && opportunitiesCount && opportunitiesCount.length <= 0}
            onClick={() => {
              setExportType('pdf');
            }}
          >
      ...
      {auditsExport && auditsExport.err && (
        <Col md="12" sm="12" lg="12" xs="12">
          <SuccessAndErrorFormat response={auditsExport} />
        </Col>
      )}
      <Col md="12" sm="12" lg="12" className="d-none">
        <div id="print_report">
          ...
          <br />
          {exportType === 'excel' && (
            <DataTable columns={checkFieldExists(dataFields)} data={auditsExport && auditsExport.data ? auditsExport.data : []} propertyAsKey="id" />
          )}
        </div>
      </Col>
