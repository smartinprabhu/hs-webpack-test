...
import {
  ...
  getAllowedCompanies, queryGeneratorV1, queryGeneratorWithUtc,
  getDefaultNoValue, filterStringGeneratorDynamic, getCompanyTimezoneDate,
} from '../../../util/appUtils';
import { getResponseTypeLabel, getStateText } from '../../utils/utils';
...
const DataExport = (props) => {
  const {
    ...
    sortedValue, apiFields,
  } = props;
  const dispatch = useDispatch();
  const exportFileName = getExportFileName('Audit_Actions')
  const [columns, setColumns] = useState(fields);
  const [pdfBody, setPdfBody] = useState([]);
  const dataFields = tableFields.fieldsNC;
  ...
  const appliedFilters = filterStringGeneratorDynamic(nonConformitieFilters);
  ...
  useEffect(() => {
    if ((userInfo && userInfo.data) && (nonConformitiesCount && nonConformitiesCount.length)) {
      ...
      dispatch(getNonConformitieExport(companies, appModels.AUDITACTION, nonConformitiesCount.length, offsetValue, AuditSystemModule.actionApiFields, customFiltersQuery, rows, sortedValue.sortBy, sortedValue.sortField));
    }
  }, [userInfo, nonConformitiesCount, nonConformitieFilters]);
  ...
  function checkFieldExists(dataColumns) {
    ...
    if (columns.some((selectedValue) => selectedValue.includes('create_date'))) {
      dataColumnsList = dataColumnsList.concat(tableFields.createField);
    }
  ...
  useEffect(() => {
    if (fields && fields.length > 0 && nonConformitiesExport && nonConformitiesExport.data && nonConformitiesExport.data.length > 0) {
    ...
    if (pdfHeader && pdfHeader.__wrapped__ && pdfHeader.__wrapped__.length > 0) {
      ...
      if (nonConformitiesExport && nonConformitiesExport.data && nonConformitiesExport.data.length > 0) {
        nonConformitiesExport.data.map((data) => {
          ...
          data.date_deadline = getCompanyTimezoneDate(data.date_deadline, userInfo, 'date');
          data.state = getStateText(data.state);
          data.type_action = getResponseTypeLabel(data.type_action);
  ...
  useEffect(() => {
    if (exportType === 'pdf') {
      ...
      savePdfContent('Audit-Actions', pdfHeaders, pdfBody, `${exportFileName}.pdf`, companyName, appliedFilters);
  ...
  const loading = nonConformitiesExport && nonConformitiesExport.loading;
  ...
  return (
    <Row>
      <Col md="12" sm="12" lg="6">
        <div className="p-3 text-center">
          ...
          <Button
            ...
            color="dark"
            className="bg-zodiac"
            disabled={loading && nonConformitiesCount && nonConformitiesCount.length <= 0}
            onClick={() => {
              setExportType('pdf');
            }}
          >
      ...
      {nonConformitiesExport && nonConformitiesExport.err && (
        <Col md="12" sm="12" lg="12" xs="12">
          <SuccessAndErrorFormat response={nonConformitiesExport} />
        </Col>
      )}
      <Col md="12" sm="12" lg="12" className="d-none">
        <div id="print_report">
          ...
          <table align="center">
            <tbody>
              <tr><td style={{ textAlign: 'center' }} colSpan={5}><b>Audit-Actions Report</b></td></tr>
              <tr>
                <td>Company</td>
          ...
          <br />
          {exportType === 'excel' && (
            <DataTable columns={checkFieldExists(dataFields)} data={nonConformitiesExport && nonConformitiesExport.data ? nonConformitiesExport.data : []} propertyAsKey="id" />
          )}
        </div>
      </Col>
