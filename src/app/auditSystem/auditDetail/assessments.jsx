...
const Assessments = (props) => {
  ...
  useEffect(() => {
    if (detailData.id) {
      dispatch(getAuditAssessments(detailData.id, appModels.AUDITSURVEYINPUT));
    }
  }, [detailData]);

  const loading = auditAssessmentDetail && auditAssessmentDetail.loading;
  const isErr = auditAssessmentDetail && auditAssessmentDetail.err;
  const inspDeata = auditAssessmentDetail && auditAssessmentDetail.data && auditAssessmentDetail.data.length ? auditAssessmentDetail.data[0] : false;
  const isChecklist = (inspDeata && inspDeata.user_input_line_ids && inspDeata.user_input_line_ids.length > 0);

  function getAnswer(type, data) {
    let res = '';
    if (type === 'number') {
  ...
  function getRow(assetData, groupId) {
    ...
    for (let i = 0; i < assetData.length; i += 1) {
      if (assetData[i].page_id && assetData[i].page_id.id && assetData[i].page_id.id === gId) {
        tableTr.push(
          <tr key={i}>
            <td className="p-2">{getDefaultNoValue(extractNameObject(assetData[i].question_id, 'question'))}</td>
            <td className="p-2">{getDefaultNoValue(getAnswer(assetData[i].answer_type, assetData[i]))}</td>
            <td className="p-2">{numToFloatView(assetData[i].quizz_mark)}</td>
          </tr>,
        );
      }
    }
  ...
  const sections = isChecklist > 0 ? groupByMultiple(inspDeata.user_input_line_ids, (obj) => (obj.page_id && obj.page_id.id ? obj.page_id.id : '')) : [];
              ...
              <CardHeader id={`heading${index}`} className="p-2 bg-lightgrey border-0">
                ...
                >
                  <span className="collapse-heading font-weight-800">
                    {section && section[0].page_id && section[0].page_id.title ? section[0].page_id.title : 'General'}
                    {' '}
                  </span>
                  {accordion[index]
                ...
                <Row className="mr-2 ml-2 mb-0 products-list-tab">
                  ...
                  <>
                    <Table responsive className="mb-0 mt-2 font-weight-400 border-0 assets-table" width="100%">
                      ...
                      <tbody>
                        {getRow(
                          inspDeata.user_input_line_ids,
                          section && section[0].page_id && section[0].page_id.id ? section[0].page_id.id : '',
                        )}
                      </tbody>
                    </Table>
                    <hr className="m-0" />
