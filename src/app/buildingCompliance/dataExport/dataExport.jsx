...
import {
  ...
  getAllowedCompanies,getDefaultNoValue, queryGeneratorWithUtc
} from '../../util/appUtils';
...
const DataExport = (props) => {
  ...
  const exportFileName = getExportFileName('Building_Compliance')
  ...
  useEffect(() => {
    if ((userInfo && userInfo.data) && (complianceCount && complianceCount.length)) {
      ...
      dispatch(getComplianceExport(companies, appModels.BULIDINGCOMPLIANCE, complianceCount.length, offsetValue, BuildingComplianceModule.buildingApiFields, customFiltersList, rows, sortedValue.sortBy, sortedValue.sortField));
    }
  ...
  function checkFieldExists(dataColumns) {
    ...
    if (columns.some((selectedValue) => selectedValue.includes('compliance_category_id'))) {
      dataColumnsList = dataColumnsList.concat(tableFields.categoryFields);
    }
    if (columns.some((selectedValue) => selectedValue.includes('sla_status'))) {
      dataColumnsList = dataColumnsList.concat(tableFields.slaFields);
    }
    if (columns.some((selectedValue) => selectedValue.includes('applies_to'))) {
