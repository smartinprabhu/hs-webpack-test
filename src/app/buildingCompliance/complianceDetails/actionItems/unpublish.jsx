...
const Unpublish = (props) => {
  const {
    complianceDetails, unpublishModal, atFinish,
  } = props;
  const dispatch = useDispatch();
  const [modal, setModal] = useState(unpublishModal);
  const toggle = () => {
    setModal(!modal);
    atFinish();
  };
  ...
  return (
    <Dialog maxWidth="md" open={unpublishModal}>
      <DialogHeader title="Unpublish" onClose={toggle} fontAwesomeIcon={faTimesCircle} response={stateChangeInfo} />
      <DialogContent>
        <DialogContentText id="alert-dialog-description">
          ...
          >
            ...
            <Row className="justify-content-center">
              ...
              <SuccessAndErrorFormat response={stateChangeInfo} successMessage="This Compliance has been unpublish successfully.." />
              )}
        ...
        {stateChangeInfo && stateChangeInfo.data
          ...
          : (
            <Button
              ...
              onClick={() => handleStateChange(complianceData.id, 'action_set_to_unpublish')}
            >
              Move
            </Button>
          )}
...
Unpublish.propTypes = {
  ...
  unpublishModal: PropTypes.oneOfType([
    PropTypes.bool,
    PropTypes.string,
  ]).isRequired,
