...
const Archive = (props) => {
  const {
    complianceDetails, archiveModal, atFinish,
  } = props;
  const dispatch = useDispatch();
  const [modal, setModal] = useState(archiveModal);
  const toggle = () => {
    setModal(!modal);
    atFinish();
  };
  ...
  return (
    <Dialog maxWidth="md" open={archiveModal}>
      <DialogHeader title="Archive" onClose={toggle} fontAwesomeIcon={faCaretSquareDown} response={stateChangeInfo} />
      <DialogContent>
        <DialogContentText id="alert-dialog-description">
          ...
          >
            ...
            <Row className="justify-content-center">
              ...
              <SuccessAndErrorFormat response={stateChangeInfo} successMessage="This Compliance has been archive successfully.." />
              )}
        ...
        {stateChangeInfo && stateChangeInfo.data
          ...
          : (
            <Button
              ...
              onClick={() => handleStateChange(complianceData.id, 'action_set_to_archive')}
            >
              Move
            </Button>
          )}
...
Archive.propTypes = {
  ...
  archiveModal: PropTypes.oneOfType([
    PropTypes.bool,
    PropTypes.string,
  ]).isRequired,
