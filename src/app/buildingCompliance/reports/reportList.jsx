...
import SideFilterCompliance from './reportList/sidebar/sideFilterCompliance';

import {
  getAllowedCompanies, getCompanyTimezoneDate, getMenuItems,
} from '../../util/appUtils';
import ReportsCompliance from './reportList/reportCompliance';
  ...
  return (
    <Row className="pt-2">
      <Col md="12" sm="12" lg={collapse ? 1 : 3} xs="12">
        ...
        <Card className={collapse ? 'd-none all-reports' : 'all-reports p-1 bg-lightblue h-100'} onMouseOver={() => setFilterIcon(true)} onMouseLeave={() => setFilterIcon(false)}>
          ...
          <CardBody className="pt-1 pl-2">
            ...
            {selectedReport === 'Compliance Report' && (
              <SideFilterCompliance />
            )}
            <div />
          </CardBody>
        </Card>
      </Col>
      <Col md="12" sm="12" lg={collapse ? 11 : 9} xs="12" className={collapse ? 'filter-margin-left' : ''}>
        {selectedReport === '' && (
          <ReportsSelect collapse={collapse} />
        )}
        {(selectedReport === 'Compliance Report') && (
        <ReportsCompliance collapse={collapse} afterReset={() => { setSelectedReport(''); }} reportName={selectedReport} />
        )}
      </Col>
    </Row>
  );
