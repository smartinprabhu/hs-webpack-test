...
import {
  getComplianceDashboard, getComplianceFilters,
...
const Insights = () => {
  ...
  useEffect(() => {
    if (userInfo && userInfo.data && currentTab && currentTab !== 'Custom') {
      ...
      if (dates.length > 0) {
        const start = dates[0];
        const end = dates[1];
        dispatch(getComplianceDashboard(start, end));
      } else {
  ...
  const onLoadCompliance = (tags) => {
    if (tags) {
      setValue(tags);
      dispatch(getComplianceFilters(tags));
    }
  };

  const total = getTotal(complianceDashboard && complianceDashboard.data ? complianceDashboard.data : []);
  ...
  const loading = (userInfo && userInfo.loading) || (complianceDashboard && complianceDashboard.loading);
  ...
  const gridData = generateArrayFromValue(complianceDashboard && complianceDashboard.data ? complianceDashboard.data : [], 'ks_dashboard_item_type', 'ks_tile');
                  ...
                  <CardFooter className="bg-med-blue border-0 pt-0">
                    {actions.datasets && actions.datasets[0] !== 0 && actions.code ? (
                      <Button
                         variant="contained"
                        size="sm"
                        onClick={() => onLoadCompliance(complianceData.dashboardStatus[actions.code].tags)}
