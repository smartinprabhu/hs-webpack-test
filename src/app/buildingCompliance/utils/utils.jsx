...
import {
  filterDataPdf, customFilterDataArray, getPathName, truncate,
} from '../../util/appUtils';
...
export function getComplianceLocationNames(type, assetIds, cids, lids) {
  if (type === 'Location' && lids && lids.length) {
    return `Locations: ${getPathName(lids, 'path_name')}`;
  }
  if (type === 'Asset' && assetIds && assetIds.length) {
    return `Assets: ${getPathName(assetIds, 'name')}`;
  }
  if (type === 'Site' && cids && cids.length) {
    return `Sites: ${getPathName(cids, 'name')}`;
  }
  return '-';
}

export function getComplianceLocationNamesTable(type, assetIds, cids, lids) {
  if (type === 'Location' && lids && lids.length) {
    return (
      getPathName(lids, 'path_name').length > 35
        ? (
          <Tooltip title={`Locations: ${getPathName(lids, 'path_name')}`} placement="top">
            <span className="">{`Locations: ${truncate(getPathName(lids, 'path_name'), 35)}`}</span>
          </Tooltip>
        )
        : <span>{`Locations: ${getPathName(lids, 'path_name')}`}</span>
    );
  }
  if (type === 'Site' && cids && cids.length) {
    return (
      getPathName(cids, 'name').length > 35
        ? (
          <Tooltip title={`Sites: ${getPathName(cids, 'name')}`} placement="top">
            <span className="">{`Sites: ${truncate(getPathName(cids, 'name'), 35)}`}</span>
          </Tooltip>
        )
        : <span>{`Sites: ${getPathName(cids, 'name')}`}</span>
    );
  }
  if (type === 'Asset' && assetIds && assetIds.length) {
    return (
      getPathName(assetIds, 'name').length > 35
        ? (
          <Tooltip title={`Assets: ${getPathName(assetIds, 'name')}`} placement="top">
            <span className="">{`Assets: ${truncate(getPathName(assetIds, 'name'), 35)}`}</span>
          </Tooltip>
        )
        : <span>{`Assets: ${getPathName(assetIds, 'name')}`}</span>
    );
  }
  return '-';
}
...
export function getComplianceStateLabel(staten, isRenewed) {
  if (complianceActions && complianceActions.status[staten]) {
    return (
      <Badge
        color={isRenewed ? 'success' : complianceActions.status[staten].color}
        className="badge-text no-border-radius"
        pill
      >
        {isRenewed ? 'Renewed' : complianceActions.status[staten].label}
      </Badge>
    );
  }
  return '';
...
export function getComplianceStateLabelList(staten, param1, param2, isRenewed) {
  if (complianceActions && complianceActions.status[staten]) {
    return (
      <Badge
        color={isRenewed ? 'success' : complianceActions.status[staten].color}
...
/*export function getComplianceLocationNamesTable(staten) {
  if (staten && staten.length) {
    return (
      getPathName(staten, 'path_name').length > 35
        ? (
          <Tooltip title={getPathName(staten, 'path_name')} placement="top">
            <span className="">{truncate(getPathName(staten, 'path_name'), 35)}</span>
          </Tooltip>
        )
        : <span>{getPathName(staten, 'path_name')}</span>
    );
  }
  return '-';
}*/

export function getComplianceStateText(staten, isRenewed) {
  if (complianceActions && complianceActions.status[staten]) {
    return isRenewed ? 'Renewed' : complianceActions.status[staten].label;
  }
  return '';
}
