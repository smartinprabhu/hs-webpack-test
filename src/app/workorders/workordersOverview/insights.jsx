...
import workorderLogo from '@images/icons/ordersBlue.svg';
...
import {
  getWorkOrderDashboard, getWorkorderFilter,
...
const Insights = () => {
  ...
  useEffect(() => {
    if (userInfo && userInfo.data && currentTab && currentTab !== 'Custom') {
      ...
      if (dates.length > 0) {
        const start = dates[0];
        const end = dates[1];
        dispatch(getWorkOrderDashboard(start, end));
      } else {
  ...
  const onLoadWorkOrders = (tags, datefield) => {
    setValue(tags);
    const value = 'Custom';
    let filters = [{
      key: currentTab, value: currentTab, label: currentTab, type: 'customdatefield', datefield,
    }];
    ...
    const filterValues = {
      states: tags, teams: [], priorities: [], customFilters: filters,
    };
    dispatch(getWorkorderFilter(filterValues));
  };

  const total = getTotal(workorderDashboard && workorderDashboard.data ? workorderDashboard.data : []);
  ...
  const gridData = generateArrayFromValue(workorderDashboard && workorderDashboard.data ? workorderDashboard.data : [], 'ks_dashboard_item_type', 'ks_tile');
  ...
  return (
    <>
      <Row className="m-0 ml-1 pl-4 pt-2">
        <h6 className="mt-2">
          {' '}
          <img src={workorderLogo} alt="workorders" className="mr-2" height="20" width="20" />
          WORK ORDERS
        </h6>
      </Row>
      <Row className="m-0 pl-3 pt-0 pb-1">
        <>
          <Col sm={open ? '7' : '12'} md={open ? '7' : '12'} lg={open ? '7' : '12'} xs={open ? '7' : '12'} className="pl-0 pr-0">
            {workorderDashboard && !workorderDashboard.loading && (
              ...
              <CardFooter className="bg-med-blue border-0 pt-0">
                {actions.datasets && actions.datasets[0] !== 0 && actions.code ? (
                  <Button
                     variant="contained"
                    size="sm"
                    onClick={() => onLoadWorkOrders(workorderActionData.dashboardStatus[actions.code].tags, workorderActionData.dashboardStatus[actions.code].datefield)}
