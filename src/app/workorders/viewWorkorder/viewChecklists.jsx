...
import { getOperationCheckListData } from '../workorderService';
import {
  getLocalTime, getDefaultNoValue, generateErrorMessage, getCompanyTimezoneDate,
} from '../../util/appUtils';
import { groupByMultiple } from '../../util/staticFunctions';
import { getQuestionTypeLabel } from '../../survey/utils/utils';

const appModels = require('../../util/appModels').default;

const ViewChecklists = ({ ppmData }) => {
  ...
  const isChecklistJson = (workorder && workorder.checklist_json_data !== '[]' && workorder.checklist_json_data !== '' && workorder.checklist_json_data);
  ...
  useEffect(() => {
    if (workorder && workorder.task_id && workorder.task_id.length) {
  ...
  function getOperationRow(assetData, groupId) {
    ...
    for (let i = 0; i < assetData.length; i += 1) {
      if (assetData[i].mro_quest_grp_id.id === groupId) {
        tableTr.push(
          <tr key={i}>
            <td className="p-2">{getDefaultNoValue(assetData[i].mro_activity_id ? assetData[i].mro_activity_id.name : '')}</td>
            <td className="p-2">{getDefaultNoValue(getQuestionTypeLabel(assetData[i].answer_type))}</td>
          </tr>,
        );
      }
    }
  ...
  return (
    <Row>
      <Col sm="12" md="12" lg="12" xs="12">
        ...
        {(isChecklistJson && cheklistJsonObj && !isChecklist) && (
          <div className="ml-0 bg-white">
        ...
        {checklistOpInfo && checklistOpInfo.loading && (
          <Loader />
        )}
        {(checklistOpInfo && checklistOpInfo.err) && (
          <ErrorContent errorTxt={generateErrorMessage(checklistOpInfo)} />
        )}
      </Col>
    </Row>
  );
