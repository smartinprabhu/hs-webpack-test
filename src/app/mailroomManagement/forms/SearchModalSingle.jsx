...
import AddCourierComponent from '../addCourier';
import { resetCourierData } from '../mailService';
...
const SearchModalSingle = (props) => {
  ...
  const onModalClose = () => {
    setModalOpen(false);
    if (addCourier && (addCourier.data || addCourier.err)) {
      dispatch(resetCourierData());
      setLoadData(true);
    }
  };
  ...
  useEffect(() => {
    if (modelName && fields) {
      let searchValueMultiple = `[["company_id","in",[${company}]],["name","!=",false]`;
      if (searchValue) {
  ...
  }, [modelName, offset, isSearch, loadData]);
      ...
      <Col sm="12" md="12" lg="12" xs="12">
        <Card className="p-2 bg-lightblue h-100">
          <CardBody className="bg-color-white p-1 m-0">
            ...
            {fieldName === 'courier_id' && (
            <Dialog size="lg" fullWidth open={modalOpen}>
              <DialogHeader title="Add Courier" imagePath={false} onClose={() => { onModalClose(); }} />
              <DialogContent>
