...
import InboundBasicDetails from './inboundBasicDetails/SchedulerBasicDetails';
...
const { TabPane } = Tabs;
const InboundDetailTabs = (props) => {
  const { detailData } = props;
  const [currentTab, setActive] = useState('Inbound Mail Overview');

  useEffect(() => {
    setActive('Inbound Mail Overview');
  }, [detailData]);
        ...
        <Row>
          <Col md={12} sm={12} xs={12} lg={12} className="pl-1 pr-1">
            ...
            <div className="tab-content-scroll hidden-scrollbar">
              {currentTab === 'Inbound Mail Overview'
                ? <InboundBasicDetails detailData={detailData} />
                : ''}
              {currentTab === 'History'
...
export default InboundDetailTabs;
