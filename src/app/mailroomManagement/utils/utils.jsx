...
export function getMailRoomLabel(staten) {
  if (mailActions && mailActions.stateTypesText[staten]) {
    return <Badge
     color={mailActions.stateTypesText[staten].color}
    className={`${mailActions.stateTypesText[staten].badgeClassName} badge-text no-border-radius`}
    pill>{mailActions.stateTypesText[staten].label}
    </Badge>;
  }
  return '';
}
