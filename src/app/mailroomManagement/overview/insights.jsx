...
import {
  geMailDashboard, getInBoundFilters, getOutboundFilters,
} from '../mailService';
import {
  truncate, generateErrorMessage,
  generateArrayFromValue, 
  getDatesOfQueryWithUtc, getDateAndTimeForDifferentTimeZones, defaultTimeZone
} from '../../util/appUtils';
...
const Insights = () => {
  ...
  const dashboardName = 'Mail Room Management';

  const { userInfo } = useSelector((state) => state.user);
  const { mailDashboard } = useSelector((state) => state.mailroom);
  ...
  useEffect(() => {
    if (userInfo && userInfo.data && currentTab && currentTab !== 'Custom') {
      ...
      if (dates.length > 0) {
        const start = dates[0];
        const end = dates[1];
        dispatch(geMailDashboard(start, end, dashboardName));
      } else {
  ...
  const onLoadMails = (data) => {
    if (data) {
      ...
      if (data.type === 'inbound') {
        dispatch(setCurrentTab('Inbound'));
        dispatch(getInBoundFilters(customFiltersList));
      } else {
        dispatch(setCurrentTab('Outbound'));
        dispatch(getOutboundFilters(customFiltersList));
      }
    }
  };

  const total = getTotal(mailDashboard && mailDashboard.data ? mailDashboard.data : []);

  const gridData = generateArrayFromValue(mailDashboard && mailDashboard.data ? mailDashboard.data : [], 'ks_dashboard_item_type', 'ks_tile');
  const gridCards = gridData.sort((a, b) => a.sequence - b.sequence);

  const isUserError = userInfo && userInfo.err;
  const isUserLoading = userInfo && userInfo.loading;
  const userErrorMsg = generateErrorMessage(userInfo);
  const errorMsg = (mailDashboard && mailDashboard.err) ? generateErrorMessage(mailDashboard) : userErrorMsg;

  if (values.length > 0) {
    return (<Redirect to="/mailroom/operations" />);
  }

  return (
    <>
      <Row className="m-0 ml-1 pl-4 pt-2 mails-title">
        <h6 className="mt-2">
          {' '}
              ...
              <CardFooter className="bg-med-blue border-0 pt-0">
                {actions.datasets && actions.datasets.length && actions.datasets.length > 0 && actions.datasets[0] !== 0 && actions.code ? (
                  <Button
                     variant="contained"
                    size="sm"
                    onClick={() => onLoadMails(customData.dashboardStatus[actions.code] ? customData.dashboardStatus[actions.code] : false)}
      ...
      {((mailDashboard && mailDashboard.loading) || (isUserLoading)) && (
        <div className="mb-4 mt-2">
          <Loader />
        </div>
      )}
      {((mailDashboard && mailDashboard.err) || (isUserError)) && (
