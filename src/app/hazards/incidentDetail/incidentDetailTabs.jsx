...
const IncidentDetailTabs = (props) => {
    ...
    if (detail) {
      if (detail.state === 'Reported' || detail.state === 'Acknowledged' || detail.state === 'Work in Progress') {
        ...
        if (detail.state === 'Reported' && configData.is_acknowledge_required) {
          setTabsList(tabs.tabsList.filter((item) => item.name !== 'Analysis' && item.name !== 'Recommendations' && item.name !== 'Validation'));
        }
        if (detail.state === 'Reported' && !configData.is_analyzed_required && !configData.is_acknowledge_required) {
          setTabsList(tabs.tabsList.filter((item) => item.name !== 'Analysis' && item.name !== 'Validation'));
        }
        if (detail.state === 'Acknowledged' && !configData.is_analyzed_required) {
          setTabsList(tabs.tabsList.filter((item) => item.name !== 'Analysis' && item.name !== 'Validation'));
        }
      ...
      } else if ((detail.state === 'Validated' || detail.state === 'Signed off' || detail.state === 'Paused' || detail.state === 'Cancelled') && !configData.is_analyzed_required) {
        setTabsList(tabs.tabsList.filter((item) => item.name !== 'Analysis'));
  ...
  useEffect(() => {
    if (detail) {
      if ((detail.state === 'Reported' && !configData.is_acknowledge_required && configData.is_analyzed_required) || (detail.state === 'Acknowledged' && configData.is_analyzed_required)) {
        setActive('Analysis');
      } else if (detail.state === 'Analyzed' || detail.state === 'Remediated' || (detail.state === 'Acknowledged' && !configData.is_analyzed_required) || (detail.state === 'Reported' && !configData.is_acknowledge_required && !configData.is_analyzed_required)) {
              ...
              {currentTab === 'Analysis'
                ? (
                  <>
                    ...
                    {configData.is_analyzed_required && (
                    <>
