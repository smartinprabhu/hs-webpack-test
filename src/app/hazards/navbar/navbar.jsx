...
const Navbar = (props) => {
  ...
  const [currentTab, setActive] = useState('Hazards');
  ...
  useEffect(() => {
    if (complianceList) {
      ...
      history.push({ pathname: '/ehs-hazards', state: { id: complianceList.id } });
    }
  }, [complianceList]);

  useEffect(() => {
    if (searchOpen) {
      dispatch(getIncidentsList(companies, appModels.EHSHAZARD, limit, offset, false, 'DESC', 'create_date', keyword));
      dispatch(getIncidentsCount(companies, appModels.EHSHAZARD, [], keyword));
      setTotalCount(0);
    }
  ...
  useEffect(() => {
    if (addIncidentInfo && addIncidentInfo.data && addIncidentInfo.data.length) {
      dispatch(getIncidentDetail(addIncidentInfo.data[0], appModels.EHSHAZARD));
    }
  }, [addIncidentInfo]);
              ...
              <Col md="6" sm="12" lg="6" className="pr-4">
                ...
                <Popover trigger="legacy" className="search-popover" placement="bottom" isOpen={searchOpen} target="asset-search" toggle={() => setSearchOpen(!searchOpen)}>
                  <PopoverBody>
                    <Row>
                      <Nav>
                        <NavLink className="insights-nav-link" onClick={() => setActive('Hazards')} href="#">Hazards</NavLink>
                      </Nav>
                    </Row>
