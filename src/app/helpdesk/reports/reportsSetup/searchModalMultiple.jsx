...
const SearchModalMultiple = (props) => {
  ...
  useEffect(() => {
    if (modelName && fields) {
      ...
      if (fieldName === 'asset_ids') {
        searchValueMultiple = `${searchValueMultiple},["is_itasset","!=",true]`;
        if (searchValue) {
          searchValueMultiple = `${searchValueMultiple},"|","|","|",["name", "ilike", "${searchValue}"],["serial", "ilike", "${searchValue}"],["brand", "ilike", "${searchValue}"],["location_id", "ilike", "${searchValue}"]`;
        }
      }
  ...
  useEffect(() => {
    if (modelName) {
      ...
      searchValueMultiple = `${searchValueMultiple}]`;
      dispatch(getExtraSelectionMultipleCount(company, modelName, fields, searchValueMultiple, fieldName === 'asset_ids' ? 'search' : false));
    }
  }, [modelName, isSearch]);
  ...
  const handleTableCellAllChange = (event) => {
    ...
    let data = [];
    if (checked) {
      if (fieldName === 'vendor_id' || fieldName === 'asset_ids') {
        ...
        <tr key={i}>
          ...
          {fieldName === 'asset_ids' && (
            <>
              ...
              <td className="p-2"><span className="font-weight-400">{getDefaultNoValue((assetData[i].serial))}</span></td>
