...
import {
  getHelpdeskFilter, resetUpdateTicket, getMaintenanceConfigurationData, getExtraSelectionMultiple, resetImage, resetAddTicket, getExtraSelectionMultipleCount, activeStepInfo,
} from '../ticketService';
...
const Navbar = (props) => {
  ...
  const title = getModuleDisplayName(userRoles && userRoles.data ? userRoles.data.allowed_modules : [], 'Helpdesk', 'display');
  const {
  ...
  useEffect(() => {
    if (ticket) {
      ...
      history.push({ pathname: '/helpdesk/tickets', state: { id: ticket.id } });
    }
  }, [ticket]);
  ...
  return (
    <>
      <Row className="m-0 helpDesk-header">
        <Col md="3" sm="3" lg="3" xs="12" className="p-0">
          <h3 className="mt-1  d-flex align-items-center">
            <img src={helpdesk} alt="actions" className="mr-3 mb-1" width="30" height="30" />
            ...
            <Row className="margin-negative-left-90px margin-negative-right-55px">
              ...
              <Col sm="12" lg="4" md="4">
                {allowedOperations.includes(actionCodes['Raise a ticket']) && (
                  <Button className="float-right insights-add-icon mt-2" onClick={() => { dispatch(resetUpdateTicket()); showAddTicketModal(!addTicketModal) }}>
                    <img
                      aria-hidden="true"
                      id="Add"
                      alt="Add"
      ...
      >
        ...
        />
        <AddTicket editLink={addTicketModal} randomProp={false} editIds={false} closeModal={() => showAddTicketModal(false)} afterReset={onAddReset} isDrawer />
      </Drawer>
