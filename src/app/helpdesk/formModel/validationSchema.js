...
export default [
  Yup.object().shape({
    ...
    [issueType.name]: Yup.string()
      .nullable()
      .required(`${issueType.requiredErrorMsg}`),
    ...
    [Channel.name]: Yup.string().when('has_channel', {
      is: 'Required',
      then: Yup.string().required(`${Channel.requiredErrorMsg}`),
      otherwise: Yup.string(),
    }),
    [ticketType.name]: Yup.string().when('has_ticket_type', {
      is: 'Required',
      then: Yup.string().required(`${ticketType.requiredErrorMsg}`),
      otherwise: Yup.string(),
    }),
    [maintenanceTeamId.name]: Yup.string().when('has_team', {
      is: 'Required',
      then: Yup.string().required(`${maintenanceTeamId.requiredErrorMsg}`),
      otherwise: Yup.string(),
    }),
    [tenantName.name]: Yup.string().when('has_tenant', {
      is: 'Required',
      then: Yup.string().required(`${tenantName.requiredErrorMsg}`),
      otherwise: Yup.string(),
    }),
  }),
];
