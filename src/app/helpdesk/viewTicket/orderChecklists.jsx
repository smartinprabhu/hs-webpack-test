...
const OrderChecklists = () => {
  ...
  const inspDeata = incidentsOrderInfo && incidentsOrderInfo.data && incidentsOrderInfo.data.data
    && incidentsOrderInfo.data.data.length ? incidentsOrderInfo.data.data[0] : false;

  // const isChecklist = (inspDeata && inspDeata.check_list_ids && inspDeata.check_list_ids.length > 0);

  const isChecklist = inspDeata && inspDeata.check_list_ids && inspDeata.check_list_ids.length > 0;
  const isChecklistJson = !isChecklist ? (inspDeata && inspDeata.checklist_json_data && inspDeata.checklist_json_data !== '[]' && inspDeata.checklist_json_data !== '') : false;

  const cheklistJsonObj = isChecklistJson && inspDeata && inspDeata.checklist_json_data ? JSON.parse(inspDeata.checklist_json_data) : false;
    ...
    <>
      <Row className="assessment-card">
        ...
        <Col sm="12" md="12" lg="12" xs="12" className="p-3 comments-list thin-scrollbar">
          ...
          {(!loading && checklistOpInfo && !isChecklist && !isChecklistJson) && (
            <div className="ml-0 bg-white">
          ...
          {!isErr && !inspDeata && !loading && !isChecklist && !isChecklistJson && !checklistOpInfo && (
            <ErrorContent errorTxt="No Data Found" />
          )}
        </Col>
      </Row>
...
export default OrderChecklists;
