...
import {
  getIncidentInjuries, resetUpdateTicket, updateTicket,
} from '../ticketService';
import {
...
import { checkInjuries, getInjuriyTypeLabel } from '../utils/utils';
...
const Injuries = () => {
  ...
  const { ticketDetail, incidentInjuries, updateTicketInfo } = useSelector((state) => state.ticket);
  ...
  useEffect(() => {
    if (incidentInjuries && incidentInjuries.data) {
      setEditData(incidentInjuries.data);
    }
  }, [incidentInjuries]);

  useEffect(() => {
    if (ticketDetail && ticketDetail.data) {
      const ids = ticketDetail.data.length > 0 ? ticketDetail.data[0].injuries_sustained_ids : [];
      dispatch(getIncidentInjuries(companies, ids, appModels.INJURIES));
    }
  }, [ticketDetail]);
  ...
  return (
    <>
      <Row className=" injuries-card">
        <Col sm="12" md="12" lg="12" xs="12" className="p-3 bg-white comments-list thin-scrollbar injuries-card ">
          <div className="font-weight-800 float-right mb-1">
            {!dataEdit && (
              <>
                ...
                {incidentInjuries && incidentInjuries.data && incidentInjuries.data.length > 0 && !dataAdd && (
                  <Tooltip title="Edit">
          ...
          <div>
            <Table responsive className="mb-0 mt-2 font-weight-400 border-0 assets-table " width="100%">
              ...
              <tbody>
                {!dataEdit
                  ? getRow(incidentInjuries && incidentInjuries.data ? incidentInjuries.data : [])
          ...
          <DetailViewFormat detailResponse={incidentInjuries} />
