...
import {
  ...
  getAllowedCompanies, filterStringGeneratorDynamic, getDefaultNoValue,
} from '../../util/appUtils';
...
const DataExport = (props) => {
  const {
    afterReset, fields, sortedValue, apiFields,
  } = props;
  const dispatch = useDispatch();
  const exportFileName = getExportFileName('Survey')
  ...
  const {
    surveyCount, surveyFilters, surveyExport, surveyRows,
  ...
  useEffect(() => {
    if ((userInfo && userInfo.data) && (surveyCount && surveyCount.length)) {
      const offsetValue = 0;
      const customFiltersList = surveyFilters.customFilters ? queryGeneratorV1(surveyFilters.customFilters) : '';
      const rows = surveyRows.rows ? surveyRows.rows : [];
      dispatch(getSurveyListExport(companies, appModels.SURVEY, surveyCount.length, offsetValue, SurveyModule.surveyApiField, customFiltersList, rows, sortedValue.sortBy, sortedValue.sortField));
    }
  }, [userInfo, surveyCount]);
  ...
  const appliedFilters = filterStringGeneratorDynamic(surveyFilters);

  useEffect(() => {
    if (fields && fields.length > 0 && surveyExport && surveyExport.data && surveyExport.data.length > 0) {
  ...
  const getNewVisitArray = (array) => {
    ...
    if (array && array.length > 0) {
      for (let i = 0; i < array.length; i += 1) {
        const val = array[i];
        val.category_type = val.category_type ? getTypeLabel(val.category_type) : '-';
        val.title = val.title ? val.title : '-';
        val.uuid = val.uuid ? `${WEBAPPAPIURL}${'survey'}/${val.uuid}` : '-';
        val.stage_id = val.stage_id && val.stage_id.length ? val.stage_id[1] : '-';
        val.user_input_ids = val.user_input_ids && val.user_input_ids.length > 0 ? val.user_input_ids.length : '0';
        val.tot_sent_survey = val.tot_sent_survey ? val.tot_sent_survey : '0';
        val.tot_start_survey = val.tot_start_survey ? val.tot_start_survey : '0';
        val.tot_comp_survey = val.tot_comp_survey ? val.tot_comp_survey : '0';
        resources.push(val);
      }
    }
    return resources;
  ...
  useEffect(() => {
    if (pdfHeader && pdfHeader.__wrapped__ && pdfHeader.__wrapped__.length > 0) {
      ...
      const exportData = surveyExport && surveyExport.data && surveyExport.data.length > 0 ? getNewVisitArray(surveyExport.data) : [];
  ...
  useEffect(() => {
    if (exportType === 'pdf') {
      ...
      savePdfContent('Survey', pdfHeaders, pdfBody, `${exportFileName}.pdf`, companyName, appliedFilters);
  ...
  return (
    <Row>
      <Col md="12" sm="12" lg="6">
        <div className="p-3 text-center">
          ...
          <Button
            ...
            color="dark"
            className="bg-zodiac"
            disabled={surveyExport && surveyExport.loading}
            onClick={() => {
              setExportType('pdf');
            }}
          >
            {(surveyExport && surveyExport.loading) ? (
      ...
      {surveyExport && surveyExport.err && (
        <Col md="12" sm="12" lg="12" xs="12">
          <SuccessAndErrorFormat response={surveyExport} />
        </Col>
      )}
      <Col md="12" sm="12" lg="12" className="d-none">
        <div id="print_report">
          ...
          <table align="center">
            <tbody>
              <tr><td style={{ textAlign: 'center' }} colSpan={5}><b>Survey Report</b></td></tr>
              <tr>
                <td>Company</td>
          ...
          <br />
          {exportType === 'excel' && (
            <DataTable columns={dataFields} data={surveyExport && surveyExport.data ? surveyExport.data : []} propertyAsKey="id" />
          )}
        </div>
      </Col>
