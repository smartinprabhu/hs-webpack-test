...
const SearchModal = (props) => {
  ...
  const handleRowClick = (data) => {
    ...
    if (fieldName === 'employee_id') {
      items = {
        id: data.id, name: data.name, work_phone: data.work_phone, work_email: data.work_email,
      };
    ...
    for (let i = 0; i < assetData.length; i += 1) {
      tableTr.push(
        <tr className={selectedId === assetData[i].id ? 'text-info cursor-pointer' : 'cursor-pointer'} onClick={() => handleRowClick(assetData[i])} key={i}>
          {(fieldName === 'space_id' || fieldName === 'location_id')
            ? (
              <>
                ...
                <td className="p-2"><span className="font-weight-400">{getDefaultNoValue(assetData[i].sequence_asset_hierarchy)}</span></td>
              </>
            )
            : <td className="p-2"><span className="font-weight-400">{fieldName === 'parent_id' ? getDefaultNoValue(assetData[i].display_name) : getDefaultNoValue(assetData[i].name)}</span></td>}
        </tr>,
      );
    }
    return tableTr;
