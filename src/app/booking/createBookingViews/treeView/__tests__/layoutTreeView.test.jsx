...
const treeBookingType = 'individual';
...
describe('Layout tree snapshots', () => {
  it('layout tree renders without crashing for individual booking', () => {
    ...
    const { asFragment } = render(<LayoutTreeView data={data} workSpaceSelect={workSpaceSelect} BookingData={BookingData} treeBookingType={treeBookingType} />);
    expect(asFragment()).toMatchSnapshot();
  });

  it('layout tree renders without crashing for group booking', () => {
    useSelector.mockImplementation((selectorFn) => selectorFn(mockedStore));
    Select.mockImplementation(
      ({ options, value, onChange }) => {
        const workstation = {
    ...
    const { asFragment } = render(<LayoutTreeView data={data} workSpaceSelect={workSpaceSelect} BookingData={BookingData} treeBookingType="group" />);
    expect(asFragment()).toMatchSnapshot();
  });

  it('dom testing for LayoutTreeView', async () => {
    useSelector.mockImplementation((selectorFn) => selectorFn(mockedStore));
    Select.mockImplementation(
      ({ options, value, onChange }) => {
        const workstation = {
    ...
    const {
      ...
      <LayoutTreeView data={data} workSpaceSelect={workSpaceSelect} BookingData={BookingData} treeBookingType="group" />,
    );
  ...
  it('renders recursive tree view,', async () => {
    useSelector.mockImplementation((selectorFn) => selectorFn(mockedStore));
    const removeNode = jest.fn();
    const setSelectedSpaceView = jest.fn();

    const { container } = render(<RecursiveTreeViewLayout
  ...
  it('renders recursive tree view for group,', async () => {
    useSelector.mockImplementation((selectorFn) => selectorFn(mockedStore));
    const removeNode = jest.fn();
    const setSelectedSpaceView = jest.fn();
