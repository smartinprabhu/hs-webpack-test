...
const LogNotes = () => {
  ...
  const loading = ctDetailsInfo && ctDetailsInfo.loading;
  const isErr = ctDetailsInfo && ctDetailsInfo.err;
  const inspDeata = ctDetailsInfo && ctDetailsInfo.data && ctDetailsInfo.data.length ? ctDetailsInfo.data[0] : false;
        ...
        {isChecklist && (
          <div>
            <Table responsive className="mb-0 mt-2 font-weight-400 border-0 assets-table" width="100%">
              ...
              <tbody>
                {getRow(ctDetailsInfo.data.length > 0 ? ctDetailsInfo.data[0].status_logs : [])}
              </tbody>
            </Table>
            <hr className="m-0" />
          </div>
        )}
        <DetailViewFormat detailResponse={ctDetailsInfo} />
