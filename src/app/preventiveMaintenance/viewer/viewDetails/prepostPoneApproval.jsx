...
const PrepostPoneApproval = (props) => {
  ...
  const handleApproveChange = async (id) => {
    if (pendingDetail && pendingDetail.id) {
      ...
      const postDataValues = {
        prepone_postpone_approval_ids: [[1, pendingDetail.id, payload]],
        // is_pending_for_approval: false,
        // starts_on: getJsonData(pendingDetail.data, 'proposed_start_date'),
        // ends_on: getJsonData(pendingDetail.data, 'proposed_end_date'),
        // is_rescheduled: true,
      };
  ...
  return (
    ...
    <Dialog PaperProps={{ style: { width: '600px', maxWidth: '600px' } }} open={actionModal}>
      <DialogHeader title="Approve/Reject Prepone/Postpone Request" onClose={toggleCancel} response={updateProductCategoryInfo} />
      <DialogContent>
        <DialogContentText id="alert-dialog-description">
          ...
          >
            ...
            <Row className="justify-content-center font-family-tab">
              {updateProductCategoryInfo && updateProductCategoryInfo.data && !loading && (
                <SuccessAndErrorFormat response={updateProductCategoryInfo} successMessage={type === 'cancel' ? 'The PPM Prepone/Postpone Request has been rejected.' : 'The PPM has been rescheduled successfully..'} />
              )}
