...
function preventiveCheckListSuccess() {
  const { addPreventiveChecklist } = useSelector((state) => state.ppm);
  return (
    <>
      {addPreventiveChecklist && addPreventiveChecklist.loading && (
      ...
      {addPreventiveChecklist && addPreventiveChecklist.data && (
        <h5 className="text-center text-success" data-testid="success-case">
          ...
          {' '}
          {' '}
          The PPM checklist has been created successfully.
        </h5>
      )}
      {addPreventiveChecklist && addPreventiveChecklist.err && (
        <ErrorContent errorTxt={generateErrorMessage(addPreventiveChecklist)} />
      )}
    </>
  );
}
