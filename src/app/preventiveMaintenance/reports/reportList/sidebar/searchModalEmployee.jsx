...
const SearchModalEmployee = (props) => {
  const {
    modelName,
    fields,
    company,
  ...
  const [checkedRows, setCheckRows] = useState(oldEmpValues);
  ...
  useEffect(() => {
    if (modelName && fields) {
      ...
      if (searchValue && fieldName === 'employee') {
        searchValueMultiple = `${searchValueMultiple},"|","|",["name","ilike","${searchValue}"],["work_email","ilike","${searchValue}"],["mobile_phone","ilike","${searchValue}"]`;
      }
    ...
    for (let i = 0; i < assetData.length; i += 1) {
      tableTr.push(
        <tr key={i}>
          ...
          <td className="p-2"><span className="font-weight-400">{getDefaultNoValue(assetData[i].mobile_phone)}</span></td>
        </tr>,
      );
    }
    return tableTr;
...
SearchModalEmployee.propTypes = {
  ...
  oldEmpValues: PropTypes.array.isRequired,
  afterReset: PropTypes.func.isRequired,
  onEmployeeChange: PropTypes.func.isRequired,
};

export default SearchModalEmployee;
