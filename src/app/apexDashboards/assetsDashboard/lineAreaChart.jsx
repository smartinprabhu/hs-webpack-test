...
const LineAreaChart = React.memo((props) => {
  const {
    chartData, height, code, selectedDateTag, width, dashboardColors,
  } = props;
  ...
  useEffect(() => {
    if (showModal && !chartId) {
      setDatasets(getDatasetsLine(chartValues.datasets, chartValues.labels, getChartColors(chartData.id), chartData.ks_dashboard_item_type, chartValues.subcolors));
  ...
  const onExpandChart = () => {
    // console.log(cd);
    // if (cd) {
    let cData = getDatasetsLine(chartValues.datasets, chartValues.labels, getChartColors(chartData.id), chartData.ks_dashboard_item_type, chartValues.subcolors);
              ...
              {!(updateLoading && isUpdateLoad) && (
                <div width="100%" height="100%" className="pr-3 pl-3" id="rechartContainer" style={{ height: '100%', width: '100%' }}>
                  <Plot
                    ...
                    data={getDatasetsLine(chartValues.datasets, chartValues.labels, getChartColors(chartData.id), chartData.ks_dashboard_item_type, chartValues.subcolors)}
