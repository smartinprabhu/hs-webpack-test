...
import { getPagesCountV2, getColumnArrayById, debounce } from '../../util/appUtils';
...
const HorizontalBarChart = React.memo((props) => {
  const {
    chartData, height, width, dashboardColors,
  } = props;
  ...
  useEffect(() => {
    if (chartId && domainData) {
      dispatch(getNinjaDashboardItem('ks_fetch_drill_down_data', appModels.NINJABOARDITEM, chartId, domainData, chartSequence));
    }
  }, [domainData]);
      ...
      dispatch(
        getExtraSelectionMultipleCount(
          ...
          searchValue && searchValue !== ''
            ? encodeURIComponent(getDomainString(JSON.stringify(domainData), searchValue, modelName)) : getDomainString(encodeURIComponent(JSON.stringify(domainData)), searchValue, modelName),
        ),
      );
    }
  ...
  const onReset = () => {
    if ((chartSequence > 0) && (chartData.max_sequnce && chartData.max_sequnce > 0)) {
      if (isViewList) {
        setViewList(false);
      }
      setSequence(chartSequence - 1);
  ...
  const onTitleFullReset = () => {
    ...
    setPage(1);
    if (showModal) {
      setDatasets(getDatasetsHBar(chartValues.datasets, chartValues.labels, getChartColors(chartData.id), chartValues.subcolors));
      setDataLayout(chartValues.datasets.length > 1 ? { ...layoutExpand, showlegend: true } : layoutExpand);
    }
  };
  ...
  const onExpandChart = () => {
    // console.log(cd);
    // if (cd) {
    let cData = getDatasetsHBar(chartValues.datasets, chartValues.labels, getChartColors(chartData.id), chartValues.subcolors);
    let cLayout = chartValues.datasets.length > 1 ? { ...layoutExpand, showlegend: true } : layoutExpand;
    if (chartId) {
          ...
          {!chartId && (
            <>
              { /* <StaticDataExport
                ...
                chartItems={false}
          /> */ }
              <div width="100%" height="100%" id="rechartContainer" className="pr-3 pl-3" style={{ height: '100%', width: '100%' }}>
                <Plot
                  key={chartId}
                  layout={{
                    ...layout,
                  ...
                  data={getDatasetsHBar(chartValues.datasets, chartValues.labels, getChartColors(chartData.id), chartValues.subcolors)}
