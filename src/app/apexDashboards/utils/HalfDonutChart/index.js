import React, {
  useCallback, useEffect, useRef, useLayoutEffect,
} from 'react';
import {
  arc,
  pie,
...
const GaugeChart = (props) => {
  ...
  const initChart = useCallback(
    (update, resize = false, prevProps) => {
      ...
      threshold.current = g.current.append('g').attr('class', 'threshold');
...
const renderChart = (
  ...
  threshold.current.selectAll('*').remove();
  ...
  const { percent, hideText, thresholdConfig } = props;
  if (!hideText) {
  ...
  if (thresholdConfig) {
    drawThreshold(
      props,
      outerRadius.current,
      outerRadius.current * (1 - props.arcWidth),
      threshold,
      g,
      width,
    );
...
const drawNeedle = (
  ...
  const {
    percent, needleColor, needleBaseColor, hideText, animate,
  } = props;
  const needleRadius = 15 * (width.current / 500); // Make the needle radius responsive
  const centerPoint = [0, -needleRadius / 2];
  // Draw the triangle
  ...
  const pathStr = calculateRotation(prevPercent || percent, outerRadius, width);
...
const calculateRotation = (percent, outerRadius, width) => {
  const needleLength = outerRadius.current * 0.55; // TODO: Maybe it should be specified as a percentage of the arc radius?
...
const percentToRad = (percent) => percent * Math.PI;
...
const addText = (percentage, props, outerRadius, width, g) => {
  const { formatTextValue, fontSize, showScaleValues } = props;
  const textPadding = 0;
  ...
  if (showScaleValues) {
    g.current
      .append('g')
      .attr('class', 'text-group')
      .attr('transform', `translate(-4, ${outerRadius.current})`)
      .append('text')
      .text('0%')
      // this computation avoid text overflow. When formatted value is over 10 characters, we should reduce font size
      .style(
        'font-size',
        `${width.current / 18 / (text.length > 10 ? text.length / 10 : 1)}px`,
      )
      .style('fill', props.textColor)
    ...
    g.current
      ...
      .attr(
        'transform',
        `translate(${outerRadius.current * 2 + 4}, ${outerRadius.current})`,
      )
      .append('text')
      .text('100%')
      // this computation avoid text overflow. When formatted value is over 10 characters, we should reduce font size
...
const drawThreshold = (props, outerRadius, innerRadius, threshold, g, width) => {
  const { formatTextValue, fontSize, thresholdConfig } = props;
  const text = formatTextValue
    ? formatTextValue(floatingNumber(thresholdConfig.value))
    : `${floatingNumber(thresholdConfig.value)}%`;
  const theta = percentToRad(thresholdConfig.value);

  const innerPoint = [
    outerRadius - innerRadius * Math.cos(theta),
    outerRadius - innerRadius * Math.sin(theta),
  ];
  const outerPoint = [
    outerRadius - outerRadius * Math.cos(theta),
    outerRadius - outerRadius * Math.sin(theta),
  ];

  const pathStr = `M ${innerPoint[0]} ${innerPoint[1]} L ${outerPoint[0]} ${outerPoint[1]}`;
  threshold.current
    .append('path')
    .attr('d', pathStr)
    .attr('stroke', thresholdConfig.color)
    .attr('stroke-width', 2);

  g.current
    .append('g')
    .attr('class', 'text-group')
    .attr('transform', `translate(${outerPoint[0]}, ${outerPoint[1]})`)
    .append('text')
    .text(text)
    // this computation avoid text overflow. When formatted value is over 10 characters, we should reduce font size
    .style('font-size', () => fontSize || `${width.current / 18 / (text.length > 10 ? text.length / 10 : 1)}px`)
    .style('fill', props.textColor)
    .style('text-anchor', 'end');
};

const floatingNumber = (value, maxDigits = 2) => Math.round(value * 100 * 10 ** maxDigits) / 10 ** maxDigits;
...
const updateDimensions = (props, container, margin, width, height) => {
  ...
  const divDimensions = container.current.node().getBoundingClientRect();
