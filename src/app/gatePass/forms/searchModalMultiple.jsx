...
const SearchModalMultiple = (props) => {
  ...
  useEffect(() => {
    if (modelName && fields) {
      ...
      if (fieldName === 'nature_work_id') {
        const searchValueMultiple = `[["company_id","in",[${company}]]]`;
  ...
  return (
    <Row>
      ...
      {(fieldName === 'vendor_id') && (
        <Modal size={(addVendorInfo && addVendorInfo.data) ? 'sm' : 'xl'} className="border-radius-50px modal-dialog-centered" isOpen={addModal}>
          <ModalHeaderComponent title={modalHead} imagePath={false} closeModalWindow={() => { onModalClose(0); onVendorClose(); }} response={addVendorInfo} />
          <ModalBody className="pt-0 mt-0">
            <AddVendor
