...
const LineAreaChart = React.memo((props) => {
  ...
  useEffect(() => {
    if (showModal && !chartId) {
      setDatasets(getDatasetsLine(chartValues.datasets, chartValues.labels, getChartColors(chartData.id), chartData.ks_dashboard_item_type, getChartSubColors(chartData), chartData.ks_show_data_value, chartData.ks_description));
  ...
  const onExpandChart = () => {
    // console.log(cd);
    // if (cd) {
    let cData = getDatasetsLine(chartValues.datasets, chartValues.labels, getChartColors(chartData.id), chartData.ks_dashboard_item_type, getChartSubColors(chartData), chartData.ks_show_data_value, chartData.ks_description);
  ...
  } else if (!chartId && chartData.ks_description && !chartData.ks_show_data_value) {
    modeBarButtonsToAdd = [
      {
        name: 'Info',
        width: 15,
              ...
              {!(dateGroupData && dateGroupData.loading) && (
                <div width="100%" height="100%" className="pr-3 pl-3" id="rechartContainer" style={{ height: '100%', width: '100%' }}>
                  <Plot
                    ...
                    data={getDatasetsLine(chartValues.datasets, chartValues.labels, getChartColors(chartData.id), chartData.ks_dashboard_item_type, getChartSubColors(chartData), chartData.ks_show_data_value, chartData.ks_description)}
