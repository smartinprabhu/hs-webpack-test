...
const DashboardIOTView = React.memo((props) => {
  const {
    dashboardCode, dashboardUuid, advanceFilter, isIAQ,
  } = props;
  ...
  const { ninjaDashboard, updateLayoutInfo } = useSelector((state) => state.analytics);
  ...
  useEffect(() => {
    if (isTimer && dashboardInterval && code) {
      ...
      dispatch(getNinjaDashboardTimer(
