...
const ChartCards = React.memo(({
  ...
  dashboardColors, customLayouts, dateFilters,
}) => {
      ...
      {dataArray && dataArray.length > 0 && dataArray.map((dl) => (
        (!dl.ks_hx_group_id || !dl.ks_hx_group_id.id)
         ...
         <div
           ...
           id={`id${dl.id}`}
           className={`${dl.ks_dashboard_item_type === 'ks_tile' || dl.ks_dashboard_item_type === 'ks_kpi' ? 'p-2' : 'p-0'} grid-layout-custom-card ${(dl.ks_model_name === 'mgmtsystem.action' && dl.ks_dashboard_item_type === 'ks_list_view') || (dl.ks_dashboard_item_type === 'ks_header') || (dl.ks_dashboard_item_type === 'ks_image') ? '' : 'shadow-card-dashboard bg-white'}`}
         >
           {(dl.ks_dashboard_item_type === 'ks_tile' || dl.ks_dashboard_item_type === 'ks_kpi') && (
           <DataCard dashboardColors={dashboardColors} code={code} dateFilters={dateFilters} editMode={editMode} selectedDateTag={selectedDateTag} dataItem={dl} height={getDimension(dl.id).h} width={getDimension(dl.id).w} />
           )}
           ...
           <BarChart code={code} selectedDateTag={selectedDateTag} dashboardColors={dashboardColors} chartData={dl} height={getDimension(dl.id).h} width={getDimension(dl.id).w} dateFilters={dateFilters} />
           )}
           {(dl.ks_dashboard_item_type === 'ks_pie_chart' || dl.ks_dashboard_item_type === 'ks_doughnut_chart') && (
           <PieChart dashboardColors={dashboardColors} chartData={dl} height={getDimension(dl.id).h} width={getDimension(dl.id).w} dateFilters={dateFilters} />
           )}
           {dl.ks_dashboard_item_type === 'ks_horizontalBar_chart' && (
           <HorizontalBarChart dashboardColors={dashboardColors} chartData={dl} height={getDimension(dl.id).h} width={getDimension(dl.id).w} dateFilters={dateFilters} />
           )}
           {dl.ks_dashboard_item_type === 'ks_list_view' && (
           <TableView chartData={dl} height={getDimension(dl.id).h} width={getDimension(dl.id).w} dateFilters={dateFilters} />
           )}
           {(dl.ks_dashboard_item_type === 'ks_line_chart' || dl.ks_dashboard_item_type === 'ks_area_chart') && (
           <LineAreaChart code={code} selectedDateTag={selectedDateTag} dashboardColors={dashboardColors} chartData={dl} height={getDimension(dl.id).h} width={getDimension(dl.id).w} dateFilters={dateFilters} />
           )}
         </div>
        )))}
      {tabLayout
