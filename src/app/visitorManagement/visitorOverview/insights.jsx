...
import {
  getVmDashboard, getVisitorRequestFilters,storeDateFilters
} from '../visitorManagementService';
import {
  truncate, generateErrorMessage, generateArrayFromValue, getAllowedCompanies, 
  getDatesOfQueryWithUtc, getDateAndTimeForDifferentTimeZones, queryGeneratorWithUtc, defaultTimeZone
} from '../../util/appUtils';
...
const insights = () => {
  ...
  const { visitorDashboard, visitorRequestCount} = useSelector((state) => state.visitorManagement);

  // useEffect(() => {
  //   const defaultFilters = [
  //     { key: "entry_status", title: "Entry Status", value: "Invited", label: "Invited", type: "inarray", Header: "Entry Status", id: "entry_status" }, { key: 'Today', value: 'Today', label: 'Today', type: 'date' }
  //   ];
  ...
  useEffect(() => {
    if (userInfo && userInfo.data && currentTab && currentTab !== 'Custom') {
      ...
      if (dates.length > 0) {
        const start = dates[0];
        const end = dates[1];
        dispatch(getVmDashboard(start, end));
      } else {
    ...
    if (userInfo && userInfo.data && currentTab && currentTab !== 'Custom' && CustomData.dashboardStatus["TE"].label==="Time Elapsed") {
      const defaultFilters = [
        { key: 'entry_status', title: 'Entry Status', value: 'time_elapsed', label: 'Time Elapsed', type: 'inarray' },{key: `${currentTab}`, value: `${currentTab}`, label: `${currentTab}`, type: 'date'} ];
        const customFiltersList = queryGeneratorWithUtc(defaultFilters, 'planned_in', userInfo.data);
       // dispatch(getVisitorRequestCount(companies, appModels.VISITREQUEST, customFiltersList));
  ...
  const total = getTotal(visitorDashboard && visitorDashboard.data ? visitorDashboard.data : []);
  ...
  const gridData = generateArrayFromValue(visitorDashboard && visitorDashboard.data ? visitorDashboard.data : [], 'ks_dashboard_item_type', 'ks_tile');
  const gridCards = gridData.sort((a, b) => a.sequence - b.sequence);

  const totalDataCount = visitorRequestCount && visitorRequestCount.length ? visitorRequestCount.length : 0;

  return (
    <>
      <Row className="m-0 ml-1 pl-4 pt-2 visitorPass-title">
        <h6 className="mt-2">
          {' '}
