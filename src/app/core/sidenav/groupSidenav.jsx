...
import BMS from '@images/sideNavImages/alarmsBlack.svg';
...
import AdminSetUpConfiguration from '../../adminSetup/navbar/navlist.json';
...
const Sidenav = () => {
  ...
  sideNavMenus?.map((eachItem) => {
    sideNavList.map((each) => {
      if (each.itemName === eachItem.name) {
  ...
  const subOption = {
    ...
    ESG: esgSidenav,
    'Admin Setup': AdminSetUpConfiguration,
  ...
  >
    {appMenus.menus[item.name].displayName}
  </Box>
) : null}
                          { /* !sidePin && menuList(item.name).length > 0 ? (
                          <>
                            <IoMdArrowDropdown
                              size={25}
                              className={
                        ...
                        {!sidePin || hoverEnable ? (
                          <Box
                            ...
                            <Menu
                              ...
                              onClose={() => onMenuClose()}
                              onMouseLeave={() => onMenuClose()}
                              ModalProps={{ hideBackdrop: false, disableScrollLock: true }}
                            >
                              {menuList(item.name).length > 0
