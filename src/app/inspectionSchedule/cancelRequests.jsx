...
import { HxInspCancelColumns } from '../commonComponents/gridColumns';
...
import { HxInspCancelModule } from '../util/field';
import {
  filterStringGenerator,
} from '../auditManagement/utils/utils';
import {
  getHxCancelRequestsCount,
  getHxCancelRequestsExport,
  getCancelRequestFilters, getHxCancelRequestsList,
  getHxInspCancelDetails,
  getInspectionCommenceDate,
} from './inspectionService';
import bcSideNav from './navbar/navlist.json';
import CancelRequestDetails from './viewer/cancelRequestDetail';
import InspectionCancelRequestBulk from './viewer/inspectionCancelRequestBulk';
...
const CancelRequests = ({ uuid }) => {
  ...
  const { apiFields } = HxInspCancelModule;

  const moduleName = 'Inspection Schedule';
  const menuName = 'Cancel Requests';

  const { userInfo, userRoles } = useSelector((state) => state.user);
  const companies = getAllCompanies(userInfo, userRoles);
  const {
    hxInspCancelCount, hxInspCancelList, hxInspCancelCountLoading,
    hxInspCancelFilters, hxInspCancelExport, hxInspCancelDetails,
  ...
  const canCreateHoliday = allowedOperations.includes(actionCodes['Create Cancel Holiday Request']);
  ...
  useEffect(() => {
    if (hxInspCancelFilters && hxInspCancelFilters.customFilters) {
      setCustomFilters(hxInspCancelFilters.customFilters);
    }
  }, [hxInspCancelFilters]);
  ...
  useEffect(() => {
    if (reload) {
      dispatch(getCancelRequestFilters([]));
      setCustomFilters([]);
      setPage(0);
      setOffset(0);
  ...
  useEffect(() => {
    if ((userInfo && userInfo.data) && (hxInspCancelCount && hxInspCancelCount.length) && startExport) {
      const offsetValue = 0;
      const customFiltersQuery = hxInspCancelFilters && hxInspCancelFilters.customFilters ? queryGeneratorWithUtc(hxInspCancelFilters.customFilters, 'requested_on', userInfo.data) : '';
      dispatch(getHxCancelRequestsExport(companies, appModels.HXINSPECTIONCANCEL, hxInspCancelCount.length, offsetValue, apiFields, customFiltersQuery, rows, sortedValue.sortBy, sortedValue.sortField, globalFilter, buttonFilterType, userId));
    }
  }, [startExport]);

  useMemo(() => {
    if (hxCreatePpmCancelRequest && hxCreatePpmCancelRequest.data) {
      const customFiltersList = hxInspCancelFilters.customFilters ? queryGeneratorWithUtc(hxInspCancelFilters.customFilters, 'requested_on', userInfo.data) : '';
      dispatch(getHxCancelRequestsCount(companies, appModels.HXINSPECTIONCANCEL, false, customFiltersList, globalFilter, false, buttonFilterType, userId));
      setCheckRows([]);
      dispatch(getHxCancelRequestsList(companies, appModels.HXINSPECTIONCANCEL, limit, offset, customFiltersList, sortedValue.sortBy, sortedValue.sortField, globalFilter, false, buttonFilterType, userId));
    }
  ...
  }, [userInfo, hxInspCancelFilters.customFilters, buttonFilterType, reset]);
  ...
  }, [userInfo, offset, sortedValue.sortBy, sortedValue.sortField, hxInspCancelFilters.customFilters, buttonFilterType, reset]);
  ...
  useEffect(() => {
    if (viewId) {
      dispatch(getHxInspCancelDetails(viewId, appModels.HXINSPECTIONCANCEL));
    }
  }, [viewId]);

  const totalDataCount = hxInspCancelCount && hxInspCancelCount.length ? hxInspCancelCount.length : 0;
  ...
  const handleRadioboxChange = (event) => {
    ...
    if (checked) {
      const oldCustomFilters = hxInspCancelFilters && hxInspCancelFilters.customFilters ? hxInspCancelFilters.customFilters : [];
      ...
      dispatch(getCancelRequestFilters(filterValues));
    } else {
  ...
  const handleCustomDateChange = (startDate, endDate) => {
    ...
    if (start && end) {
      setCustomFiltersList(filters);
      const oldCustomFilters = hxInspCancelFilters && hxInspCancelFilters.customFilters
        ? hxInspCancelFilters.customFilters
        : [];
      const filterValues = {
        states:
        hxInspCancelFilters && hxInspCancelFilters.states ? hxInspCancelFilters.states : [],
        customFilters: [...(oldCustomFilters.length > 0 ? oldCustomFilters.filter((item) => item.type !== 'date' && item.type !== 'customdate' && item.type !== 'datearray') : ''), ...filters],
        // customFilters: [
  ...
  const loading = (userInfo && userInfo.loading) || (hxInspCancelList && hxInspCancelList.loading) || (hxInspCancelCountLoading);
  ...
  const handleCustomFilterClose = (cfValue, cf) => {
    ...
    dispatch(getCancelRequestFilters(customFiltersList));
    setOffset(0);
    setPage(0);
  };
  ...
  useEffect(() => {
    if (detailArrowNext !== '') {
      setViewId(hxInspCancelList && hxInspCancelList.data && hxInspCancelList.data.length && hxInspCancelList.data[0].id);
    }
    if (detailArrowPrev !== '') {
      setViewId(hxInspCancelList && hxInspCancelList.data && hxInspCancelList.data.length && hxInspCancelList.data[hxInspCancelList.data.length - 1].id);
    }
  }, [hxInspCancelList]);
  ...
  const onFilterChange = (data) => {
    ...
    if (data.items && data.items.length) {
      if (valueCheck(data.items)) {
        ...
        uniqueCustomFilter = getNewDataGridFilterArray(HxInspCancelColumns(), uniqueCustomFilter);
        const customFilters = [...dateFilters, ...uniqueCustomFilter];
        dispatch(getCancelRequestFilters({ customFilters }));
        setOffset(0);
        setPage(0);
      } else {
        ...
        if (fData && fData.length) {
          ...
          dispatch(getCancelRequestFilters({ customFilters: CustomFilters }));
          setOffset(0);
          setPage(0);
        }
      }
  ...
  useEffect(() => {
    dispatch(
      updateHeaderData({
        ...
        moduleName,
        menuName,
        link: '/inspection-overview/cancel-requests',
  ...
  return (
    <Box>
      <CommonGrid
        ...
        tableData={
            hxInspCancelList && hxInspCancelList.data && hxInspCancelList.data.length
              ? hxInspCancelList.data
              : []
        }
        columns={HxInspCancelColumns()}
        checkboxSelection
        pagination
        ...
        exportInfo={{ err: hxInspCancelExport.err, loading: hxInspCancelExport.loading, data: hxInspCancelExport.data ? hxInspCancelExport.data : false }}
        page={currentpage}
        isButtonFilter
        ...
        createOption={{
          ...
          text: 'Create',
          func: () => onAddCancel(),
          dropdown: gpConfig && gpConfig.is_can_cancel && canCreateHoliday,
          options: [{ id: 1, name: 'Cancellation on Holidays', func: () => onAddCancelHoliday() }],
        }}
        setRows={setRows}
        rows={rows}
        visibleColumns={visibleColumns}
        setVisibleColumns={setVisibleColumns}
        filters={filterStringGenerator(hxInspCancelFilters)}
        onFilterChanges={debouncedOnFilterChange}
        loading={hxInspCancelList && hxInspCancelList.loading}
        err={hxInspCancelList && hxInspCancelList.err}
      ...
      >
        <DrawerHeader
         ...
         // isEditable={isEditable}
          onClose={() => onViewReset()}
          onEdit={() => {
            setEditId(hxInspCancelDetails && (hxInspCancelDetails.data && hxInspCancelDetails.data.length > 0) ? hxInspCancelDetails.data[0].id : false);
            showEditModal(!editModal);
          }}
          onPrev={() => {
            const prevId = getNextPreview(viewId, 'Prev', hxInspCancelList);
            if (prevId === 0) {
          ...
          onNext={() => {
            const nextId = getNextPreview(viewId, 'Next', hxInspCancelList);
            if (nextId === 0) {
