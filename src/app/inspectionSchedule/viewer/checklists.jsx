...
import { getInspectionChecklists } from '../inspectionService';
...
const Checklists = () => {
  ...
  useEffect(() => {
    if (orderData && orderData.id && orderData.state && orderData.state === 'done') {
      dispatch(getInspectionChecklists(orderData.id));
    }
  }, [inspectionChecklistDetail]);

  console.log(inspectionChecklists);

  const isChecklistJson = (inspectionChecklists && inspectionChecklists.data && inspectionChecklists.data.length && inspectionChecklists.data[0].checklist_json_data && inspectionChecklists.data[0].checklist_json_data !== '');
      ...
      if (gId && assetData[i].mro_quest_grp_id && assetData[i].mro_quest_grp_id.id && assetData[i].mro_quest_grp_id.id === gId) {
        tableTr.push(
          <tr key={i}>
            ...
            <td className="p-2">{getDefaultNoValue(inspDeata.employee_name)}</td>
  ...
  return (
    <>
      ...
      {!isErr && inspDeata && !isChecklist && !isChecklistJson && !loading && (
