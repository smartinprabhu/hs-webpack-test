...
const searchModal = (props) => {
  ...
  useEffect(() => {
    if (modelName && fields) {
      ...
      if (searchValue && fieldName === 'parent_id') {
        searchValueMultiple = `[${searchValueMultiple}["uuid","ilike","${searchValue}"]]`;
      }
  ...
  const handleRowClick = (data) => {
    setSelected(data.id);
    const items = { id: data.id, uuid: data.uuid ? data.uuid : '' };
    ...
    for (let i = 0; i < assetData.length; i += 1) {
      tableTr.push(
        <tr className={selectedId === assetData[i].id ? 'text-info cursor-pointer' : 'cursor-pointer'} onClick={() => handleRowClick(assetData[i])} key={i}>
          {fieldName === 'parent_id'
            ? <td className="p-2"><span className="font-weight-400">{getDefaultNoValue(assetData[i].uuid ? assetData[i].uuid : '')}</span></td> : ''}
