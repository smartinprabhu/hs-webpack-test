...
const UploadDocuments = ({
  model, isCapture, isRequired, onCapture, saveData, limit, setFieldValue, uploadFileType, labelName, fileImage,
}) => {
  ...
  useEffect(() => {
    if (isCapture && fileImage) {
      setSavedFileImage(fileImage);
    }
  }, [fileImage]);
  ...
  const handleFiles = (files) => {
    ...
    if (files !== undefined && filesList.length < filesLimit) {
      ...
      if (uploadFileType && uploadFileType === 'images') {
        if (name && getFileExtension(name).match(/^(jpg|jpeg|svg|png)$/i)) {
        ...
        if (name && getFileExtension(name).match(/^(jpg|jpeg|png|pdf|doc|docx|ppt|xls|xlsx|csv)$/i)) {
          if (isMemorySizeLarge(files.fileList)) {
            setFilesList([...filesList, ...mergeArray([files.base64], files.fileList, companyId, model)]);
            if (setFieldValue) {
              const remfile = `data:${files.fileList[0].type};base64,`;
      ...
      /* } else if (uploadFileType && uploadFileType === 'images') {
        ...
        if (type && type.match(/.(jpg|jpeg|png|pdf|doc|docx|ppt|xls|xlsx|csv)$/i)) {
          if (isMemorySizeLarge(files.fileList)) {
            setFilesList([...filesList, ...mergeArray([files.base64], files.fileList, companyId, model)]);
            setFinish(true);
            setDocumentReady(Math.random());
    ...
    <>
      ...
      >
        ...
        <Box>
          <Box sx={{ display: 'flex', justifyContent: 'center' }}>
            <ReactFileReader
              ...
              fileTypes={uploadFileType && uploadFileType === 'images' ? ['.png', '.jpg', '.jpeg'] : ['.csv', '.png', '.jpg', '.jpeg', '.doc', '.docx', '.xls', '.xlsx', '.ppt', '.pdf']}
              base64
            >
              <Button
                variant="contained"
        ...
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: '20px' }}>
          ...
          {uploadPhoto && uploadPhoto.length && uploadPhoto.length > 0 && !savedFileImage ? (
            <>
              {uploadPhoto.map((fl, index) => (
                <div className="position-relative mt-3 mr-3">
                  ...
                  {(getFileExtension(fl.datas_fname) !== 'png' && getFileExtension(fl.datas_fname) !== 'jpeg' && getFileExtension(fl.datas_fname) !== 'jpg') && (
                    <>
                      <img
                        src={fileMiniIcon}
                        alt={fl.name}
        ...
        <Stack spacing={2} sx={{ width: '100%' }}>
          <Snackbar open={imgValidation} autoHideDuration={2000} onClose={handleSizeClose}>
            <Alert onClose={handleImageClose} severity="error" sx={{ width: '100%' }}>
              {uploadFileType && uploadFileType === 'images' ? 'Upload images only' : 'Allowed types are .csv, .png, .jpg, .jpeg, .doc, .docx, .xls, .xlsx, .ppt, .pdf'}
            </Alert>
          </Snackbar>
        </Stack>
        <Dialog
