...
const Neighbourhoods = () => {
  ...
  const { employeeDetails, employeeNeighbours } = useSelector((state) => state.employee);

  useEffect(() => {
    if (employeeDetails && employeeDetails.data) {
      const ids = employeeDetails.data.length > 0
        ? employeeDetails.data[0].space_neighbour_ids : [];
  ...
  function getRow(assetData) {
    ...
    for (let i = 0; i < assetData.length; i += 1) {
      tableTr.push(
        <tr key={i}>
          <td
            className="p-2 min-width-160 text-info font-weight-700"
          >
